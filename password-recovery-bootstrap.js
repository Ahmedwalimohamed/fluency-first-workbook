...
async function resetAccountPassword(body){
 ...
 try{
  ...
  if(accessToken){
   ...
   q=await client.query("select id,username,role,name,whatsapp_number,login_token,school_id from users where login_token=$1 and role='student' for update",[accessToken]);
   ...
   q=await client.query("select id,username,role,name,whatsapp_number,login_token,school_id from users where lower(username)=lower($1) and role in ('admin','teacher','student') for update",[username]);
   const stored=q.rowCount?normalizeWhatsapp(q.rows[0].whatsapp_number):null;
   if(!q.rowCount||!stored||stored!==whatsappNumber)throw userError(404,'The username and registered WhatsApp number do not match an EnglishGate account.');
  }
  const user=q.rows[0];
  const destination=normalizeWhatsapp(user.whatsapp_number);
  if(!destination){
   const help=user.role==='admin'?'Ask the platform owner to update the WhatsApp number on this admin account.':'Ask your administrator to update the WhatsApp number on this account.';
   throw userError(409,'No registered WhatsApp number is available for this account. '+help);
  }
  ...
  return {ok:true,delivery:'whatsapp',role:user.role};
 }catch(error){
  try{await client.query('rollback')}catch{}
  if(error?.publicMessage)throw error;
  console.error('Password reset delivery failed:',String(error?.message||error).slice(0,300));
...
async function passwordResetHandler(req,res){
 try{
  const result=await resetAccountPassword(req.body||{});
...
async function passwordResetPageHandler(req,res){
 ...
 try{
  await resetAccountPassword({username,whatsappNumber});
  res.set('Cache-Control','no-store');
