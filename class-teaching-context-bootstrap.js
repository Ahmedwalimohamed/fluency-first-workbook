...
function install(app){
 ...
 nativeUse.call(app,async(req,res,next)=>{
  ...
  try{await schema();const owns=await pool.query('select 1 from classes where id=$1 and teacher_id=$2',[classId,u.id]);if(owns.rowCount){const lesson=Math.max(1,Number(req.body?.lessonNumber)||1),section=Math.max(0,Number(req.body?.sectionIndex)||0);await pool.query(`insert into class_teaching_contexts(teacher_id,class_id,lesson_number,section_index,updated_at) values($1,$2,$3,$4,now()) on conflict(teacher_id,class_id) do update set lesson_number=excluded.lesson_number,section_index=excluded.section_index,updated_at=now()`,[u.id,classId,lesson,section])}}catch(e){console.error('class teaching context mirror error',e)}next();
 });
 nativeGet.call(app,'/api/teacher/class-contexts',async(req,res)=>{
  ...
  try{await schema();const q=await pool.query(`select t.class_id as "classId",t.lesson_number as "lessonNumber",t.section_index as "sectionIndex",t.updated_at as "updatedAt" from class_teaching_contexts t join classes c on c.id=t.class_id where t.teacher_id=$1 and c.teacher_id=$1 order by t.updated_at desc`,[u.id]);res.set('Cache-Control','no-store');res.json({contexts:q.rows})}catch(e){console.error('class teaching contexts error',e);res.status(500).json({error:'Teaching positions are temporarily unavailable.'})}
 });
}
