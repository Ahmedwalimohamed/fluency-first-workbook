...
const MCQ_BANK={
  'going to':{
    aliases:['going to','be going to','future plans'],
    title:'Going to: Future Plans',
    tip:'Use am/is/are + going to + the base form of the verb.',
    questions:[
  ...
  'present simple':{
    aliases:['present simple','simple present','daily routine','routines'],title:'Present Simple: Routines',tip:'Use the base verb with I/you/we/they and add -s/-es with he/she/it.',questions:[
  ...
  'past simple':{
    aliases:['past simple','simple past','yesterday','last weekend'],title:'Past Simple: Finished Actions',tip:'Use the past form for finished actions in the past.',questions:[
  ...
  'present continuous':{
    aliases:['present continuous','continuous','right now','now'],title:'Present Continuous: Now',tip:'Use am/is/are + verb-ing for actions happening now.',questions:[
  ...
  'comparatives':{
    aliases:['comparative','comparatives','bigger than','more than'],title:'Comparatives',tip:'Use -er for many short adjectives and more for many longer adjectives, usually followed by than.',questions:[
...
function ensureSchema(){
  if(!schemaPromise){schemaPromise=(async()=>{
    await pool.query(`create table if not exists live_tasks(
      id text primary key,
      class_id text not null references classes(id) on delete cascade,
      teacher_id text not null references users(id) on delete cascade,
      title text not null,
      task_type text not null check(task_type in ('mcq','writing')),
    ...
    await pool.query(`create table if not exists live_task_submissions(
      task_id text not null references live_tasks(id) on delete cascade,
...
function findBank(request){
  const text=String(request||'').toLowerCase();
  for(const [key,entry] of Object.entries(MCQ_BANK))if(entry.aliases.some(a=>text.includes(a)))return {key,entry};
  return null;
}
...
function extractTopic(request){
  const raw=cleanText(request,300);
  const patterns=[/(?:about|on|for)\s+(.+?)(?:\s+(?:for|in)\s+\d+\s*(?:minutes?|mins?))?[.!?]*$/i,/writing\s+task\s*[:\-]?\s*(.+)$/i];
  for(const p of patterns){const m=raw.match(p);if(m?.[1])return cleanText(m[1].replace(/\b\d+\s*(?:mcq|mcqs|questions?)\b/ig,''),80)}
  return 'this topic';
}
function prepareTask(request){
  ...
  if(wantsWriting){
    const topic=extractTopic(text),bank=findBank(text),minutes=parseMinutes(text,7);
    let instructions=`Write 5–7 clear sentences about ${topic}. Use complete sentences and check your work before submitting.`;
    let minWords=45;
    if(bank?.key==='going to'){instructions='Write 5–7 sentences about your future plans. Use “going to” at least three times and include at least one negative or question form.';minWords=50}
    return {taskType:'writing',title:`Writing · ${titleCase(topic)}`.slice(0,100),durationSeconds:minutes*60,content:{instructions,minWords,topic}};
  }
  const bank=findBank(text);
  if(!bank)throw Object.assign(new Error('No free MCQ bank matches that topic yet. Try “going to”, “present simple”, “past simple”, “present continuous”, or “comparatives”, or create a writing task on any topic.'),{status:422});
  const count=Math.min(parseCount(text),bank.entry.questions.length),minutes=parseMinutes(text,5);
  const questions=bank.entry.questions.slice(0,count).map((q,i)=>({id:`q${i+1}`,...q}));
  return {taskType:'mcq',title:bank.entry.title,durationSeconds:minutes*60,content:{topic:bank.key,tip:bank.entry.tip,questions}};
}
function validateLaunch(body){
  const taskType=String(body?.taskType||''),title=cleanText(body?.title,100),durationSeconds=clamp(Number(body?.durationSeconds)||300,60,3600),requestText=cleanText(body?.requestText,500),content=body?.content;
  if(!['mcq','writing'].includes(taskType)||title.length<2||!content||typeof content!=='object')throw Object.assign(new Error('Invalid live task.'),{status:400});
  if(taskType==='mcq'){
    const qs=Array.isArray(content.questions)?content.questions.slice(0,10):[];if(!qs.length)throw Object.assign(new Error('Add at least one MCQ.'),{status:400});
    const questions=qs.map((raw,i)=>{const q=cleanText(raw.q,300),options=Array.isArray(raw.options)?raw.options.slice(0,4).map(x=>cleanText(x,220)):[],answer=Number(raw.answer),explanation=cleanText(raw.explanation,350);if(q.length<3||options.length<2||options.some(x=>!x)||!Number.isInteger(answer)||answer<0||answer>=options.length)throw Object.assign(new Error(`Check question ${i+1} and its correct answer.`),{status:400});return {id:`q${i+1}`,q,options,answer,explanation}});
    return {taskType,title,durationSeconds,requestText,content:{topic:cleanText(content.topic,80),tip:cleanText(content.tip,300),questions}};
  }
  const instructions=cleanText(content.instructions,1200),minWords=clamp(Number(content.minWords)||40,1,500);if(instructions.length<5)throw Object.assign(new Error('Add writing instructions.'),{status:400});
  return {taskType,title,durationSeconds,requestText,content:{topic:cleanText(content.topic,80),instructions,minWords}};
}
...
function publicTask(row,includeAnswers=false){
  ...
  let safeContent;
  if(row.task_type==='mcq')safeContent={topic:content.topic||'',tip:content.tip||'',questions:(content.questions||[]).map(q=>includeAnswers?q:{id:q.id,q:q.q,options:q.options})};
  else safeContent={topic:content.topic||'',instructions:content.instructions||'',minWords:content.minWords||0};
  return {id:row.id,classId:row.class_id,className:row.class_name||'',title:row.title,taskType:row.task_type,durationSeconds:row.duration_seconds,status:row.status,startsAt:row.starts_at,endsAt:row.ends_at,content:safeContent};
}
function gradeMcq(taskContent,answers){
  const qs=Array.isArray(taskContent?.questions)?taskContent.questions:[];let correct=0;
  const detail=qs.map((q,i)=>{const selected=Number(answers?.[q.id]),ok=Number.isInteger(selected)&&selected===Number(q.answer);if(ok)correct++;return {id:q.id,selected:Number.isInteger(selected)?selected:null,answer:q.answer,correct:ok,explanation:q.explanation||''}});
  return {correct,total:qs.length,score:qs.length?Math.round(correct*100/qs.length):0,detail};
}
