...
function isTeacher(){return String(document.getElementById('pageEyebrow')?.textContent||'').trim()==='Teacher'}
...
function goToSaved(classId,ctx){
 // Use EnglishGate's existing teacher navigation so the same live-book state and safeguards are preserved.
...
function boot(){enhance();new MutationObserver(schedule).observe(document.body,{subtree:true,childList:true,characterData:true});window.addEventListener('focus',()=>{load(true).then(enhanceCards)})}
