...
function grammarHtml(data){
 ...
 return `<section class="grammar-live-lesson" data-grammar-live>
  <header class="grammar-live-hero">
   <div><span class="grammar-live-kicker">Grammar Lesson</span><h3>${esc(data.focus)}</h3><p>Learn the pattern, notice it in context, practise it, then use it yourself.</p></div>
  </header>

  <section class="grammar-live-rule">
   <div class="grammar-step-number">1</div>
   <div><small>Understand the grammar</small><h4>Mini rule</h4><p>${esc(data.rule)}</p>${sample?`<div class="grammar-example"><b>Example</b><span>${esc(sample)}</span></div>`:''}</div>
  </section>

  <section class="grammar-live-practice">
   <div class="grammar-live-section-head"><div><span class="grammar-step-number">2</span><div><small>Guided practice</small><h4>Choose the best answer</h4></div></div><strong data-grammar-progress>1 / ${qs.length}</strong></div>
   <div data-grammar-question-box>${qs.map((q,i)=>questionCard(q,i,qs.length)).join('')}</div>
   <div class="grammar-live-nav"><button type="button" class="ghost-btn" data-grammar-prev disabled>← Previous</button><button type="button" class="primary-btn" data-grammar-next disabled>${qs.length===1?'Finish practice':'Next →'}</button></div>
  </section>

  <section class="grammar-live-apply">
   <div class="grammar-step-number">3</div>
   <div class="grammar-live-apply-copy"><small>Apply it</small><h4>Make it yours</h4><p>Write one true sentence using <strong>${esc(data.focus)}</strong>.</p><textarea data-grammar-production rows="3" placeholder="Write your own sentence here..."></textarea><div class="grammar-production-actions"><button type="button" class="primary-btn" data-grammar-production-check>Check my sentence</button><span data-grammar-production-feedback aria-live="polite"></span></div></div>
  </section>
 </section>`;
}
...
function decorateStageCards(root=document){
 root.querySelectorAll('.eg-stage,.teacher-presentation-stage').forEach(card=>{
  ...
  if(/^B2\s+LIFT$/i.test(label)||/^Grammar Lesson$/i.test(label)){
   strong.textContent='Grammar Lesson';
   card.classList.remove('b2-lift-stage');card.classList.add('grammar-lesson-stage');
   card.querySelectorAll('.b2-stage-badge').forEach(x=>x.remove());
  }
 });
 // Remove any legacy exact heading that may have been rendered before this patch loaded.
 root.querySelectorAll('h1,h2,h3,h4,small,span').forEach(node=>{if(/^B2\s+LIFT$/i.test(clean(node.textContent))&&node.children.length===0)node.textContent='Grammar Lesson'});
}
...
function showQuestion(root,index){
 ...
 const progress=root.querySelector('[data-grammar-progress]');if(progress)progress.textContent=`${nextIndex+1} / ${cards.length}`;
 ...
 if(next){next.textContent=nextIndex===cards.length-1?'Finish practice':'Next →';next.disabled=!cards[nextIndex].dataset.answered;}
}

document.addEventListener('click',e=>{
 const option=e.target.closest?.('[data-grammar-option]');
 if(option){
  const card=option.closest('[data-grammar-question]'),root=option.closest('[data-grammar-live]');if(!card||!root)return;
  ...
  const feedback=card.querySelector('[data-grammar-feedback]');
  if(feedback){feedback.className='grammar-live-feedback '+(correct?'is-good':'is-bad');feedback.textContent=correct?'Correct. Notice why this form matches the meaning.':'Not quite. Compare your choice with the highlighted correct answer.';}
  const state=questionState(root),next=root.querySelector('[data-grammar-next]');if(next)next.disabled=false;
  return;
 }
 ...
 const prev=e.target.closest?.('[data-grammar-prev]');
 if(prev){const root=prev.closest('[data-grammar-live]'),state=questionState(root);showQuestion(root,state.index-1);return;}
 const check=e.target.closest?.('[data-grammar-production-check]');
 if(check){
  const root=check.closest('[data-grammar-live]'),box=root?.querySelector('[data-grammar-production]'),feedback=root?.querySelector('[data-grammar-production-feedback]');if(!box||!feedback)return;
  const text=clean(box.value);
  if(text.length<8){feedback.textContent='Write one complete sentence first.';feedback.className='is-bad';return;}
  feedback.textContent='Good. Read it aloud and check that the grammar matches the meaning you want.';feedback.className='is-good';
 }
},true);
