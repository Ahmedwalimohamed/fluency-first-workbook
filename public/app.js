...
function makeVocabulary(topic,data){
  ...
  return {items:[
    ...
    {type:'choice',q:'A classmate asks for a word meaning â'+data.v[3][1]+'.â What should you suggest?',options:wordOptions(3,3),answer:data.v[3][0],tag:'vocabulary:context'},
    {type:'choice',q:'What does â'+data.v[4][0]+'â mean?',options:defOptions(4),answer:data.v[4][1],tag:'vocabulary:reverse-meaning'},
    {type:'choice',q:'Choose the correct definition of â'+data.v[5][0]+'â.',options:defOptions(5,1),answer:data.v[5][1],tag:'vocabulary:reverse-meaning'},
    ...
    {type:'open',q:'Use â'+data.v[1][0]+'â in one complete sentence about '+topic.toLowerCase()+'.',min:6,tag:'vocabulary:production'}
  ]};
}
function makeReading(topic,level,data){
  ...
  return {title:topic+" Â· Reading & Listening",text,readingText:text,questions:[
...
const LADDER_SKILL_GOALS={
 vocabulary:[
  'I can recognize the meaning of todayâs key words and expressions.',
...
function ladderQuestionStage(index,total){
 ...
 return `Level ${i+1} Â· ${stage.label}`;
}
...
function learningLadderHtml(l,skill,totalItems=10){
 ...
 return `<section class="eg-learning-ladder teacher-only-learning-ladder" aria-label="Five-step learning ladder">
  ...
  <div class="eg-ladder-track">${LEARNING_LADDER.map((stage,i)=>{
   ...
   return `<article class="eg-ladder-step ${done?'is-reached':''} ${current?'is-current':''}"><span class="eg-ladder-number">${done?'â':i+1}</span><div><small>Level ${i+1}</small><strong>${stage.label}</strong><p>${escapeHtml(goals[i]||stage.goal)}</p></div><b>${done?'Reached':current?'Next':'Later'}</b></article>`
  }).join('')}</div>
...
function ladderResultHtml(result){
 ...
 return `<div class="eg-ladder-result"><div><span class="eg-skill-kicker">Your learning ladder</span><strong>${highest>=5?'Level 5 Â· Perform reached':highest?`Level ${highest} Â· ${LEARNING_LADDER[highest-1].label} reached`:'Start with Level 1 Â· Recognize'}</strong><p>${highest>=5?'You completed the full progression from recognition to independent use.':`Next goal: Level ${highest+1} Â· ${next.label}. ${next.goal}`}</p></div><div class="eg-ladder-mini">${LEARNING_LADDER.map((stage,i)=>`<span class="${i<highest?'done':i===highest&&highest<5?'current':''}">${i<highest?'â':i+1}</span>`).join('')}</div></div>`;
}
...
const NAV={student:[['home','â','Home'],['course','â£','My book'],['my-writings','â','My Writings'],['progress','â','Progress'],['leaderboard','â¥','Leaderboard']],teacher:[['teacher-home','â','Overview'],['teach','â£','Teach'],['teacher-workbooks','â¤','Workbooks'],['students','â','Students'],['writings','â','Writings'],['classes','â¤','Classes'],['leaderboard','â¥','Leaderboard'],['weaknesses','â³','Needs review'],['reports','â¦','Reports']],admin:[['admin-home','â','Overview'],['admin-books','â£','Books'],['admin-classes','â¤','Classes'],['admin-reports','â¦','Reports'],['admin-writings','â','Writings'],['admin-admins','â','Admins'],['admin-teachers','â','Teachers'],['admin-students','â','Students']]};
...
function vocabularyRow(line){
 ...
 const modern=t.match(/^(.+?)\s+[ââ-]\s+(.+?)\.\s*Example:\s*(.+)$/i);
 if(modern){
...
function isVocabularyHeader(text){return /^WORD(?:\s+[ââ-]\s+MEANING\s+[ââ-]\s+EXAMPLE|\s+EXAMPLE)$/i.test(String(text||'').trim())}
...
function normalizeVocabWord(word){return String(word||'').replace(/[ââ"]/g,'').replace(/\s+/g,' ').trim().toLowerCase()}
...
function lessonVocabExample(word,l,q={}){
 ...
 if(/[.!?]$/.test(rawWord)||/\b(?:I|you|we|they|he|she|my|your|our)\b/i.test(rawWord)){
  return /\?$/.test(rawWord)?'â'+rawWord+'â she asked during the conversation.':'â'+rawWord+'â she said during the conversation.';
 }
...
function extractQuotedTarget(question){
 ...
 const patterns=[
  /What does\s+[â"]([^â"]+)[â"]\s+mean/i,
  /definition of\s+[â"]([^â"]+)[â"]/i,
  /Match\s+[â"]([^â"]+)[â"]\s+to its meaning/i,
  /meaning of\s+[â"]([^â"]+)[â"]/i
 ];
...
function vocabFeedbackHtml(meta,correct){
 ...
 return '<div class="mcq-learning-feedback '+(correct?'is-correct':'is-incorrect')+'" role="status">'+
   '<strong class="mcq-learning-status">'+(correct?'â Correct':'â Incorrect')+'</strong>'+
...
function openVocabularyMeaning(word,example,meaning=''){const definition=String(meaning||'').trim()||vocabMeaning(word);showModal(`<div class="section-head"><div><span class="role-kicker">Vocabulary meaning</span><h3>${escapeHtml(word)}</h3></div><button class="icon-btn" data-close>Ã</button></div><div class="vocab-meaning-card"><small>Meaning</small><p>${escapeHtml(withPeriod(definition))}</p><small>Example</small><p>${escapeHtml(example)}</p></div>`);document.querySelector('[data-close]').onclick=closeModal}
...
function findSomeoneHint(statement){
 ...
 if((m=s.match(/^doesn['â']?t\s+like\s+(.+)$/i)))return "I don't like "+m[1]+' because ___.';
 if((m=s.match(/^can['â']?t\s+(.+)$/i)))return "I can't "+m[1]+'.';
...
function discussionHintHtml(text,type){
 if(type==='find')return '<small class="discussion-hint find-someone-hint"><b>Hint:</b> â'+escapeHtml(findSomeoneHint(text))+'â</small>';
 return '<small class="discussion-hint"><b>Hint:</b> â'+escapeHtml(thinkTalkHint(text))+'â</small>';
}
function findSomeoneCardHtml(text){
 const hint=discussionHintHtml(text,'find'),student=session?.role==='student';
 if(student)return '<label class="find-someone-card find-someone-student-card"><span class="find-someone-check"><input type="checkbox" data-live-check="'+escapeAttr(text)+'"><span aria-hidden="true">â</span></span><span class="find-someone-content"><strong>'+escapeHtml(text)+'</strong>'+hint+'</span></label>';
 return '<button class="find-someone-card find-someone-teacher-card" type="button" data-find-spotlight data-find-text="'+escapeAttr(text)+'"><span class="find-someone-number" aria-hidden="true">â</span><span class="find-someone-content"><strong>'+escapeHtml(text)+'</strong>'+hint+'</span><span class="find-someone-spotlight-label">Spotlight â¶</span></button>';
}
...
function liveLineHtml(line,mode='normal'){
 ...
 if(/^â/.test(t)){const text=t.replace(/^â\s*/,'');if(mode==='find')return findSomeoneCardHtml(text);return session?.role==='student'?'<label class="live-check-row"><input type="checkbox" data-live-check="'+escapeAttr(text)+'"><span class="live-check-copy"><b>'+escapeHtml(text)+'</b></span></label>':'<div class="live-check-row is-readonly"><span class="live-check-copy"><b>'+escapeHtml(text)+'</b></span></div>'}
...
function setBrowserSpeechStatus(status,play,text){if(status)status.textContent=text;if(play)play.textContent=/Playing|Paused/.test(text)&&text!=='Paused'?'ââ':'â¶'}
function playBrowserSpeech(text,rate,status,play,restart=false){
 ...
 if(!restart&&activeBrowserSpeech&&activeBrowserSpeechKey===key&&synth.speaking){
  if(synth.paused){synth.resume();setBrowserSpeechStatus(status,play,'Playing Â· browser voice')}else{synth.pause();setBrowserSpeechStatus(status,play,'Paused')}
 ...
 utterance.onstart=()=>setBrowserSpeechStatus(status,play,'Playing Â· browser voice');
...
function dialogueTranscriptTurns(text){
 const source=String(text||'').replace(/\s+/g,' ').trim(),re=/(?:^|(?<=[.!?])\s+)([A-Z][A-Za-z'â-]{1,24}(?:\s+[A-Z][A-Za-z'â-]{1,24})?):\s*/g,matches=[...source.matchAll(re)];
...
function liveAudioPlayerHtml(script,speakers=[]){
 ...
 return '<section class="audio-player live-audio-player" data-live-audio-player data-audio-text="'+escapeAttr(text)+'" data-audio-speakers="'+escapeAttr(JSON.stringify(speakerData))+'">'+
  '<div class="live-audio-heading"><div><span class="stage-badge">Listening audio</span><strong>Listen first â do not read the transcript yet.</strong></div><span data-live-audio-status>Ready</span></div>'+
  '<div class="audio-player-main">'+
   '<button class="play-btn" type="button" data-live-audio-play aria-label="Play listening audio">â¶</button>'+
   '<button class="audio-icon-btn" type="button" data-live-audio-restart aria-label="Restart listening audio">âº</button>'+
   ...
   '<select class="audio-speed" data-live-audio-speed aria-label="Playback speed"><option value=".85">0.85Ã</option><option value="1" selected>1Ã</option><option value="1.15">1.15Ã</option><option value="1.3">1.3Ã</option></select>'+
  '</div>'+
  '<details class="live-audio-transcript"><summary>Transcript Â· open after listening</summary><div class="live-audio-transcript-body">'+dialogueTranscriptHtml(text,'is-live')+'</div></details>'+
 '</section>'
}
...
function wireLiveAudioPlayers(root=document){
 root.querySelectorAll('[data-live-audio-player]').forEach(player=>{
  ...
  const sync=audio=>{if(current)current.textContent=fmt(audio.currentTime);if(duration)duration.textContent=fmt(audio.duration);if(seek&&Number.isFinite(audio.duration)&&audio.duration>0)seek.value=String(audio.currentTime/audio.duration*100);if(play)play.textContent=audio.paused?'â¶':'ââ'};
  const bind=audio=>{audio.playbackRate=Number(speed?.value)||1;audio.onloadedmetadata=()=>sync(audio);audio.ontimeupdate=()=>sync(audio);audio.onplay=()=>{if(status)status.textContent='Playing Â· natural English voice';sync(audio)};audio.onpause=()=>{if(status&&audio.currentTime<audio.duration)status.textContent='Paused';sync(audio)};audio.onended=()=>{if(status)status.textContent='Finished';sync(audio)};audio.onerror=()=>{player.dataset.audioFallback='1';if(status)status.textContent='Ready Â· browser voice';if(seek)seek.disabled=true};return audio};
...
function liveSectionHeading(line){
 ...
 const patterns=[
  /^(?:PAGE\s+\d+\s*[â-]\s*)?(WARM UP|VOCABULARY|READING|GRAMMAR|LISTENING|FLUENCY MISSION|CLASSROOM CHALLENGE|REFLECTION(?:\s*&\s*HOMEWORK)?|HOMEWORK)$/i,
...
function writingLessonLabel(w){const l=lessonById(w.lessonId);return l?`Lesson ${l.number} Â· ${l.title}`:'English writing'}
...
function writingExcerpt(text,max=360){const clean=String(text||'').replace(/\s+/g,' ').trim();return clean.length>max?clean.slice(0,max).trimEnd()+'â¦':clean}
...
async function toggleWritingLike(w,btn,after){
 ...
 try{
  ...
  showModal(`<div class="section-head"><div><span class="role-kicker">My Writings</span><h3>Could not update thumbs up</h3><p class="muted">${escapeHtml(e.message)}</p></div><button class="icon-btn" data-close>Ã</button></div>`);
...
function writingCardHtml(w,index){
 ...
 return `<article class="writing-review-card" data-writing-card="${index}">
  ...
  <button class="writing-review-body" type="button" data-writing-read="${index}" aria-label="Read full writing by ${escapeAttr(w.studentName)}">
   <span class="writing-review-quote" aria-hidden="true">â</span>
   <p>${escapeHtml(excerpt)}</p>
  </button>
  <footer class="writing-review-actions">
   <button class="writing-like-btn ${w.likedByMe?'is-liked':''}" type="button" data-writing-like="${index}" ${w.canLike?'':'disabled'} aria-pressed="${w.likedByMe?'true':'false'}"><span aria-hidden="true">ð</span><strong>${w.likeCount}</strong><span>${w.likeCount===1?'thumbs up':'thumbs up'}</span></button>
...
function writingPaginationHtml(total){
 ...
 return `<nav class="writing-pagination" aria-label="Writing pages"><button class="ghost-btn" id="writingPrevPage" type="button" ${writingPage<=1?'disabled':''}>â Previous</button><span>Page <strong>${writingPage}</strong> of ${pages}</span><button class="ghost-btn" id="writingNextPage" type="button" ${writingPage>=pages?'disabled':''}>Next â</button></nav>`
}
...
async function createWritingPoster(w){
 ...
 ctx.font='700 31px Arial, sans-serif';ctx.fillText(IOU_PUBLIC_NAME,80,158);ctx.font='500 27px Arial, sans-serif';ctx.fillText(IOU_PUBLIC_CONTACT+'  Â·  '+IOU_PUBLIC_SITE,80,200);
 ...
 if(lines.length>22){lines=lines.slice(0,22);lines[21]=lines[21].replace(/[.,;:!?]?$/,'')+'â¦'}
 ...
 ctx.fillStyle='#F8FAFC';ctx.fillRect(55,1705,970,150);ctx.fillStyle='#102465';ctx.font='700 31px Arial, sans-serif';ctx.fillText(IOU_PUBLIC_NAME,95,1765);ctx.font='500 28px Arial, sans-serif';ctx.fillText('Contact: '+IOU_PUBLIC_CONTACT+'  Â·  '+IOU_PUBLIC_SITE,95,1813);
...
async function shareWritingPoster(index,btn){
 const w=writingFeed[index];if(!w?.canShare)return;const original=btn?.textContent;if(btn){btn.disabled=true;btn.textContent='Creating posterâ¦'}
 try{
  ...
  if(navigator.share&&navigator.canShare&&navigator.canShare({files:[file]})){await navigator.share({files:[file],title:'My English Writing',text:'My English writing at '+IOU_PUBLIC_NAME+'. Contact: '+IOU_PUBLIC_CONTACT})}
   ...
   showModal(`<div class="section-head"><div><span class="role-kicker">Poster ready</span><h3>Share it on WhatsApp Status</h3><p class="muted">The branded poster has been saved. Open WhatsApp â Updates â My status and choose the image.</p></div><button class="icon-btn" data-close>Ã</button></div><div class="feedback good"><strong>${IOU_PUBLIC_NAME}</strong><br>${IOU_PUBLIC_CONTACT} is already printed on the poster.</div>`);document.querySelector('[data-close]').onclick=closeModal
  }
 }catch(e){if(e?.name!=='AbortError'){showModal(`<div class="section-head"><div><span class="role-kicker">Share writing</span><h3>Could not create the poster</h3><p class="muted">${escapeHtml(e.message||'Try again.')}</p></div><button class="icon-btn" data-close>Ã</button></div>`);document.querySelector('[data-close]').onclick=closeModal}}
...
function studentHome(){
 ...
 const recentHtml=recent.length?recent.map(a=>{
  ...
  return `<button class="eg-premium-recent-row" type="button" data-recent-lesson="${escapeAttr(a.lessonId)}"><span class="eg-recent-skill-icon" data-skill="${escapeAttr(a.skill||'practice')}" aria-hidden="true"></span><span class="eg-recent-copy"><strong>${escapeHtml(lesson)}</strong><small>${escapeHtml(skill)} Â· ${new Date(a.at).toLocaleDateString()}</small></span><b>${escapeHtml(score)}</b><span class="eg-recent-chevron" aria-hidden="true">âº</span></button>`;
 }).join(''):'<div class="eg-premium-empty"><span aria-hidden="true">â¦</span><div><strong>Your practice history starts here.</strong><p>Complete an activity and your latest results will appear on this page.</p></div></div>';
 title('EnglishGate','Home');
 $('content').innerHTML=`<section class="eg-student-home eg-premium-home">
  <div class="eg-premium-alert"><span class="eg-alert-icon" aria-hidden="true">â</span><div class="eg-alert-message">${studentTickerHtml(sid)}</div><span class="eg-alert-encourage">Keep going! <b>â</b></span></div>

  <header class="eg-premium-welcome">
   <div class="eg-premium-welcome-main">${studentAvatarMarkup(studentPhoto(sid),session.name,'eg-home-student-photo')}<div class="eg-premium-greeting"><span class="eg-premium-level">${bookLevel}</span><h1>Hi, ${firstName}.</h1><p>Great to see you again. Letâs keep making progress.</p></div></div>
  ...
  <div class="eg-premium-dashboard">
   <section class="eg-premium-learning-card">
    <div class="eg-premium-learning-copy">
     ...
     <h2>${next?escapeHtml(next.title):available.length?'Youâve finished the available practice.':'Your activities are on the way.'}</h2>
     <p class="eg-premium-context">${bookTitle}${next?' Â· Lesson '+next.number:''}</p>
     ${next?`<button class="eg-premium-next-skill" id="nextSkillBtn" type="button"><span class="eg-next-skill-icon" aria-hidden="true">â</span><span><small>Your next activity</small><strong>${escapeHtml(WORKBOOK_LABELS[step])}</strong></span><b aria-hidden="true">âº</b></button><button class="primary-btn eg-premium-continue" id="continueBtn">${completed?'Continue':'Start activity'} <span>â</span></button>`:`<div class="eg-premium-finished"><strong>${available.length?'Available practice complete':'Workbook coming soon'}</strong><p>${available.length?'Revisit any lesson in your book to keep practising.':'Your teacher will let you know when your workbook is ready.'}</p></div>`}
    </div>
   ...
   <aside class="eg-premium-side">
    <div class="eg-premium-quick-actions">
     <button class="eg-premium-action" id="viewClassBookBtn" type="button"><span class="eg-action-icon" aria-hidden="true">â¤</span><strong>Class book</strong><b aria-hidden="true">âº</b></button>
     <button class="eg-premium-action" id="viewCourseBtn" type="button"><span class="eg-action-icon" aria-hidden="true">â§</span><strong>Workbook</strong><b aria-hidden="true">âº</b></button><button class="eg-premium-action" id="viewWritingsBtn" type="button"><span class="eg-action-icon" aria-hidden="true">â</span><strong>My Writings</strong><b aria-hidden="true">âº</b></button>
    </div>
    <section class="eg-premium-progress-card">
     <div class="eg-progress-card-head"><div><span class="eg-premium-kicker">Your book progress</span><strong>Keep building momentum</strong></div><span class="eg-progress-mini-icon" aria-hidden="true">â¥</span></div>
     ...
     <button class="eg-progress-cta" id="openProgressBtn" type="button">View progress <span>â</span></button>
    </section>
   </aside>
  </div>

  <section class="eg-premium-recent">
   <div class="eg-premium-section-head"><div><span class="eg-premium-kicker">Your learning history</span><h2>Recent practice</h2></div><button class="eg-premium-section-link" id="viewAllPracticeBtn" type="button">View progress <span>â</span></button></div>
...
function studentCourse(){
 ...
 $('content').innerHTML=`<section class="course-shell"><button class="back-link" id="courseHome">â Home</button><div class="course-intro"><div><span class="pill teal">${escapeHtml(b.level||'')}</span><h1>${escapeHtml(b.title)}</h1><p>Revise the class book first, then complete the workbook activities.</p></div><div class="course-overall"><strong>${completionPct(sid)}%</strong><span>Book progress</span>${progress(completionPct(sid))}</div></div><div class="course-path student-course-list">${COURSE.lessons.map(l=>{
   ...
   return `<div class="course-topic ${complete?'complete':''} ${ready?'current':'pending'} ${upgrade?'has-upgrade':''}"><span class="topic-state">${complete?'â':ready?'â':'â'}</span><span class="topic-copy"><small>Lesson ${l.number}</small><strong>${escapeHtml(l.title)}</strong><span>${status}</span></span><span class="topic-actions">${ready?`<button class="ghost-btn" data-revise-lesson="${l.number}">Revise first</button><button class="primary-btn" data-topic="${l.id}" ${upgrade?'data-retry-upgraded="true"':''}>${workbookLabel}</button>`:'â'}</span></div>`
  }).join('')}</div></section>`;
...
function workbook(){
 ...
 const stageButtons=steps.map((step,i)=>{
  ...
  return `<button class="eg-stage ${step===currentStep?'is-current':''} ${done?'is-done':''}" data-workbook-stage="${allowed?step:''}" ${allowed?'':'disabled'} ${step===currentStep?'aria-current="step"':''}><span>${done?'â':i+1}</span><strong>${escapeHtml(WORKBOOK_LABELS[step].toLowerCase())}</strong></button>`;
 }).join('');
...
{type:'open',stage:'Personalise',q:`Write one true sentence about your own life or work using â${e[1].text}â`,min:6,tag:'vocabulary:personalisation'},
...
function shortAnswerEvidence(name,answer,tag){return `<div class="open-evidence short-answer-evidence"><input type="text" name="${name}" data-open="1" data-short-answer="1" data-exact="${escapeAttr(answer||'')}" data-min="1" data-tag="${escapeAttr(tag||'')}" autocomplete="off" placeholder="Type a short answerâ¦"><small class="muted">Answer in a word, phrase, or short sentence.</small></div>`}
 ...
 return `<div class="eg-skill-page eg-vocabulary-page">
  ...
  <div class="eg-skill-layout">
   <aside class="eg-editorial-panel eg-vocab-reference">
    ...
    <div class="eg-vocab-reference-grid">${refs.length?refs.map((x,i)=>`<article class="eg-vocab-reference-card"><span>${i+1}</span><strong>${escapeHtml(x.word)}</strong><p>${escapeHtml(x.meaning)}</p>${x.example?`<small>â${escapeHtml(x.example)}â</small>`:''}</article>`).join(''):'<p class="muted">Use the activity questions to discover todayâs key language.</p>'}</div>
...
['infinitive','After language about a goal or something you want to do, look for âto + base verbâ.'],
...
['connector-because','Use âbecauseâ when the second idea gives a reason.'],
['connector-but','Use âbutâ when the second idea contrasts with the first.'],
['connector-so','Use âsoâ when the second idea is a result.'],
['connector','Decide whether the ideas show addition, contrast, reason or result.'],
['gerund','After âresponsible forâ, the next verb normally uses the -ing form.'],
...
function writingNormalize(text){
 let s=String(text||'').toLowerCase().replace(/[ââ]/g,'"').replace(/[ââ]/g,"'").trim();
 const contractions=[
...
function cleanA1WritingTask(task){
 const original=String(task||'').trim(),m=original.match(/^Write\s+\d+[â-]\d+\s+words\s+(.+)$/i);
...
const A1_EARLY_WRITING_MODELS={
 ...
 10:{
  ...
  organize:[
   ...
   ['What are those?','Those are the other studentsâ bags.']
  ]
 }
};
...
function writingCoreExercises(l){
 ...
 combines.forEach((x,i)=>out.push({type:'combine',label:`${i+4} Â· ${early?'Join with '+x.connector:'Sentence Combining'}`,instruction:early?`Write one simple sentence using â${x.connector}â.`:`Combine the two sentences using â${x.connector}â.`,...x}));
 corrections.forEach((x,i)=>out.push({type:'correct',label:`${i+7} Â· ${early?'Fix the Sentence':'Error Correction'}`,instruction:early?'Write the simple sentence correctly.':'Rewrite the sentence correctly.',...x}));
 paragraphs.forEach((answer,i)=>out.push({type:'organize',label:`${i+10} Â· ${early?'Sentence Order':'Paragraph Ordering'}`,instruction:early?'Tap the two sentences in the natural order.':'Tap the sentences in the most logical paragraph order.',answer,pieces:writingShuffle(answer,l.id+`|organize|${i}`)}));
 return out
}
...
function writingCoreHtml(l){
 ...
 return writingCoreExercises(l).map(ex=>{
  ...
  const heading=ex.type!==last?`<div class="writing-core-group-head"><span>${isWorkbookPreview()?`Level ${g.level} Â· ${escapeHtml(g.label)}`:escapeHtml(g.label)}</span><strong>3 questions</strong></div>`:'';
  last=ex.type;
...
function writingActivity(l){
 ...
 const earlyWriting=isA1EarlyWriting(l),teacherGuide=teacherView?`<div class="teacher-only-writing-guide"><span class="role-kicker">Teacher view</span><strong>Activity design</strong><p>${earlyWriting?'A1 Lessons 1â10 use very short, highly supported writing: build a sentence â join two ideas â fix a sentence â order two sentences.':'12 auto-graded preparation questions: Build â Combine â Correct â Organize, followed by one real-life writing task.'}</p></div>`:'';
 return `<div class="eg-skill-page eg-writing-page">
  ...
  <div class="eg-writing-layout eg-writing-layout-designed">
   <main class="eg-writing-workspace">
    <section class="writing-builder writing-core-sequence">${teacherView?`<div class="eg-task-panel-head"><div><small>Part 1 Â· Auto-graded practice</small><h2>${earlyWriting?'Build â Join â Fix â Order':'Build â Combine â Correct â Organize'}</h2><p>${earlyWriting?'Super-simple A1 writing for Lessons 1â10. No paragraph ordering yet.':'Three questions for each exercise type before the final task.'}</p></div></div>`:''}${writingCoreHtml(l)}</section>
    <article class="guided-question writing-guided-question final-writing-card eg-message-composer">
     ...
     <div class="writing-task-head"><div><span class="stage-badge">${teacherView?'Level 5 Â· Perform':'Final writing'}</span><strong>${escapeHtml(spec.task)}</strong></div><span>${spec.min}â${spec.max} words</span></div>
     <textarea class="writing-final-response" data-count-key="final" data-min="${spec.min}" data-max="${spec.max}" autocomplete="off" autocapitalize="sentences" spellcheck="true" placeholder="Type your response hereâ¦">${escapeHtml(saved.final)}</textarea><div class="word-count" data-count="final">0 words</div><div class="writing-integrity-status" id="writingIntegrityStatus" role="status" aria-live="polite"></div>
     <fieldset class="writing-share-choice">
      ...
      <div class="writing-share-options">
       <label><input type="radio" name="writing-community-share" value="yes"><span><strong>Yes â share it</strong><small>Publish this writing to My Writings.</small></span></label>
       <label><input type="radio" name="writing-community-share" value="no"><span><strong>No â keep it private</strong><small>Save it for learning, but do not show it in My Writings.</small></span></label>
      </div>
     </fieldset>
    </article>
   </main>
...
function syncAudioUi(){
 ...
 if(btn)btn.textContent=activeAudio.paused?'â¶':'ââ';
}
...
function playListening(l){
 ...
 if(!workbookAudioReady(l)){status.textContent='Audio is still preparingâ¦';return}
...
function restartListening(l){
 ...
 if(!workbookAudioReady(l)){if(status)status.textContent='Audio is still preparingâ¦';return}
...
function writingCoreFeedback(card,correct,complete){
 ...
 if(correct){box.innerHTML='<div class="writing-core-feedback is-correct"><strong>â Correct</strong></div>';return}
 if(type==='organize'){
  const order=answer.split('||');
  box.innerHTML='<div class="writing-core-feedback is-incorrect"><strong>â Not quite</strong><span>Correct order:</span><ol>'+order.map(x=>'<li>'+escapeHtml(x)+'</li>').join('')+'</ol></div>';return
 }
 const label=type==='combine'?'Best combined sentence:':'Correct sentence:';
 box.innerHTML='<div class="writing-core-feedback is-incorrect"><strong>â Not quite</strong><span>'+label+' <b>'+escapeHtml(withPeriod(answer))+'</b></span></div>'
}
...
function profile(){
 ...
 $('content').innerHTML=`<section class="profile-shell student-photo-profile">
  <div class="card profile-card student-profile-card">
   ...
   <div class="profile-photo-privacy"><span aria-hidden="true">â</span><p>Your profile picture is visible to signed-in EnglishGate students and teachers on the leaderboard.</p></div>
   ...
   <div class="profile-details">
    ...
    <div class="profile-row"><span>Class</span><strong>${escapeHtml(c?.name||'â')}</strong></div>
    <div class="profile-row"><span>Teacher</span><strong>${escapeHtml(teacher?.name||'â')}</strong></div>
 ...
 jobSave.onclick=async()=>{
  const value=jobInput.value.trim();jobStatus.classList.remove('error');jobSave.disabled=true;jobSave.textContent='Savingâ¦';
  try{
...
function teacherLiveToolsHtml(){
 return `<div class="live-class-tools" role="toolbar" aria-label="Live class teaching tools">
  <div class="live-tool-modes">
   ${teacherToolButton('interact','Interact','â')}
   ${teacherToolButton('pointer','Pointer','â')}
   ${teacherToolButton('highlight','Highlighter','â°')}
   ${teacherToolButton('text','Text','T')}
   ${teacherToolButton('example','Give example','â¦')}
   ${teacherToolButton('pronunciation','Pronunciation','ð')}
  </div>
  <div class="live-tool-actions">
   <button class="live-tool-btn" type="button" id="undoLiveAnnotation" title="Undo last annotation">â¶ <span>Undo</span></button>
   <button class="live-tool-btn" type="button" id="clearLiveAnnotations" title="Clear annotations">Ã <span>Clear</span></button>
  </div>
 </div>`;
}
function setTeacherPresentationMode(on){
 ...
 document.querySelectorAll('[data-presentation-toggle]').forEach(b=>{b.setAttribute('aria-pressed',teacherPresentationMode?'true':'false');if(b.dataset.presentationLabel==='dynamic')b.textContent=teacherPresentationMode?'Exit presentation':'Present â¶'});
...
function openTeacherSpotlight(rows,index){
 ...
 overlay.innerHTML=`<div class="teacher-spotlight-card" data-tone="${safeIndex%3}">
  <header><span>Question ${escapeHtml(number)}</span><button type="button" class="teacher-spotlight-close" id="closeTeacherSpotlight">Show all questions Ã</button></header>
  ...
  <footer><button type="button" class="ghost-btn" id="spotlightPrev" ${safeIndex===0?'disabled':''}>â Previous</button><span>${safeIndex+1} of ${rows.length}</span><button type="button" class="primary-btn" id="spotlightNext" ${safeIndex===rows.length-1?'disabled':''}>Next â</button></footer>
 </div>`;
...
function openTeacherReadingSpotlight(reading){
 ...
 card.innerHTML=`<header><span>Reading spotlight</span><div class="teacher-reading-controls"><button type="button" class="teacher-reading-size" id="readingSmaller" title="Smaller text">Aâ</button><button type="button" class="teacher-reading-size" id="readingLarger" title="Larger text">A+</button><button type="button" class="teacher-spotlight-close" id="closeTeacherSpotlight">Show lesson Ã</button></div></header><div class="teacher-reading-spotlight-body"><h2>${escapeHtml(reading.title||'Reading')}</h2><div class="teacher-reading-copy" id="teacherReadingCopy"></div></div><footer><span>Reading Â· Classroom view</span><button type="button" class="primary-btn" id="closeReadingSpotlight">Back to lesson</button></footer>`;
...
function openTeacherFindSpotlight(cards,index){
 ...
 overlay.innerHTML=`<div class="teacher-spotlight-card teacher-find-spotlight-card" data-tone="${safeIndex%3}">
  <header><span>Find someone whoâ¦</span><button type="button" class="teacher-spotlight-close" id="closeTeacherSpotlight">Show all cards Ã</button></header>
  <div class="teacher-find-spotlight-body"><div class="teacher-find-icon">â</div><h2>${escapeHtml(statement)}</h2><div class="teacher-find-language teacher-find-language-single"><div><b>Hint</b><p>â${escapeHtml(starter)}â</p></div></div></div>
  <footer><button type="button" class="ghost-btn" id="spotlightPrev" ${safeIndex===0?'disabled':''}>â Previous card</button><span>${safeIndex+1} of ${cards.length}</span><button type="button" class="primary-btn" id="spotlightNext" ${safeIndex===cards.length-1?'disabled':''}>Next card â</button></footer>
 </div>`;
 document.body.appendChild(overlay);
 $('closeTeacherSpotlight').onclick=closeTeacherSpotlight;
...
function wireTeacherSpotlight(){
 ...
 if(reading){
  ...
  const btn=document.createElement('button');btn.type='button';btn.className='reading-spotlight-trigger';btn.innerHTML='<span aria-hidden="true">â¶</span> Spotlight reading';btn.setAttribute('aria-label','Open reading in spotlight view');
...
function teacherLocalExample(word,lesson){
 ...
 const common={
  ...
  at:'The class starts at eight oâclock.',
 ...
 return common[key]||withPeriod('We used the word â'+clean+'â during todayâs lesson')
}
...
async function showTeacherExampleForWord(target,word){
 ...
 bubble.innerHTML='<small>Example Â· '+escapeHtml(clean)+'</small><p>Creating exampleâ¦</p>';
 ...
 bubble.innerHTML='<small>Example Â· '+escapeHtml(clean)+'</small><p>'+escapeHtml(sentence)+'</p>';
...
function teacherPronunciationBubble(target,word,status='Playing'){
 ...
 bubble.innerHTML='<span aria-hidden="true">ð</span><div><small>American pronunciation</small><strong>'+escapeHtml(word)+'</strong><em>'+escapeHtml(status)+'</em></div>';
...
async function playTeacherPronunciation(target,word){
 const clean=String(word||'').trim();if(!clean)return;
 if(activeTeacherPronunciationAudio){try{activeTeacherPronunciationAudio.pause();activeTeacherPronunciationAudio.currentTime=0}catch{}}
 const key=normalizeVocabWord(clean),loading=teacherPronunciationBubble(target,clean,'Preparingâ¦');
 try{
...
function wireTeacherLiveTools(){
 ...
 const addTextEditor=p=>{
  ...
  editor.type='text';editor.className='live-text-editor';editor.placeholder='Type textâ¦';editor.maxLength=140;
...
function teacherLiveLesson(){
 ...
 $('content').innerHTML=`<section class="eg-lesson">
  ...
  <footer class="eg-lesson-footer"><button class="ghost-btn" id="prevLiveSection" ${activeTeacherSectionIndex===0?'disabled':''}>â Previous stage</button><span class="teacher-footer-stage">Stage ${activeTeacherSectionIndex+1} of ${Math.max(1,total)}</span>${nextAction}</footer></div></div>
 </section>`;
...
function studentRow(s){const w=weakest(s.id),className=getDB().classes.find(c=>s.classIds?.includes(c.id))?.name||'â';return `<tr><td><div class="student-cell">${studentAvatarMarkup(studentPhoto(s.id),s.name,'roster-avatar')}<div><button class="student-name-link" data-student-report="${escapeAttr(s.id)}" type="button">${escapeHtml(s.name)}</button><div class="muted">@${escapeHtml(s.username)}</div><small>WhatsApp: ${escapeHtml(s.whatsappNumber||'Not recorded')}</small></div></div></td><td>${escapeHtml(className)}</td><td>${completionPct(s.id)}%</td><td>${w?.label||'No scored activity yet'}</td><td><div class="management-row-meta"><button class="primary-btn compact-action" data-manage-student="${escapeAttr(s.id)}">Manage</button><button class="ghost-btn" data-review-student="${escapeAttr(s.id)}">View report</button></div></td></tr>`}
...
function openManageStudent(id){
 ...
 <form id="manageStudentForm" class="form-grid">
  ...
  <label>Class<select id="manageStudentClass">${classes.map(c=>`<option value="${escapeAttr(c.id)}" ${c.id===current?.id?'selected':''}>${escapeHtml(c.name)} Â· ${escapeHtml(bookMeta(c.bookId||c.course_id)?.title||c.level)}</option>`).join('')}</select><small>Moving class keeps all existing scores, writing, attempts, and completion records.</small></label>
 ...
 $('manageStudentForm').onsubmit=async e=>{
  e.preventDefault();const btn=e.submitter||$('manageStudentForm').querySelector('button[type="submit"]'),result=$('manageStudentResult');btn.disabled=true;btn.textContent='Savingâ¦';
  try{
...
function openStudentPasswordHelp(id){
 ...
 const save=async password=>{
  ...
  try{
   ...
   $('copyStudentPassword').onclick=async()=>{try{await navigator.clipboard.writeText(pw);$('copyStudentPassword').textContent='Copied â'}catch{$('copyStudentPassword').textContent='Copy failed'}};
...
async function leaderboard(teacher=false){
 ...
 try{
  ...
  content.innerHTML=`<section class="professional-leaderboard">
   ...
   ${top.length?`<div class="leaderboard-top-three">${top.map(st=>`<article class="leaderboard-top-card ${st.id===session.id?'is-me':''}"><span class="leaderboard-rank-label">#${st.rank}</span><div class="leaderboard-avatar">${avatarInner(st.photoUrl,st.name)}</div><div><strong>${escapeHtml(st.name)}${st.id===session.id?' Â· You':''}</strong><small>${escapeHtml(st.jobTitle||st.className||st.level||'EnglishGate learner')}</small></div><div class="leaderboard-top-metrics"><span><b>${st.score}%</b> score</span><span><b>${st.completion}%</b> complete</span><span><b>${Number.isFinite(st.average)?st.average+'%':'â'}</b> average</span></div></article>`).join('')}</div>`:''}
...
function credentialResult(label,r){const title=label==='Password reset'?'New password ready':`${label} created`,extra=r.whatsappMessage?`<div class="whatsapp-delivery"><strong>WhatsApp message ready</strong><small>${r.className?`Class: ${escapeHtml(r.className)} Â· `:''}${r.courseName?`Course: ${escapeHtml(r.courseName)}`:''}</small><textarea class="whatsapp-message" readonly>${escapeHtml(r.whatsappMessage)}</textarea><div class="credential-actions">${r.whatsappLink?`<button class="primary-btn" type="button" data-open-whatsapp-link="${escapeAttr(r.whatsappLink)}">Open WhatsApp</button>`:''}<button class="ghost-btn" type="button" data-copy-whatsapp>Copy message</button></div></div>`:'';return `<div class="credential-card"><span>${title}</span><strong>@${escapeHtml(r.username)}</strong><code>${escapeHtml(r.temporaryPassword)}</code><small>${r.passwordWasGenerated?'A secure password was generated. ':''}${label==='Password reset'?'Use this password to sign in, then change it later if needed.':'Copy this password now. It will not be shown again.'}</small>${extra}</div>`}
...
async function adminResetPassword(id){try{const r=await api(`/api/admin/users/${id}/reset-password`,{method:'POST'});showModal(`<div class="section-head"><div><span class="role-kicker">Password reset</span><h3>New temporary password</h3></div><button class="icon-btn" data-close>Ã</button></div>${credentialResult('Password reset',r)}`);document.querySelector('[data-close]').onclick=closeModal;bindCredentialActions();openWhatsAppLink(r.whatsappLink)}catch(e){alert(e.message)}}
