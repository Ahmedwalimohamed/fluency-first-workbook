...
function openStudentReportPage(id){activeStudentReportId=id;currentPage='student-performance-report';rememberCurrentPage();renderNav();renderStudentReportPage()}
...
function openStudentLesson360(id,lid){
 const st=getDB().users.find(u=>u.id===id),l=lessonById(lid);if(!st||!l)return;
 ...
 showModal(`<div class="report-360-shell"><header class="report-360-head"><button class="ghost-btn" type="button" id="reportBackToStudent">← Student overview</button><button class="icon-btn" data-close>×</button><div><p class="eyebrow">${escapeHtml(st.name)} · Lesson ${l.number}</p><h2>${escapeHtml(l.title)}</h2><p>360° lesson performance from the learner's saved workbook evidence.</p></div></header><div class="report-kpi-strip"><div><small>Completion</small><strong>${Math.round(done/WORKBOOK_STEPS.length*100)}%</strong><span>${done}/${WORKBOOK_STEPS.length} activities</span></div><div><small>Lesson score</small><strong>${Number.isFinite(avg)?avg+'%':'—'}</strong><span>${scores.length} scored activities</span></div><div><small>Total attempts</small><strong>${all.length}</strong><span>Across this lesson</span></div><div><small>Needs attention</small><strong>${weak?escapeHtml(skillLabel(weak.skill)):'—'}</strong><span>${weak?weak.e.score+'% latest score':'No scored weakness yet'}</span></div></div><section class="report-360-section"><div class="section-head"><div><h3>Activities</h3><p>Open an activity to inspect its saved attempts and learner evidence.</p></div></div><div class="report-activity-grid">${cells.map(x=>{const e=x.e,state=e.completed?'Completed':e.latest?'Attempted':'Not started';return `<button type="button" class="report-activity-card ${e.completed?'is-complete':e.latest?'is-attempted':'is-empty'}" data-report-activity="${escapeAttr(x.skill)}" data-report-lesson="${escapeAttr(lid)}" data-report-student="${escapeAttr(id)}"><span>${escapeHtml(skillLabel(x.skill))}</span><strong>${Number.isFinite(e.score)?e.score+'%':state}</strong><small>${e.attemptCount?e.attemptCount+' attempt'+(e.attemptCount===1?'':'s'):state} · View evidence</small></button>`}).join('')}</div></section><section class="report-360-section"><div class="section-head"><div><h3>Learning journey</h3><p>Saved attempts in chronological order show retries and improvement.</p></div></div>${all.length?`<div class="report-timeline">${all.slice().reverse().map(a=>`<div class="report-timeline-item"><span></span><div><strong>${escapeHtml(skillLabel(a.skill))} · ${Math.round(Number(a.score))}%</strong><small>${new Date(a.at).toLocaleString()}</small></div></div>`).join('')}</div>`:'<div class="empty-state"><h4>No lesson evidence yet</h4><p>The learner has not submitted work in this lesson.</p></div>'}</section></div>`);
 document.querySelector('#modalRoot [data-close]')?.addEventListener('click',closeModal);$('reportBackToStudent')?.addEventListener('click',()=>openStudentDetail(id));wireWorkbookReportInteractions()
}
