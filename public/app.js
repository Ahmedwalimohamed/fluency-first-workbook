...
function studentReportLessonCards(id){
 ...
 return `<div class="report-lesson-grid">${lessons.map(l=>{const done=skillCompletionFor(id,l.id).length,started=done||WORKBOOK_STEPS.some(k=>attempts(id,k,l.id).length),scores=WORKBOOK_STEPS.map(k=>latestAttemptFor(id,l.id,k)).filter(Boolean).map(a=>Number(a.score)),avg=scores.length?Math.round(scores.reduce((a,b)=>a+b,0)/scores.length):null;return `<button type="button" class="report-lesson-card ${started?'is-started':'is-empty'}" data-report-lesson-card="${escapeAttr(l.id)}" data-report-student="${escapeAttr(id)}"><span>Lesson ${l.number}</span><strong>${escapeHtml(l.title)}</strong><small>${started?`${done}/${WORKBOOK_STEPS.length} complete${Number.isFinite(avg)?' · '+avg+'% avg':''}`:'Not started'}</small></button>`}).join('')}</div>`
}
...
function openStudentDetail(id){
 ...
 showModal(`<div class="student-performance-shell"><header class="student-performance-hero"><div><p class="eyebrow">Student performance · ${escapeHtml(b?.title||'Workbook')}</p><h2>${escapeHtml(st.name)}</h2><p>${escapeHtml(c?.name||'No class')} · ${escapeHtml(st.whatsappNumber||'WhatsApp not recorded')}</p></div><button class="icon-btn" data-close>×</button></header><div class="report-kpi-strip"><div><small>Workbook progress</small><strong>${snap.pct}%</strong><span>${snap.done} of ${snap.total} activities</span></div><div><small>Current performance</small><strong>${Number.isFinite(snap.o)?snap.o+'%':'—'}</strong><span>Latest lesson scores</span></div><div><small>Active lessons</small><strong>${snap.activeLessons.length}</strong><span>With saved evidence</span></div><div><small>Needs attention</small><strong>${snap.w?escapeHtml(snap.w.label):'—'}</strong><span>${snap.w?snap.w.score+'% recorded average':'No scored weakness yet'}</span></div></div><section class="report-360-section report-skill-summary"><div class="section-head"><div><h3>Skill snapshot</h3><p>Current evidence from completed workbook activities.</p></div></div><div class="report-skill-grid">${skills.map(k=>{const v=mastery(id,k);return `<div><span>${escapeHtml(skillLabel(k))}</span><strong>${Number.isFinite(v)?v+'%':'—'}</strong><small>${Number.isFinite(v)?'Recorded performance':'No scored evidence'}</small></div>`}).join('')}</div></section><section class="report-360-section"><div class="section-head"><div><h3>Lessons</h3><p>Select a lesson for its complete 360° performance view.</p></div></div>${studentReportLessonCards(id)}</section><section class="report-360-section"><div class="section-head"><div><h3>Recent evidence</h3><p>The learner's latest saved workbook activity.</p></div><button class="ghost-btn" id="resetPass">Password help</button></div>${snap.latest?`<button class="report-latest-card" data-report-activity="${escapeAttr(snap.latest.skill)}" data-report-lesson="${escapeAttr(snap.latest.lessonId)}" data-report-student="${escapeAttr(id)}"><span>Lesson ${lessonById(snap.latest.lessonId)?.number||'—'} · ${escapeHtml(skillLabel(snap.latest.skill))}</span><strong>${Math.round(Number(snap.latest.score))}%</strong><small>${new Date(snap.latest.at).toLocaleString()} · View evidence</small></button>`:'<div class="empty-state"><h4>No workbook evidence yet</h4><p>Activity evidence will appear after the learner starts the workbook.</p></div>'}</section>${certificateStatusHtml(id)}${finalWritingAssessmentHtml(id)}</div>`);
