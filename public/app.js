...
function studentReportSnapshot(id){
 const c=courseForStudent(id),lessons=readyLessons(c),done=completedActivityCount(id),total=lessons.length*WORKBOOK_STEPS.length,pct=completionPct(id),o=overall(id),all=attempts(id).slice().sort((a,b)=>new Date(b.at)-new Date(a.at)),latest=all[0]||null,w=weakest(id);
 const activeLessons=lessons.filter(l=>skillCompletionFor(id,l.id).length||WORKBOOK_STEPS.some(k=>attempts(id,k,l.id).length));
...
function studentReportLessonCards(id){
 ...
 return `<div class="student-lesson-report-list">${lessons.map(l=>{const done=skillCompletionFor(id,l.id).length,cells=WORKBOOK_STEPS.map(k=>workbookEvidenceFor(id,l.id,k)),scores=cells.filter(e=>Number.isFinite(e.score)).map(e=>e.score),avg=scores.length?Math.round(scores.reduce((a,b)=>a+b,0)/scores.length):null,attemptCount=cells.reduce((n,e)=>n+e.attemptCount,0),started=done>0||attemptCount>0,signal=lessonPerformanceSignal(id,l.id);return `<button type="button" class="student-lesson-report-row ${started?'is-started':'is-empty'}" data-report-lesson-card="${escapeAttr(l.id)}" data-report-student="${escapeAttr(id)}"><span class="student-lesson-number">Lesson ${l.number}</span><span class="student-lesson-name">${escapeHtml(l.title)}</span><span class="student-lesson-points"><strong>${Number.isFinite(avg)?avg+'%':'—'}</strong><small>${started?`${done}/${WORKBOOK_STEPS.length} activities · ${attemptCount} attempt${attemptCount===1?'':'s'} · ${signal.label}`:'Not started'}</small></span><span class="student-lesson-open">View lesson →</span></button>`}).join('')}</div>`
}
