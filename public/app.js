...
function reports(){
 const sts=reportStudents(),ids=new Set(sts.map(s=>s.id)),ats=getDB().attempts.filter(a=>ids.has(a.studentId)),avgComp=sts.length?Math.round(sts.reduce((a,st)=>a+completionPct(st.id),0)/sts.length):0,scoredStudents=sts.map(st=>overall(st.id)).filter(Number.isFinite),avgScore=scoredStudents.length?Math.round(scoredStudents.reduce((a,b)=>a+b,0)/scoredStudents.length):null;
 title(session?.role==='admin'?'System Admin':'Workbook evidence','Student performance');
 $('content').innerHTML=`<main class="reports-overview-page"><div class="grid grid-3 reports-metrics">${metric('Workbook completion',avgComp+'%','Exact completion records')}${metric('Recorded score average',Number.isFinite(avgScore)?avgScore+'%':'—',scoredStudents.length+' students with scores')}${metric('Attempts saved',ats.length,'Exact practice records')}</div><section class="section card reports-student-section"><div class="section-head"><div><h3>Students</h3><p>Select a student to open the full performance report.</p></div></div><div class="reports-student-list">${sts.map(st=>{const latest=latestActivityFor(st.id),o=overall(st.id);return `<article class="reports-student-row"><div class="reports-student-identity"><strong>${escapeHtml(st.name)}</strong><small>${escapeHtml(studentClassName(st))} · ${escapeHtml(bookMeta(bookIdForStudent(st.id))?.title||'—')}</small></div><div class="reports-student-stat"><small>Latest</small><strong>${latest?`L${lessonById(latest.lessonId)?.number||'—'} · ${escapeHtml(skillLabel(latest.skill))}`:'—'}</strong></div><div class="reports-student-stat"><small>Completion</small><strong>${completionPct(st.id)}%</strong></div><div class="reports-student-stat"><small>Overall</small><strong>${Number.isFinite(o)?o+'%':'—'}</strong></div><button class="primary-btn reports-review-btn" data-review-student="${escapeAttr(st.id)}">View report</button></article>`}).join('')||'<div class="empty-state"><h3>No students found</h3><p>Student performance will appear here when learners are available.</p></div>'}</div></section></main>`;
 bindStudentActions()
}
