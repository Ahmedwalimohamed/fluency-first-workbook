...
function studentHome(){
 ...
 <header class="eg-student-welcome"><div><p class="eg-label">${escapeHtml(book.level||'Your workbook')}</p><h1>Hi, ${escapeHtml(session.name?.trim().split(/\s+/)[0]||'there')}.</h1></div><div class="student-cta-row"><button class="ghost-btn" id="viewClassBookBtn">Class book</button><button class="ghost-btn" id="viewCourseBtn">Workbook →</button></div></header>
 <div class="eg-home-layout"><section class="eg-next-practice"><p class="eg-label">${next?(completed?'Continue learning':'Start here'):available.length?'Available practice complete':'Your book'}</p><h2>${next?escapeHtml(next.title):available.length?'You’ve finished the available practice.':'Your activities are on the way.'}</h2><p class="eg-next-context">${escapeHtml(book.title||'Workbook')}${next?' · Lesson '+next.number:''}</p>
 ${next?`<div class="eg-next-skill"><span>${WORKBOOK_STEPS.indexOf(step)+1}</span><div><small>Your next activity</small><strong>${WORKBOOK_LABELS[step]}</strong></div></div><button class="primary-btn" id="continueBtn">${completed?'Continue':'Start activity'} →</button>`:`<p>${available.length?'Revisit any lesson in your book to keep practising.':'Your teacher will let you know when your workbook is ready.'}</p>`}
 </section><aside class="eg-home-progress"><p class="eg-label">Your book progress</p><strong>${total?Math.round(completed/total*100):0}<span>%</span></strong>${progress(total?completed/total*100:0)}<p>${completed} of ${total} available activities complete</p><button class="text-link" id="openProgressBtn">View progress →</button></aside></div>
 <section class="eg-recent-practice"><h2>Recent practice</h2>${recent.length?recent.map(a=>`<div class="eg-recent-row"><div><strong>${escapeHtml(lessonById(a.lessonId)?.title||'Lesson')}</strong><span>${escapeHtml(skillLabel(a.skill))} · ${new Date(a.at).toLocaleDateString()}</span></div><b>${a.score!==null&&Number.isFinite(Number(a.score))?Number(a.score)+'%':'Saved'}</b></div>`).join(''):'<p>Your completed practice will appear here.</p>'}</section></section>`;
