...
function grammarQuestions(focus){
  ...
  if(f.includes("reported")) return [
    ...
    qg("Choose the time change.",["today → that day","today → next year","yesterday → tomorrow always"],"today → that day"),
  ...
  if(f.includes("present perfect")) return [
    ...
    qg("We haven’t finished ___.",["yet","yesterday","last week"],"yet"),
    ...
    qg("Choose the correct negative.",["I haven’t tried that app.","I didn’t tried that app ever.","I haven’t try that app."],"I haven’t tried that app."),
  ...
  if(f.includes("past")) return [
    ...
    qg("Choose the correct negative.",["We didn’t travel last week.","We didn’t travelled last week.","We not travelled last week."],"We didn’t travel last week."),
  ...
  if(f.includes("future")||f.includes("going to")||f.includes("would like")||f.includes("want to")) return [
    ...
    qg("Choose a spontaneous decision.",["I’ll answer the phone.","I am answered the phone tomorrow.","I will to answer."],"I’ll answer the phone."),
    qg("Choose a fixed arrangement.",["I’m meeting the director at 10 tomorrow.","I meet the director yesterday.","I will meeting the director at 10."],"I’m meeting the director at 10 tomorrow."),
    qg("Look at those clouds. It ___.",["is going to rain","will raining","rains yesterday"],"is going to rain"),
    qg("Choose the polite preference.",["I’d like to apply.","I like apply.","I would like applying."],"I’d like to apply."),
  ...
  if(f.includes("modal")||f.includes("should")||f.includes("can / could")||f.includes("ability")||f.includes("obligation")) return [
    ...
    qg("Choose no obligation.",["You don’t have to attend.","You mustn’t attend.","You shouldn’t to attend."],"You don’t have to attend."),
    qg("Choose prohibition.",["You mustn’t smoke here.","You don’t have to smoke here.","You could not smoke here maybe."],"You mustn’t smoke here."),
    ...
    qg("The plan ___ work, but I’m not sure.",["could","must definitely","can to"],"could")
  ];
  if(f.includes("compar")||f.includes("superl")||f.includes("quantifier")||f.includes("as...as")) return [
    ...
    qg("We don’t have ___ time.",["much","many","a few"],"much"),
  ...
  if(f.includes("gerund")||f.includes("infinitive")) return [
    ...
    qg("Choose the accurate sentence.",["I’m interested in learning photography.","I’m interested to learning photography.","I’m interested learn photography."],"I’m interested in learning photography."),
  ...
  if(f.includes("question")) return [
    ...
    qg("You work here, ___?",["don’t you","do you","aren’t you"],"don’t you"),
    qg("She isn’t coming, ___?",["is she","isn’t she","does she"],"is she"),
    qg("They arrived early, ___?",["didn’t they","did they","weren’t they"],"didn’t they"),
    qg("Do you know ___?",["what time it starts","what time does it start","what time starts it"],"what time it starts"),
    qg("He can drive, ___?",["can’t he","can he","doesn’t he"],"can’t he"),
    ...
    qg("You haven’t seen it, ___?",["have you","haven’t you","did you"],"have you"),
  ...
  if(f.includes("present simple")||f.includes("frequency")) return [
    ...
    qg("Choose the correct negative.",["He doesn’t drive to work.","He doesn’t drives to work.","He not drive to work."],"He doesn’t drive to work."),
...
SPEAKUP_A2_B1_BOOK.lessons=[SPEAKUP_A2_B1_BOOK.lessons[0],...SPEAKUP_A2_B1_SYLLABUS.slice(1).map((x,i)=>makeLesson('su-a2b1',i+2,x[0],x[1],'A2 → B1',TOPIC_LIBRARY[x[0]]))];
SPEAKUP_A2_B1_BOOK.totalLessons=22;
function makeBlueprintB2Lesson(x){return{id:x.id,number:x.number,title:x.title,outcome:x.outcome,ready:true,targetVocabulary:x.vocabulary,foundation:x.foundation,b2Lift:x.b2Lift,performance:x.performance,pronunciation:x.pronunciation,mediation:x.mediation,functions:x.functions||[],discourse:x.discourse||[],chunks:x.chunks||[],interactionExpressions:x.interactionExpressions||[],scenario:x.scenario||null,interact:x.interact||null,improve:x.improve||null,evidence:x.evidence||null,adaptive:x.adaptive||null,northstar:x.northstar||null,expressions:[...(x.chunks||[]),...(x.interactionExpressions||[])].slice(0,6).map(text=>({text})),vocabulary:{items:x.vocabularyItems},listening:{title:x.title+' · Reading & Listening',readingText:x.readingText,audioScript:x.audioScript,text:x.audioScript,questions:x.questions},grammar:{focus:x.grammarFocus,rule:x.grammarRule,items:(x.grammarItems||grammarQuestions(x.grammarFocus))},writing:x.writing}}
const B2_BLUEPRINT=Array.isArray(window.SPEAKUP_B2_BLUEPRINT)?window.SPEAKUP_B2_BLUEPRINT:[];
const IOU_PUBLIC_NAME='Islamic Online University-Borama';
const IOU_PUBLIC_CONTACT='063 325 3947';
const IOU_PUBLIC_SITE='learnenglish.iouborama.com';
const B2_UPGRADE_VERSION='b2-living-standard-v1';
const B2_UPGRADE_RELEASE_AT='2026-09-08T21:47:25.822Z';
const SPEAKUP_B2_BOOK={id:'speakup-b2',title:'B2 Upper Intermediate',level:'B2',moduleTitle:'B2 Upper Intermediate',moduleGoal:'Independent workbook practice for vocabulary, listening & reading, grammar and writing that supports the live B2 fluency course.',totalLessons:22,lessons:B2_BLUEPRINT.length===22?B2_BLUEPRINT.map(makeBlueprintB2Lesson):SPEAKUP_B2_SYLLABUS.map((x,i)=>makeLesson('su-b2',i+1,x[0],x[1],'B2',TOPIC_LIBRARY[x[0]]))};
const CAREER_MUNA_BOOK=buildCareerBook('career-fluency-muna','Career Fluency — Muna Edition','A1+/A2 → functional B1',CAREER_REVISED_LESSON_SPECS);
const CAREER_ABDISALAN_BOOK=buildCareerBook('career-fluency-abdisalan','Career Fluency — Abdisalan Edition','A2+ → functional B1');
const CAREER_ABDISHAKUR_BOOK=buildCareerBook('career-fluency-abdishakur','Career Fluency — Abdishakur Edition','A2+ → functional B1',CAREER_REVISED_LESSON_SPECS);
...
function currentQuestionEvidence(){
 return [...document.querySelectorAll('#activityPanel .guided-question')].map((card,index)=>{
  const question=String(card.querySelector('p')?.textContent||card.querySelector('.question-stage')?.textContent||('Question '+(index+1))).trim();
  const open=card.querySelector('[data-open="1"]');
  if(open){
   const studentAnswer=open.value.trim(),exact=open.dataset.exact||'',min=Number(open.dataset.min||1),short=open.dataset.shortAnswer==='1';
   const correct=short?shortAnswerMatches(studentAnswer,exact):exact?writingNormalize(studentAnswer)===writingNormalize(exact):(studentAnswer?studentAnswer.split(/\s+/).length:0)>=min;
   return {index:index+1,question,studentAnswer,correctAnswer:exact||'Open response',correct,tag:open.dataset.tag||''}
  }
  const radios=[...card.querySelectorAll('input[type="radio"]')];
  if(radios.length){
   const selected=radios.find(x=>x.checked),answer=radios[0]?.dataset.answer||'';
   return {index:index+1,question,studentAnswer:selected?.value||'',correctAnswer:answer,correct:Boolean(selected&&selected.value===answer),tag:selected?.dataset.tag||radios[0]?.dataset.tag||''}
  }
  return null
 }).filter(Boolean)
}
...
const NAV={student:[['home','⌂','Home'],['course','▣','My book'],['my-writings','✎','My Writings'],['progress','◔','Progress'],['leaderboard','▥','Leaderboard']],teacher:[['teacher-home','⌂','Overview'],['teach','▣','Teach'],['teacher-workbooks','▤','Workbooks'],['students','◎','Students'],['writings','✎','Writings'],['classes','▤','Classes'],['leaderboard','▥','Leaderboard'],['weaknesses','△','Needs review'],['reports','▦','Reports']],admin:[['admin-home','⌂','Overview'],['admin-books','▣','Books'],['admin-test','◉','Test app'],['admin-classes','▤','Classes'],['admin-reports','▦','Reports'],['admin-writings','✎','Writings'],['admin-admins','◇','Admins'],['admin-teachers','◎','Teachers'],['admin-students','○','Students']]};
...
let studentPronunciationMode=false;
...
function renderStudent(){if(!studentPhoto(session.id)&&currentPage!=='profile'){currentPage='profile';renderNav()}return ({home:studentHome,workbook,course:studentCourse,'class-book':studentClassBook,'student-live-lesson':studentLiveLesson,'my-writings':myWritings,progress:studentProgress,leaderboard:()=>leaderboard(false),profile}[currentPage]||studentHome)()}
function renderTeacher(){return ({'teacher-home':teacherHome,teach:teacherTeach,'teacher-book':teacherBook,'teacher-live-lesson':teacherLiveLesson,'teacher-workbooks':teacherWorkbooks,'teacher-book-browse':teacherBookBrowse,'teacher-workbook-view':teacherWorkbookView,students,writings:myWritings,classes,leaderboard:()=>leaderboard(true),weaknesses,reports,'student-performance-report':renderStudentReportPage,'student-lesson-performance':renderStudentLessonPage}[currentPage]||teacherHome)()}
function renderAdmin(){return ({'admin-home':adminHome,'admin-books':adminBooks,'admin-test':adminTestApp,'admin-live-book-browse':adminLiveBookBrowse,'admin-live-lesson':adminLiveLesson,'admin-book-browse':adminBookBrowse,'admin-workbook':adminBooks,'admin-workbook-view':adminWorkbookView,'admin-classes':adminClasses,'admin-reports':reports,'student-performance-report':renderStudentReportPage,'student-lesson-performance':renderStudentLessonPage,'admin-writings':myWritings,'admin-admins':adminAdmins,'admin-teachers':adminTeachers,'admin-students':adminStudents}[currentPage]||adminHome)()}
...
Object.assign(VOCAB_MEANINGS,{
 'screen time':'the amount of time you spend looking at or using a phone, tablet, computer, or television screen',
 subject:'an area of study taught at school or university',
 strict:'making people follow rules carefully and not allowing much freedom',
 exam:'a formal test of what you know or can do',
 classmate:'a person who is in the same class as you',
 memorise:'to learn something so that you can remember it exactly',
 skill:'an ability to do something well, usually because of practice or training',
 budget:'a plan for how much money you can spend and what you will spend it on',
 invest:'to put money, time, or effort into something in order to get a future benefit',
 profit:'the money a business keeps after paying its costs',
 loan:'money that you borrow and must pay back',
 ...
 rubbish:'waste or things that are thrown away',
 responsibility:'a duty or something that you are expected to take care of',
 reduce:'to make something smaller or less',
 sibling:'a brother or sister',
 relative:'a person who is connected to you by family',
 raise:'to care for a child while they grow up',
 'close (to someone)':'having a strong, friendly, or loving relationship with someone',
 ...
 source:'the person, place, document, or website that information comes from',
 reliable:'able to be trusted or depended on',
 report:'to give information about an event, situation, or subject',
 'breaking news':'important news about an event that has just happened or is still happening',
 'fake news':'false or misleading information presented as news',
 opponent:'a person or team that you compete against',
 skilful:'good at doing something because of ability or training',
 fitness:'the condition of being physically healthy and strong',
 train:'to practise or exercise in order to improve performance',
 compete:'to try to win against other people or teams',
 effort:'the physical or mental energy used to do something',
 crowded:'full of many people or things, with little space',
 peaceful:'quiet, calm, and free from disturbance or conflict',
 traffic:'the cars, buses, trucks, and other vehicles moving on roads',
 facility:'a place, building, or service provided for a particular purpose',
 landscape:'the visible features of an area of land, such as hills, fields, and buildings',
 community:'the people who live in the same area or share something in common',
 ambition:'a strong desire to achieve something important',
 achieve:'to succeed in reaching a goal after effort',
 goal:'something you want to achieve',
 inspire:'to make someone feel motivated to do or create something',
 pursue:'to continue trying to achieve a goal, dream, or activity',
 determined:'strongly decided to do something and unwilling to give up',
 witness:'a person who sees an event happen',
 evidence:'facts, information, or objects that help show whether something is true',
 theft:'the crime of taking something that belongs to another person',
 punishment:'a penalty given for doing something wrong or breaking a rule',
 'report (a crime)':'to tell the police or another authority that a crime has happened',
 ...
 breakthrough:'an important discovery or development that allows major progress',
 develop:'to create, improve, or cause something to grow over time',
 research:'careful study done to discover facts or new information',
 release:'to make something available to the public',
 performance:'a show, act, or piece of work done in front of an audience',
 talented:'having a natural ability to do something very well',
 audience:'the people who watch or listen to a performance, speech, or programme',
 'release (a film/song)':'to make a film, song, or other creative work available to the public',
 ticket:'a document or electronic record that allows you to enter a place or use a service',
 venue:'the place where an event or performance happens',
 produce:'to make, create, or grow something',
 affect:'to cause a change in someone or something',
 solution:'a way of solving a problem',
 awareness:'knowledge or understanding of a situation or issue',
 relax:'to rest and become less tense or worried',
 avoid:'to stay away from someone or something, or prevent something from happening',
 enjoy:'to get pleasure from something',
 'manage (to)':'to succeed in doing something, especially something difficult',
 'spare time':'time when you are not working or doing necessary activities',
 tradition:'a custom or belief passed from one generation to another',
 identity:'the qualities, beliefs, and characteristics that make a person or group who they are',
 proverb:'a short traditional saying that gives advice or expresses a general truth',
 custom:'a traditional way of behaving or doing something in a particular society or group',
 heritage:'the traditions, culture, history, and values passed down from earlier generations',
 generation:'a group of people born and living at about the same time',
 decision:'a choice that you make after thinking about different options',
 option:'one possible choice among several',
 instinct:'a natural feeling that tells you what to do without careful thought',
 consider:'to think carefully about something before deciding',
 regret:'to feel sorry about something you did or did not do',
 imaginary:'existing only in the imagination, not in real life',
 improve:'to become better or make something better',
 progress:'movement or development toward a better or more advanced state',
 fluent:'able to speak or write a language smoothly and easily',
 resign:'to officially leave a job or position',
 qualification:'a skill, certificate, degree, or experience that makes you suitable for a job or activity',
 workload:'the amount of work a person has to do',
 itinerary:'a plan showing the places and times for a journey or trip',
 breathtaking:'extremely beautiful, impressive, or surprising',
 'off the beaten track':'in a place that is not often visited by tourists or most people',
 'culture shock':'the feeling of confusion or discomfort when experiencing a very different culture',
 layover:'a period of waiting between parts of a journey, especially between flights',
 addictive:'making you want to keep doing or using something and making it difficult to stop',
 'online privacy':'the protection of your personal information and activities on the internet',
 multitask:'to do more than one task at the same time',
 burnout:'extreme physical or mental tiredness caused by long-term stress or overwork',
 'balanced diet':'a way of eating that includes the right amounts of different kinds of healthy food',
 'stress relief':'something that helps reduce stress or tension',
 chronic:'lasting for a long time or happening repeatedly, especially with an illness or problem',
 recover:'to become healthy, strong, or normal again after illness, injury, or difficulty',
 wellbeing:'the state of being healthy, comfortable, and happy',
 'staple food':'a basic food that people in a particular place eat regularly and in large amounts',
 ...
 'self-taught':'having learned a skill by yourself rather than from a teacher or formal course',
 scholarship:'money given to a student to help pay for education, usually based on ability or need',
 'hands-on':'involving practical experience and active participation rather than only theory',
 procrastinate:'to delay doing something that you should do',
 investment:'money, time, or effort put into something in order to gain a future benefit',
 risk:'the possibility that something bad, harmful, or unsuccessful may happen',
 entrepreneur:'a person who starts and runs a business, taking financial risks',
 'extended family':'family members beyond parents and children, such as grandparents, aunts, uncles, and cousins',
 bond:'a strong feeling of connection or relationship between people',
 supportive:'giving help, encouragement, or understanding',
 'in-laws':'the relatives of your husband or wife',
 'role model':'a person whose behaviour or achievements you admire and want to copy',
 bias:'an unfair preference for or against a person, group, or idea',
 broadcast:'to send a programme, event, or information by television, radio, or the internet',
 credible:'believable and able to be trusted',
 stamina:'the physical or mental strength to continue doing something for a long time',
 injury:'physical damage or harm to the body',
 coach:'a person who trains and guides a person or team in a sport or skill',
 'warm up':'to prepare your body for exercise by doing light activity first',
 'personal best':'the best result or performance you have ever achieved',
 commute:'the regular journey between your home and your place of work or study',
 congestion:'a situation where roads are very crowded with traffic and movement is slow',
 opportunity:'a chance to do something useful, valuable, or desirable',
 isolated:'far away from other people or places, or feeling separated from others',
 milestone:'an important stage or achievement in progress or development',
 'long-term':'continuing for or relating to a long period of time',
 aspire:'to strongly hope or aim to achieve something',
 fulfil:'to achieve or complete something that was hoped for, promised, or expected',
 justice:'fair treatment according to laws, rights, and what is morally right',
 innovation:'a new idea, method, or product, or the process of creating one',
 'artificial intelligence':'computer technology that can perform tasks that normally require human intelligence',
 experiment:'a scientific test carried out to discover or prove something',
 theory:'an idea or set of ideas intended to explain why something happens',
 discovery:'something that is found or learned for the first time',
 masterpiece:'an extremely good work of art, writing, music, or film',
 creative:'able to produce original ideas or make new and imaginative things',
 inequality:'an unfair difference in wealth, rights, opportunities, or treatment',
 poverty:'the condition of being very poor and not having enough money for basic needs',
 humanitarian:'concerned with helping people who are suffering or in need',
 global:'relating to the whole world',
 solidarity:'support and unity between people or groups who share a goal or concern',
 'take up':'to begin a new hobby, activity, or interest',
 'give up':'to stop doing or trying something',
 'hang out':'to spend time relaxing or socialising with other people',
 unwind:'to relax after a period of work or stress',
 'try out':'to test or do something new to see whether you like it',
 'get into':'to become interested or involved in an activity',
 belong:'to feel accepted as part of a group or place',
 preserve:'to protect something so that it continues to exist or stays in good condition',
 diaspora:'people from one country or community who live in many different parts of the world',
 'weigh up':'to consider the advantages and disadvantages before making a decision',
 commit:'to make a firm decision to do something or give your time and effort to it',
 hesitate:'to pause before doing or saying something because you are unsure',
 indecisive:'unable to make decisions easily',
 fluency:'the ability to speak or write smoothly, easily, and naturally',
 confidence:'belief in your own ability to do something successfully',
 consistency:'the quality of doing something in a regular and steady way'
});
function normalizeVocabWord(word){return String(word||'').replace(/[“”"]/g,'').replace(/\s+/g,' ').trim().toLowerCase()}
...
function vocabMeaning(word){const key=normalizeVocabWord(word),meaning=VOCAB_MEANINGS[key]||meaningFromWorkbookWord(key)||meaningFromTopicLibrary(key);if(!meaning)console.warn('Missing vocabulary definition:',word);return meaning||'Definition not available yet.'}
...
function firstPersonPhrase(text){
 return String(text||'')
...
function liveAudioPlayerHtml(script,speakers=[]){
 ...
 return '<section class="audio-player live-audio-player" data-live-audio-player data-audio-text="'+escapeAttr(text)+'" data-audio-speakers="'+escapeAttr(JSON.stringify(speakerData))+'">'+
  ...
  '<div class="audio-player-main">'+
   ...
   '<button class="audio-icon-btn" type="button" data-live-audio-restart aria-label="Restart listening audio">↺</button>'+
...
function wireLiveAudioPlayers(root=document){
 root.querySelectorAll('[data-live-audio-player]').forEach(player=>{
  ...
  const fail=message=>{
   player.dataset.audioState='error';player.dataset.audioError=message||'Natural listening voice is temporarily unavailable.';
  ...
  if(play)play.onclick=()=>{
   ...
   audio.playbackRate=Number(speed?.value)||1;
   if(audio.paused){const promise=audio.play();if(promise?.catch)promise.catch(()=>fail('Audio playback failed. Tap retry.'))}else audio.pause();
   sync(audio)
  };
...
function renderLiveContent(text){
 const lines=String(text||'').split('\n'),listening=renderLiveListeningContent(lines);if(listening!==null)return listening;const out=[];
...
function liveSectionHeading(line){
 ...
 const patterns=[
  /^(?:PAGE\s+\d+\s*[—-]\s*)?(WARM UP|VOCABULARY|READING|GRAMMAR|LISTENING|FLUENCY MISSION|CLASSROOM CHALLENGE|REFLECTION(?:\s*&\s*HOMEWORK)?|HOMEWORK)$/i,
...
function scoreChip(score,at=''){const n=Number(score),tone=n>=80?'good':n>=60?'warn':'bad';return `<span class="score-chip ${tone}" ${at?`title="${escapeAttr(new Date(at).toLocaleString())}"`:''}>${Number.isFinite(n)?Math.round(n):'—'}%</span>`}
function lessonSkillCell(sid,lid,skill){const a=latestAttemptFor(sid,lid,skill);if(a)return scoreChip(a.score,a.at);return completionFor(sid,lid).includes(skill)?'<span class="score-chip done">Done</span>':'<span class="score-chip empty">—</span>'}
...
function studentClassName(st){return getDB().classes.find(c=>st.classIds?.includes(c.id))?.name||'—'}
function workbookEvidenceFor(sid,lid,skill){
 const all=attempts(sid,skill,lid).filter(a=>Number.isFinite(Number(a.score))).slice().sort((a,b)=>new Date(b.at)-new Date(a.at));
 const completed=completionFor(sid,lid).includes(skill),latest=all[0]||null;
 return {completed,latest,attemptCount:all.length,lastAt:latest?.at||null,score:latest?Math.round(Number(latest.score)):null}
}
function workbookEvidenceCell(sid,lid,skill){
 const e=workbookEvidenceFor(sid,lid,skill);
 if(!e.completed&&!e.latest)return `<button type="button" class="report-work-cell report-work-button is-not-started" data-report-activity="${escapeAttr(skill)}" data-report-lesson="${escapeAttr(lid)}" data-report-student="${escapeAttr(sid)}"><strong>Not started</strong><small>No saved workbook work</small></button>`;
 const score=e.latest?scoreChip(e.score,e.lastAt):'<span class="score-chip done">Done</span>';
 const state=e.completed?'Completed':'Attempted';
 const attemptsText=e.attemptCount?e.attemptCount+' '+(e.attemptCount===1?'attempt':'attempts'):'Completion saved';
 return `<button type="button" class="report-work-cell report-work-button ${e.completed?'is-complete':'is-attempted'}" data-report-activity="${escapeAttr(skill)}" data-report-lesson="${escapeAttr(lid)}" data-report-student="${escapeAttr(sid)}"><span class="report-work-line">${score}<span class="report-work-state">${state}</span></span><small>${attemptsText} · View details</small></button>`
}
function attemptQuestionEvidenceHtml(a){
 const rows=Array.isArray(a?.evidence)?a.evidence:[];
 if(!rows.length)return '<div class="report-evidence-missing"><strong>Question-level evidence unavailable</strong><span>This attempt was recorded before detailed response capture was enabled, or this activity did not have question-level responses.</span></div>';
 return '<div class="report-question-evidence-list">'+rows.map((x,i)=>{const state=x.correct===true?'correct':x.correct===false?'incorrect':'open',label=x.correct===true?'Correct':x.correct===false?'Needs review':'Open response';return '<article class="report-question-evidence '+state+'"><div class="report-question-evidence-head"><span>Question '+escapeHtml(String(x.index||i+1))+'</span><strong>'+escapeHtml(label)+'</strong></div><p>'+escapeHtml(x.question||'Question')+'</p><div class="report-answer-grid"><div><small>Student response</small><b>'+escapeHtml(x.studentAnswer||'No response recorded')+'</b></div><div><small>Expected / reference</small><b>'+escapeHtml(x.correctAnswer||'Open response')+'</b></div></div>'+(x.tag?'<span class="report-evidence-tag">'+escapeHtml(String(x.tag).replace(/[-_:]+/g,' '))+'</span>':'')+'</article>'}).join('')+'</div>'
}
function attemptHistoryEvidenceHtml(rows){
 if(!rows.length)return '<div class="empty-state"><h4>No attempts yet</h4><p>The student has not submitted scored work for this activity.</p></div>';
 return '<div class="report-attempt-evidence-list">'+rows.map((a,i)=>'<details class="report-attempt-evidence" '+(i===0?'open':'')+'><summary><div><strong>Attempt '+(rows.length-i)+'</strong><span>'+new Date(a.at).toLocaleString()+'</span></div><strong>'+Math.round(Number(a.score))+'%</strong></summary><div class="report-attempt-evidence-body"><div class="report-attempt-tags">'+((a.tags||[]).filter(t=>!String(t).startsWith('ladder:')).map(t=>'<span class="pill">'+escapeHtml(String(t).replace(/[-_:]+/g,' '))+'</span>').join(''))+'</div>'+attemptQuestionEvidenceHtml(a)+'</div></details>').join('')+'</div>'
}
function openWorkbookActivityEvidence(sid,lid,skill){
 const st=getDB().users.find(u=>u.id===sid),l=lessonById(lid),e=workbookEvidenceFor(sid,lid,skill),rows=attempts(sid,skill,lid).slice().sort((a,b)=>new Date(b.at)-new Date(a.at)),writing=skill==='writing'?writingFor(sid,lid):'';
 showModal(`<div class="section-head"><div><p class="eyebrow">${escapeHtml(st?.name||'Student')} · Lesson ${l?.number||'—'}</p><h3>${escapeHtml(skillLabel(skill))}</h3><p class="muted">${escapeHtml(l?.title||'Workbook activity')}</p></div><button class="icon-btn" data-close>×</button></div><div class="grid grid-3 compact-grid">${metric('Status',e.completed?'Completed':e.latest?'Attempted':'Not started','Workbook record')}${metric('Latest score',e.latest?e.score+'%':'—',e.latest?'Saved '+new Date(e.lastAt).toLocaleDateString():'No score saved')}${metric('Attempts',String(e.attemptCount),'Saved attempts')}</div>${writing?`<section class="section card"><span class="pill teal">Student writing</span><h4>Submitted response</h4><p class="report-writing-evidence">${escapeHtml(writing)}</p></section>`:''}<section class="section report-question-history"><div class="section-head"><div><h4>Attempts and question evidence</h4><p>Open any attempt to inspect the exact questions, learner responses, reference answers, and recurring errors that were captured.</p></div></div>${attemptHistoryEvidenceHtml(rows)}</section>`);
 document.querySelector('#modalRoot [data-close]')?.addEventListener('click',closeModal)
}
function wireWorkbookReportInteractions(){
 document.querySelectorAll('[data-report-activity]').forEach(btn=>btn.onclick=()=>openWorkbookActivityEvidence(btn.dataset.reportStudent,btn.dataset.reportLesson,btn.dataset.reportActivity))
}
function studentPerformanceTable(sid){
 const lessons=readyLessons(courseForStudent(sid));
 return `<section class="section student-work-report"><div class="section-head"><div><h3>Workbook work</h3><p>This report follows the student's actual workbook: one row per lesson and the same activities the learner completes.</p></div></div><div class="table-wrap performance-detail-table"><table class="data-table"><thead><tr><th>Lesson</th>${WORKBOOK_STEPS.map(k=>`<th>${skillLabel(k)}</th>`).join('')}<th>Lesson progress</th></tr></thead><tbody>${lessons.map(l=>`<tr><td><strong>Lesson ${l.number}</strong><small>${escapeHtml(l.title)}</small></td>${WORKBOOK_STEPS.map(k=>`<td>${workbookEvidenceCell(sid,l.id,k)}</td>`).join('')}<td><strong>${lessonProgress(sid,l.id)}%</strong><small>${skillCompletionFor(sid,l.id).length} of ${WORKBOOK_STEPS.length} activities completed</small></td></tr>`).join('')||'<tr><td colspan="6">No ready lessons in this workbook yet.</td></tr>'}</tbody></table></div></section>`
}
...
function weakest(sid){const tips={vocabulary:'Review the words and expressions that are not yet automatic.',grammar:'Review the language pattern needed for this lesson topic.',reading:'Practise finding key information in short practical texts.',listening:'Practise finding the main idea and key details in short audio.',writing:'Build clearer sentences, then organise them into a complete written response.',speaking:'Practise short, clear responses and ask a relevant question back.'},course=courseForStudent(sid),skills=[...new Set(readyLessons(course).flatMap(l=>workbookStepsForLesson(l)))].filter(k=>k!=='review'),r=skills.map(k=>[k,mastery(sid,k)]).filter(x=>Number.isFinite(x[1])).sort((a,b)=>a[1]-b[1]);if(!r.length)return null;const [k,score]=r[0];return{key:k,label:k==='grammar'&&course?.standardVersion==='a1-gold-v1'?'Language':skillLabel(k),score,tip:tips[k]||'Review this activity and retry the target communication.'}}
...
async function posterAvatarBitmap(url){
 if(!url)return null;try{const res=await fetch(url,{credentials:'include'});if(!res.ok)return null;const blob=await res.blob();return await createImageBitmap(blob)}catch{return null}
}
async function createWritingPoster(w){
 const canvas=document.createElement('canvas');canvas.width=1080;canvas.height=1920;const ctx=canvas.getContext('2d');
 const g=ctx.createLinearGradient(0,0,0,1920);g.addColorStop(0,'#102465');g.addColorStop(.34,'#1B3CCA');g.addColorStop(1,'#F8FAFC');ctx.fillStyle=g;ctx.fillRect(0,0,1080,1920);
 ...
 ctx.fillStyle='#64748B';ctx.font='500 28px Arial, sans-serif';ctx.fillText(writingLessonLabel(w),280,448);
 ctx.fillStyle='#2563EB';ctx.font='700 28px Arial, sans-serif';ctx.fillText('EnglishGate learner',280,490);
 ctx.strokeStyle='#E2E8F0';ctx.lineWidth=2;ctx.beginPath();ctx.moveTo(105,550);ctx.lineTo(975,550);ctx.stroke();
 let fontSize=45,lines=[];do{ctx.font='500 '+fontSize+'px Georgia, serif';lines=wrapPosterText(ctx,w.content,850);if(lines.length>22)fontSize-=2;else break}while(fontSize>34);
 if(lines.length>22){lines=lines.slice(0,22);lines[21]=lines[21].replace(/[.,;:!?]?$/,'')+'…'}
 ctx.fillStyle='#172033';ctx.font='500 '+fontSize+'px Georgia, serif';let y=635;const lh=Math.round(fontSize*1.48);for(const line of lines){ctx.fillText(line,115,y);y+=lh}
 ctx.fillStyle='#0F172A';ctx.font='700 30px Arial, sans-serif';ctx.fillText('Learning English. Building confidence.',105,1560);
 ctx.fillStyle='#F8FAFC';ctx.fillRect(55,1705,970,150);ctx.fillStyle='#102465';ctx.font='700 31px Arial, sans-serif';ctx.fillText(IOU_PUBLIC_NAME,95,1765);ctx.font='500 28px Arial, sans-serif';ctx.fillText('Contact: '+IOU_PUBLIC_CONTACT+'  ·  '+IOU_PUBLIC_SITE,95,1813);
 return await new Promise((resolve,reject)=>canvas.toBlob(b=>b?resolve(b):reject(new Error('Could not create poster.')),'image/png',.95))
}
async function shareWritingPoster(index,btn){
 const w=writingFeed[index];if(!w?.canShare)return;const original=btn?.textContent;if(btn){btn.disabled=true;btn.textContent='Creating poster…'}
 try{
  const blob=await createWritingPoster(w),file=new File([blob],`EnglishGate-${String(w.studentName||'writing').replace(/[^a-z0-9]+/gi,'-')}.png`,{type:'image/png'});
  if(navigator.share&&navigator.canShare&&navigator.canShare({files:[file]})){await navigator.share({files:[file],title:'My English Writing',text:'My English writing at '+IOU_PUBLIC_NAME+'. Contact: '+IOU_PUBLIC_CONTACT})}
  else{
   const url=URL.createObjectURL(blob),link=document.createElement('a');link.href=url;link.download=file.name;document.body.appendChild(link);link.click();link.remove();setTimeout(()=>URL.revokeObjectURL(url),5000);
   showModal(`<div class="section-head"><div><span class="role-kicker">Poster ready</span><h3>Share it on WhatsApp Status</h3><p class="muted">The branded poster has been saved. Open WhatsApp → Updates → My status and choose the image.</p></div><button class="icon-btn" data-close>×</button></div><div class="feedback good"><strong>${IOU_PUBLIC_NAME}</strong><br>${IOU_PUBLIC_CONTACT} is already printed on the poster.</div>`);document.querySelector('[data-close]').onclick=closeModal
  }
 }catch(e){if(e?.name!=='AbortError'){showModal(`<div class="section-head"><div><span class="role-kicker">Share writing</span><h3>Could not create the poster</h3><p class="muted">${escapeHtml(e.message||'Try again.')}</p></div><button class="icon-btn" data-close>×</button></div>`);document.querySelector('[data-close]').onclick=closeModal}}
 finally{if(btn){btn.disabled=false;btn.textContent=original}}
}
function b2AttemptIsPreUpgrade(a){return String(a?.lessonId||'').startsWith('su-b2-l')&&new Date(a.at).getTime()<new Date(B2_UPGRADE_RELEASE_AT).getTime()}
function b2AttemptIsLivingStandard(a){return Array.isArray(a?.tags)&&(a.tags.includes('curriculum:'+B2_UPGRADE_VERSION))}
function b2HasPreUpgradeEvidence(sid){return attempts(sid).some(b2AttemptIsPreUpgrade)}
function b2LessonPreUpgradeAttempts(sid,lid){return attempts(sid,null,lid).filter(b2AttemptIsPreUpgrade)}
function b2LessonLivingAttempts(sid,lid){return attempts(sid,null,lid).filter(b2AttemptIsLivingStandard)}
function b2LatestSkillAverage(rows){
 const latest={};rows.slice().sort((a,b)=>new Date(a.at)-new Date(b.at)).forEach(a=>latest[a.skill]=a);
 const vals=Object.values(latest).map(a=>Number(a.score)).filter(Number.isFinite);
 return vals.length?Math.round(vals.reduce((sum,n)=>sum+n,0)/vals.length):null
}
function b2UpgradeAvailableForLesson(sid,l){
 if(activeBookId!=='speakup-b2'||lessonProgress(sid,l.id)!==100||!b2LessonPreUpgradeAttempts(sid,l.id).length)return false;
 const upgraded=new Set(b2LessonLivingAttempts(sid,l.id).map(a=>a.skill));
 return WORKBOOK_STEPS.some(skill=>!upgraded.has(skill))
}
function b2ScoreComparisonHtml(sid,l){
 const oldScore=b2LatestSkillAverage(b2LessonPreUpgradeAttempts(sid,l.id)),newRows=b2LessonLivingAttempts(sid,l.id),newScore=b2LatestSkillAverage(newRows);
 if(oldScore===null)return'';
 return `<span class="b2-score-compare"><b>Previous score: ${oldScore}%</b>${newScore===null?'<span>Try the upgraded lesson and beat your score.</span>':`<span>New score${new Set(newRows.map(a=>a.skill)).size<WORKBOOK_STEPS.length?' so far':''}: <strong>${newScore}%</strong></span>`}</span>`
}
async function acknowledgeB2UpgradeNotice(){
 try{
  await api('/api/student/b2-upgrade-notice',{method:'PUT',body:JSON.stringify({version:B2_UPGRADE_VERSION})});
  getDB().profiles??={};getDB().profiles[session.id]??={};getDB().profiles[session.id].b2UpgradeNoticeVersion=B2_UPGRADE_VERSION;getDB().profiles[session.id].b2UpgradeNoticeSeenAt=new Date().toISOString()
 }catch(e){console.warn('Could not save B2 upgrade notice acknowledgement',e)}
}
function maybeShowB2UpgradeNotice(){
 if(session?.role!=='student')return;
 const sid=session.id;
 if(bookIdForStudent(sid)!=='speakup-b2'||!b2HasPreUpgradeEvidence(sid))return;
 if(getDB().profiles?.[sid]?.b2UpgradeNoticeVersion===B2_UPGRADE_VERSION)return;
 setActiveBook('speakup-b2');
 showModal(`<section class="b2-upgrade-modal"><span class="b2-upgrade-kicker">B2 course upgrade</span><h2>Your B2 course has been upgraded</h2><p>We've improved your previous lessons with stronger practice, clearer feedback, and more challenging activities.</p><div class="b2-upgrade-saved"><strong>Your progress and previous scores are still saved.</strong><span>You can continue from where you stopped, or redo earlier lessons to improve your scores and strengthen your English.</span></div><div class="b2-upgrade-actions"><button class="primary-btn" id="b2UpgradeContinue" type="button">Continue where I stopped</button><button class="ghost-btn" id="b2UpgradeReview" type="button">Review earlier lessons</button></div></section>`);
 const backdrop=$('modalRoot')?.querySelector('.modal-backdrop');if(backdrop)backdrop.onclick=e=>{if(e.target===backdrop)e.preventDefault()};
 $('b2UpgradeContinue').onclick=()=>{closeModal();acknowledgeB2UpgradeNotice();const next=readyLessons(COURSE).find(l=>skillCompletionFor(sid,l.id).length<WORKBOOK_STEPS.length);if(next){activeLessonId=next.id;currentStep=firstOpenStep(sid,next.id);currentPage='workbook';renderNav();workbook()}else{currentPage='home';renderNav();studentHome()}};
...
function studentHome(){
 ...
 const book=bookMeta(activeBookId)||{title:COURSE.title||COURSE.moduleTitle,level:COURSE.level},available=readyLessons(COURSE),next=available.find(l=>skillCompletionFor(sid,l.id).length<workbookStepsForLesson(l).length),completed=completedActivityCount(sid),total=available.reduce((n,l)=>n+workbookStepsForLesson(l).length,0);
 ...
 const firstName=escapeHtml(session.name?.trim().split(/\s+/)[0]||'there'),bookTitle=escapeHtml(book.title||'Your English book'),bookLevel=escapeHtml(book.level||'Your level');
 const recentHtml=recent.length?recent.map(a=>{
  const score=a.score!==null&&Number.isFinite(Number(a.score))?Number(a.score)+'%':'Saved',skill=skillLabel(a.skill),lesson=lessonById(a.lessonId)?.title||'Lesson';
  return `<button class="eg-premium-recent-row" type="button" data-recent-lesson="${escapeAttr(a.lessonId)}"><span class="eg-recent-skill-icon" data-skill="${escapeAttr(a.skill||'practice')}" aria-hidden="true"></span><span class="eg-recent-copy"><strong>${escapeHtml(lesson)}</strong><small>${escapeHtml(skill)} · ${new Date(a.at).toLocaleDateString()}</small></span><b>${escapeHtml(score)}</b><span class="eg-recent-chevron" aria-hidden="true">›</span></button>`;
 }).join(''):'<div class="eg-premium-empty"><span aria-hidden="true">✦</span><div><strong>Your practice history starts here.</strong><p>Complete an activity and your latest results will appear on this page.</p></div></div>';
 title('EnglishGate','Home');
 $('content').innerHTML=`<section class="eg-student-home eg-premium-home">
  ...
  <div class="eg-premium-dashboard">
   <section class="eg-premium-learning-card">
    <div class="eg-premium-learning-copy">
     <p class="eg-premium-kicker">${next?(completed?'Continue learning':'Start here'):available.length?'Practice complete':'Your book'}</p>
     <h2>${next?escapeHtml(next.title):available.length?'You’ve finished the available practice.':'Your activities are on the way.'}</h2>
     <p class="eg-premium-context">${bookTitle}${next?' · Lesson '+next.number:''}</p>
     ${next?`<button class="eg-premium-next-skill" id="nextSkillBtn" type="button"><span class="eg-next-skill-icon" aria-hidden="true">✎</span><span><small>Your next activity</small><strong>${escapeHtml(WORKBOOK_LABELS[step])}</strong></span><b aria-hidden="true">›</b></button><button class="primary-btn eg-premium-continue" id="continueBtn">${completed?'Continue':'Start activity'} <span>→</span></button>`:`<div class="eg-premium-finished"><strong>${available.length?'Available practice complete':'Workbook coming soon'}</strong><p>${available.length?'Revisit any lesson in your book to keep practising.':'Your teacher will let you know when your workbook is ready.'}</p></div>`}
    </div>
    <div class="eg-premium-book-cover" aria-label="${bookTitle}">
     <span class="eg-cover-level">${bookLevel}</span><div class="eg-cover-mark" aria-hidden="true">EG</div><div class="eg-cover-copy"><small>ENGLISHGATE</small><strong>${bookTitle}</strong><span>${next?'Lesson '+next.number:'Keep learning'}</span></div>
    </div>
   </section>

   <aside class="eg-premium-side">
    <div class="eg-premium-quick-actions">
     <button class="eg-premium-action" id="viewClassBookBtn" type="button"><span class="eg-action-icon" aria-hidden="true">▤</span><strong>Class book</strong><b aria-hidden="true">›</b></button>
     <button class="eg-premium-action" id="viewCourseBtn" type="button"><span class="eg-action-icon" aria-hidden="true">▧</span><strong>Workbook</strong><b aria-hidden="true">›</b></button><button class="eg-premium-action" id="viewWritingsBtn" type="button"><span class="eg-action-icon" aria-hidden="true">✎</span><strong>My Writings</strong><b aria-hidden="true">›</b></button>
    </div>
    <section class="eg-premium-progress-card">
     <div class="eg-progress-card-head"><div><span class="eg-premium-kicker">Your book progress</span><strong>Keep building momentum</strong></div><span class="eg-progress-mini-icon" aria-hidden="true">▥</span></div>
     <div class="eg-progress-ring" style="--eg-progress:${pct}"><div><strong>${pct}</strong><span>%</span></div></div>
     <p>${completed} of ${total} available activities complete</p>
     <button class="eg-progress-cta" id="openProgressBtn" type="button">View progress <span>→</span></button>
    </section>
   </aside>
  </div>

  <section class="eg-premium-recent">
   <div class="eg-premium-section-head"><div><span class="eg-premium-kicker">Your learning history</span><h2>Recent practice</h2></div><button class="eg-premium-section-link" id="viewAllPracticeBtn" type="button">View progress <span>→</span></button></div>
   <div class="eg-premium-recent-list">${recentHtml}</div>
  </section>
 </section>`;
 ...
 $('viewWritingsBtn').onclick=()=>{currentPage='my-writings';renderNav();myWritings()};
 $('openProgressBtn').onclick=$('viewAllPracticeBtn').onclick=()=>{currentPage='progress';renderNav();studentProgress()};
 const openNext=()=>{if(!next)return;activeLessonId=next.id;currentStep=step;currentPage='workbook';renderNav();workbook()};
 if($('continueBtn'))$('continueBtn').onclick=openNext;
 if($('nextSkillBtn'))$('nextSkillBtn').onclick=openNext;
...
function studentCourse(){
 ...
 $('content').innerHTML=`<section class="course-shell"><button class="back-link" id="courseHome">← Home</button><div class="course-intro"><div><span class="pill teal">${escapeHtml(b.level||'')}</span><h1>${escapeHtml(b.title)}</h1><p>Revise the class book first, then complete the workbook activities.</p></div><div class="course-overall"><strong>${completionPct(sid)}%</strong><span>Book progress</span>${progress(completionPct(sid))}</div></div><div class="course-path student-course-list">${COURSE.lessons.map(l=>{
   const pct=lessonProgress(sid,l.id),ready=l.ready!==false,complete=pct===100,upgrade=b2UpgradeAvailableForLesson(sid,l);
   const status=upgrade?`<span class="b2-upgrade-badge">Upgrade available</span>${b2ScoreComparisonHtml(sid,l)}`:complete?'Completed':ready?`${WORKBOOK_LABELS[firstOpenStep(sid,l.id)]} next`:'Workbook activity coming next';
   const workbookLabel=upgrade?'Retry upgraded lesson':complete?'Practise again':'Start workbook';
   return `<div class="course-topic ${complete?'complete':''} ${ready?'current':'pending'} ${upgrade?'has-upgrade':''}"><span class="topic-state">${complete?'✓':ready?'→':'○'}</span><span class="topic-copy"><small>Lesson ${l.number}</small><strong>${escapeHtml(l.title)}</strong><span>${status}</span></span><span class="topic-actions">${ready?`<button class="ghost-btn" data-revise-lesson="${l.number}">Revise first</button><button class="primary-btn" data-topic="${l.id}" ${upgrade?'data-retry-upgraded="true"':''}>${workbookLabel}</button>`:'—'}</span></div>`
  }).join('')}</div></section>`;
 $('courseHome').onclick=()=>{currentPage='home';renderNav();studentHome()};
 document.querySelectorAll('[data-topic]').forEach(btn=>btn.onclick=()=>{activeLessonId=btn.dataset.topic;currentStep=btn.dataset.retryUpgraded==='true'?'vocabulary':firstOpenStep(sid,activeLessonId);currentPage='workbook';renderNav();workbook()});
 document.querySelectorAll('[data-revise-lesson]').forEach(btn=>btn.onclick=()=>{activeStudentLiveLessonNumber=Number(btn.dataset.reviseLesson);activeStudentSectionIndex=0;currentPage='student-live-lesson';renderNav();studentLiveLesson()})
}
...
function prepareStudentPronunciationWords(){
 const content=document.querySelector('.student-live-book .eg-stage-content');
 if(!content||content.dataset.pronunciationWordsReady==='1')return;
 content.dataset.pronunciationWordsReady='1';
 content.querySelectorAll('[data-vocab-word]').forEach(el=>{
  el.dataset.studentPronunciationWord=el.dataset.vocabWord||el.textContent.trim();
  el.classList.add('student-pronunciation-word')
 });
 const skip='button,input,select,textarea,option,summary,a,canvas,svg,script,style,[data-vocab-word],[data-live-audio-player],[data-student-pronunciation-word]';
 ...
 nodes.forEach(node=>{
  ...
  parts.forEach(part=>{
   if(/^[\p{L}][\p{L}\p{M}'’\-]*$/u.test(part)){
    ...
    span.className='student-pronunciation-word';
    span.dataset.studentPronunciationWord=part;
...
function setStudentPronunciationMode(on){
 studentPronunciationMode=Boolean(on);
 const shell=document.querySelector('.student-live-book'),btn=$('studentPronunciationToggle');
 shell?.classList.toggle('student-pronunciation-mode',studentPronunciationMode);
 if(btn){btn.classList.toggle('active',studentPronunciationMode);btn.setAttribute('aria-pressed',studentPronunciationMode?'true':'false');btn.setAttribute('aria-label',studentPronunciationMode?'Turn pronunciation off':'Turn pronunciation on');btn.innerHTML='<span class="pronunciation-icon" aria-hidden="true">🔊</span><span class="pronunciation-label">'+(studentPronunciationMode?'Pronunciation on':'Pronunciation')+'</span>'}
 if(studentPronunciationMode)prepareStudentPronunciationWords();
 else stopTeacherPronunciation()
}
function wireStudentPronunciationTool(){
 const shell=document.querySelector('.student-live-book'),content=shell?.querySelector('.eg-stage-content'),btn=$('studentPronunciationToggle');
 if(!shell||!content||!btn)return;
 if(studentPronunciationMode)prepareStudentPronunciationWords();
 setStudentPronunciationMode(studentPronunciationMode);
 btn.onclick=()=>setStudentPronunciationMode(!studentPronunciationMode);
 content.addEventListener('click',e=>{
  if(!studentPronunciationMode)return;
  const target=e.target.closest?.('[data-student-pronunciation-word]');
  if(!target||!content.contains(target))return;
  e.preventDefault();e.stopPropagation();
  playTeacherPronunciation(target,target.dataset.studentPronunciationWord||target.textContent.trim())
 },true)
}
async function studentLiveLesson(){await ensureLiveBooks();const sid=session.id,c=studentClass(sid),live=liveBookForClass(c),wb=workbookForClass(c);if(!live){currentPage='class-book';studentClassBook();return}const l=live.lessons.find(x=>x.number===activeStudentLiveLessonNumber)||live.lessons[0],w=wb?.lessons?.find(x=>x.number===l.number);activeStudentLiveLessonNumber=l.number;const sections=liveSections(l.content),total=sections.length;activeStudentSectionIndex=Math.max(0,Math.min(activeStudentSectionIndex,Math.max(0,total-1)));const section=sections[activeStudentSectionIndex],isLast=activeStudentSectionIndex===total-1,goal=lessonCanDoGoal(l),stageName=section?sectionLabel(section.title,activeStudentSectionIndex,total):'Lesson unavailable';title('Class book','Lesson '+l.number);const stages=sections.map((x,i)=>`<button class="eg-stage ${i===activeStudentSectionIndex?'is-current':''}" data-student-live-section="${i}" ${i===activeStudentSectionIndex?'aria-current="step"':''}><span>${i+1}</span><strong>${escapeHtml(sectionLabel(x.title,i,total).toLowerCase())}</strong></button>`).join('');const nextAction=!total?'':!isLast?'<button class="primary-btn" id="nextStudentLiveSection">Next stage →</button>':w&&w.ready!==false?'<button class="primary-btn" id="openStudentWorkbookFooter">Start workbook →</button>':'<span class="eg-unavailable">Matching workbook is not available yet.</span>';$('content').innerHTML=`<section class="eg-lesson student-live-book"><header class="eg-lesson-header"><button class="ghost-btn" id="backStudentBook">← Lessons</button>${englishGateLogo('englishgate-logo-lesson')}<div><p>${escapeHtml(c?.name||'Class')} · ${escapeHtml(live.title)}</p><h1>Lesson ${l.number} · ${escapeHtml(l.title)}</h1></div>${w&&w.ready!==false?'<button class="ghost-btn" id="openStudentWorkbook">Workbook</button>':''}</header><div class="eg-lesson-layout"><aside class="eg-stage-list"><p class="eg-label">Lesson stages</p><nav aria-label="Lesson stages">${stages}</nav>${goal?`<details class="eg-goal"><summary>Lesson goal</summary><p>${escapeHtml(goal)}</p></details>`:''}</aside><div class="eg-teaching-surface"><header class="eg-stage-heading student-live-stage-heading"><div><p class="eg-label">${total?'Stage '+(activeStudentSectionIndex+1)+' of '+total:'No stages'}</p><h2 id="liveStageTitle" tabindex="-1">${escapeHtml(stageName.toLowerCase())}</h2></div><button class="ghost-btn student-pronunciation-toggle" type="button" id="studentPronunciationToggle" aria-pressed="${studentPronunciationMode?'true':'false'}"><span class="pronunciation-icon" aria-hidden="true">🔊</span><span class="pronunciation-label">Pronunciation</span></button></header><article class="live-book-content eg-stage-content ${/reading/i.test(stageName)?'is-reading-stage':''} ${/listening/i.test(stageName)?'is-listening-stage':''} ${/grammar/i.test(stageName)?'is-grammar-stage':''}" aria-labelledby="liveStageTitle">${activeStudentSectionIndex===0?lessonVisualHtml(l):''}${section?renderLiveContent(liveSectionContent(section).split('\n').filter(line=>!/^LESSON\s+\d+|^WEEK\s+\d+.*LESSON\s+\d+/i.test(line.trim())).join('\n')):'<p>This lesson has no learning content yet. Return to the book and choose another lesson.</p>'}</article>${isLast?`<div class="eg-workbook-note">${w?`<strong>After revision</strong><span>Workbook ${w.number} · ${escapeHtml(w.title)}</span>`:'<span>No matching workbook for this lesson.</span>'}</div>`:''}<footer class="eg-lesson-footer"><button class="ghost-btn" id="prevStudentLiveSection" ${activeStudentSectionIndex===0?'disabled':''}>← Previous</button>${nextAction}</footer></div></div></section>`;$('backStudentBook').onclick=()=>{currentPage='class-book';renderNav();studentClassBook()};const openWorkbook=()=>{activeLessonId=w.id;currentStep=firstOpenStep(sid,w.id);currentPage='workbook';renderNav();workbook()};if($('openStudentWorkbook'))$('openStudentWorkbook').onclick=openWorkbook;if($('openStudentWorkbookFooter'))$('openStudentWorkbookFooter').onclick=openWorkbook;const goToStage=async index=>{activeStudentSectionIndex=index;await studentLiveLesson();focusWithoutScroll($('liveStageTitle'));resetAppScroll()};document.querySelectorAll('[data-student-live-section]').forEach(b=>b.onclick=()=>goToStage(Number(b.dataset.studentLiveSection)));$('prevStudentLiveSection').onclick=()=>{if(activeStudentSectionIndex>0)goToStage(activeStudentSectionIndex-1)};if($('nextStudentLiveSection'))$('nextStudentLiveSection').onclick=()=>goToStage(activeStudentSectionIndex+1);wireLiveVocabulary();wireLiveChecks();wireLiveAudioPlayers();wireStudentPronunciationTool()}
...
{type:'open',stage:'Personalise',q:`Write one true sentence about your own life or work using “${e[1].text}”`,min:6,tag:'vocabulary:personalisation'},
...
function guidedChunks(chunks){const items=chunks.flatMap(c=>c.items.map(item=>({stage:c.label,item}))),total=items.length;return `<div class="activity-runner" data-activity-runner><div class="runner-meta"><span data-runner-stage>${escapeHtml(items[0]?.stage||'Practice')}</span></div><div class="runner-progress"><span data-runner-progress style="width:${total?100/total:100}%"></span></div><div class="runner-question-stack" data-runner-stack>${items.map((x,i)=>`<div class="runner-question" data-runner-question data-stage="${escapeAttr(x.stage)}" ${i?'hidden':''}>${x.item}</div>`).join('')}</div><div class="runner-actions"><button class="runner-back" type="button" data-runner-prev hidden>Back</button><button class="primary-btn runner-next" type="button" data-runner-next disabled>${total===1?'Finish':'Next'}</button></div></div>`}
 ...
 return `<div class="eg-skill-page eg-vocabulary-page">
  ...
  <div class="eg-skill-layout">
   <aside class="eg-editorial-panel eg-vocab-reference">
    ...
    <div class="eg-vocab-reference-grid">${refs.length?refs.map((x,i)=>`<article class="eg-vocab-reference-card"><span>${i+1}</span><strong>${escapeHtml(x.word)}</strong><p>${escapeHtml(x.meaning)}</p>${x.example?`<small>“${escapeHtml(x.example)}”</small>`:''}</article>`).join(''):'<p class="muted">Use the activity questions to discover today’s key language.</p>'}</div>
...
function listeningActivity(l){
 ...
 if(split){
  return `<div class="eg-skill-page eg-reading-listening-page">
   ...
   <section class="eg-source-task-section eg-reading-section eg-reading-connected">
...
['infinitive','After language about a goal or something you want to do, look for “to + base verb”.'],
...
['connector-because','Use “because” when the second idea gives a reason.'],
['connector-but','Use “but” when the second idea contrasts with the first.'],
['connector-so','Use “so” when the second idea is a result.'],
['connector','Decide whether the ideas show addition, contrast, reason or result.'],
['gerund','After “responsible for”, the next verb normally uses the -ing form.'],
...
function activityDraftKey(){
 ...
 const draftVersion=activeBookId==='speakup-b2'?'eg-workbook-draft-b2-living-standard-v1':'eg-workbook-draft-v1';
 return [draftVersion,session.id,activeBookId||'book',activeLessonId,currentStep].join(':')
}
...
function syncAudioUi(){
 ...
 if(btn)btn.textContent=activeAudio.paused?'▶':'❚❚';
}
...
function setWorkbookAudioError(message){
 ...
 if(status)status.textContent=message||'Natural listening voice is temporarily unavailable. Tap retry.'
}
function playListening(l){
 ...
 if(activeAudio.paused){const promise=activeAudio.play();if(promise?.catch)promise.catch(()=>setWorkbookAudioError('Audio playback failed. Tap retry.'))}else{activeAudio.pause();if(status)status.textContent='Paused'}
 syncAudioUi();
}
...
function wireStudentQuestionFlow(){
 if(session?.role!=='student'||isWorkbookPreview())return;
 const root=$('activityPanel');if(!root)return;
 const questions=[...root.querySelectorAll('.activity-question-list > .guided-question,.writing-core-sequence > .guided-question,.final-writing-card.guided-question')];
 if(questions.length<2)return;
 document.body.classList.add('student-question-focus-mode');
 root.dataset.studentQuestionFlow='1';
 const containers=[...new Set(questions.map(q=>q.closest('.activity-question-list,.writing-core-sequence')).filter(Boolean))];
 containers.forEach(list=>list.dataset.studentQuestionFlow='1');
 let advanceTimer=0,activeReadingSource=null;
 ...
 const isWritingCore=q=>Boolean(q.matches?.('[data-writing-core]'));
 const answered=q=>{
  if(isWritingCore(q)){
   const type=q.dataset.writingCore||'';
   if(type==='build'||type==='organize'){
    const total=q.querySelectorAll('[data-writing-piece]').length,used=q.querySelectorAll('[data-writing-piece-answer] [data-writing-piece]').length;
    return total>0&&used===total;
   }
   return Boolean(q.querySelector('[data-writing-core-input]')?.value.trim());
  }
  return Boolean(q.querySelector('input[type="radio"]:checked')||[...q.querySelectorAll('textarea,input[type="text"]')].some(el=>el.value.trim().length>0));
 };
 let firstOpen=questions.findIndex(q=>!answered(q));
 let index=firstOpen<0?questions.length-1:firstOpen,furthest=index;
 ...
 const fitQuestion=()=>{
  ...
  const list=q.closest('.activity-question-list,.writing-core-sequence')||q.parentElement;
  ...
  const topbarEl=document.querySelector('.student-mode .topbar'),bottomEl=document.querySelector('.student-mode .bottom-nav');
  const topbar=topbarEl&&getComputedStyle(topbarEl).display!=='none'?topbarEl.getBoundingClientRect().height:0;
  const bottom=bottomEl&&getComputedStyle(bottomEl).display!=='none'?bottomEl.getBoundingClientRect().height:0;
  const head=progress.getBoundingClientRect().height||58;
  const inlineCheck=Boolean(q.querySelector('[data-writing-core-check]'));
  const action=(continueBtn.dataset.autoAdvance==='1'||inlineCheck)?0:(continueBtn.getBoundingClientRect().height||46)+8;
  const available=Math.max(250,viewport-topbar-bottom-head-action-24);
  if(list?.style)list.style.setProperty('--question-space',available+'px');
 ...
 const sync=({focus=false,scroll=false}={})=>{
  const q=questions[index];if(!q)return;
  questions.forEach((card,i)=>{
   const unlocked=i<=furthest;
   card.hidden=!unlocked;
   card.classList.toggle('is-flow-current',unlocked&&i===index);
   card.classList.toggle('is-flow-previous',unlocked&&i!==index);
   card.dataset.flowIndex=String(i);
  });
  containers.forEach(list=>{
   const visible=[...list.querySelectorAll(':scope > .guided-question')].some(card=>!card.hidden);
   list.dataset.flowActiveList=visible?'1':'0';
  });
  root.querySelectorAll('.eg-source-task-section,.eg-shared-comprehension').forEach(sec=>{
   const cards=[...sec.querySelectorAll('.guided-question')];
   if(cards.length)sec.hidden=!cards.some(card=>!card.hidden);
  });
  ...
  continueBtn.dataset.autoAdvance=(choice||inlineCheck)&&!last?'1':'0';
  activeReadingSource=null;
  const section=q.closest('.eg-source-task-section');
  ...
  if(focus){q.setAttribute('tabindex','-1');try{q.focus({preventScroll:true})}catch{}}
  fitQuestion();
  if(scroll)requestAnimationFrame(()=>q.scrollIntoView({behavior:'smooth',block:'start',inline:'nearest'}));
 };
 const move=(nextIndex,{unlock=true,scroll=true}={})=>{
  clearTimeout(advanceTimer);
  index=Math.max(0,Math.min(questions.length-1,nextIndex));
  if(unlock)furthest=Math.max(furthest,index);
  sync({focus:true,scroll});
 };
 ...
 root.addEventListener('pointerdown',e=>{const card=e.target.closest?.('.guided-question');if(card&&!card.hidden)activateCard(card)},{capture:true});
 root.addEventListener('focusin',e=>{const card=e.target.closest?.('.guided-question');if(card&&!card.hidden)activateCard(card)});
 root.addEventListener('change',e=>{
  const radio=e.target.closest?.('input[type="radio"]');if(!radio)return;
  const card=radio.closest('.guided-question'),i=questions.indexOf(card);if(i<0||i>furthest)return;
  index=i;sync();setTimeout(fitQuestion,0);
  if(i>=questions.length-1)return;
  clearTimeout(advanceTimer);
  const hasLearningFeedback=Boolean(card.querySelector('[data-vocab-feedback]'));
  advanceTimer=setTimeout(()=>move(i+1),hasLearningFeedback?520:300);
 });
 root.addEventListener('input',e=>{
  const card=e.target.closest?.('.guided-question'),i=questions.indexOf(card);if(i<0||i>furthest)return;
  if(i!==index){index=i;sync()}
  continueBtn.disabled=!answered(card);fitQuestion();
 });
 root.addEventListener('click',e=>{
  const check=e.target.closest?.('[data-writing-core-check]');
  if(check){
   const card=check.closest('.guided-question'),i=questions.indexOf(card);if(i<0||i>furthest)return;
   index=i;sync();clearTimeout(advanceTimer);
   advanceTimer=setTimeout(()=>{
    if(card.dataset.coreComplete!=='1'){sync();return}
    if(i<questions.length-1)move(i+1);
   },card.dataset.coreCorrect==='1'?520:850);
   return;
  }
  if(e.target.closest?.('[data-writing-piece],[data-writing-reset]'))setTimeout(()=>{continueBtn.disabled=!answered(questions[index]);fitQuestion()},0);
 });
...
async function finishB2Understand(l){
 ...
 const correct=selected.filter(x=>x.value===x.dataset.answer).length,score=Math.round(correct/qs.length*100),evidence=qs.map((q,i)=>({index:i+1,question:q.q,studentAnswer:selected[i]?.value||'',correctAnswer:q.answer||selected[i]?.dataset.answer||'',correct:Boolean(selected[i]&&selected[i].value===(q.answer||selected[i].dataset.answer)),tag:q.tag||''}));
 if(!isWorkbookPreview()){await recordAttempt(session.id,l.id,'listening',score,qs.map(q=>q.tag).filter(Boolean).slice(0,8),evidence);await markDone(session.id,l.id,'listening');await refreshState()}
...
async function finishB2Interact(l){
 ...
 const richer=answers.filter(x=>b2WordCount(x)>=7).length,score=Math.min(100,60+richer*8),evidence=boxes.map((x,i)=>({index:i+1,question:String(document.querySelector('[data-b2-prompt="'+i+'"]')?.textContent||('Conversation turn '+(i+1))).trim(),studentAnswer:answers[i],correctAnswer:'Authentic open response',correct:null,tag:'conversation:turn'}));
 saveB2Evidence({interact:answers});
 if(!isWorkbookPreview()){await recordAttempt(session.id,l.id,'vocabulary',score,['conversation:responds','conversation:extends','conversation:follow-up','speaking:relevance','speaking:fluency'],evidence);await markDone(session.id,l.id,'vocabulary');await refreshState()}
...
async function finishB2Produce(l){
 ...
 if(!isWorkbookPreview()){
  ...
  await recordAttempt(session.id,l.id,'writing',100,['writing:real-life-message','writing:interactive-follow-up','writing:task-completion'],[{index:1,question:'Message to Sara',studentAnswer:message,correctAnswer:'Authentic task response',correct:null,tag:'writing:real-life-message'},{index:2,question:'Reply to Sara',studentAnswer:followUp,correctAnswer:'Authentic follow-up response',correct:null,tag:'writing:interactive-follow-up'}]);
  await refreshState()
 }
...
async function finishB2Improve(l){
 ...
 if(!isWorkbookPreview()){await recordAttempt(session.id,l.id,'grammar',score,['improve:personalized','improve:reapply','conversation:repair'],currentQuestionEvidence());await markDone(session.id,l.id,'grammar');await refreshState()}
...
function wireWritingIntegrity(){
 ...
 box.dataset.integrityWired='1';
 const explain=()=>{if(status){status.textContent='Copy and paste are disabled here. Please type your own response.';status.classList.add('is-visible');setTimeout(()=>status.classList.remove('is-visible'),2600)}};
 const block=e=>{e.preventDefault();explain();box.focus()};
 ['copy','cut','paste','drop','dragstart'].forEach(type=>box.addEventListener(type,block));
 box.addEventListener('beforeinput',e=>{if(['insertFromPaste','insertFromDrop'].includes(e.inputType)){e.preventDefault();explain()}});
 box.addEventListener('keydown',e=>{if((e.ctrlKey||e.metaKey)&&['c','x','v'].includes(String(e.key||'').toLowerCase())){e.preventDefault();explain()}});
}
...
async function saveWriting(l){
 ...
 try{
  ...
  await recordAttempt(session.id,l.id,'writing',finalScore,[`ladder:highest:${ladder.highest}`,'writing:task-completion','writing:grammar','writing:vocabulary','writing:clarity'],[{index:1,question:spec.task,studentAnswer:response,correctAnswer:'Evaluated against the writing rubric',correct:null,tag:'writing:task-completion'}]);
...
async function checkCurrent(){
 ...
 try{
  await recordAttempt(session.id,activeLessonId,currentStep,score,tags,currentQuestionEvidence());
...
async function recordAttempt(studentId,lessonId,skill,score,tags=[],evidence=[]){await api('/api/attempts',{method:'POST',body:JSON.stringify({lessonId,skill,score,tags:[...new Set(tags)],evidence:Array.isArray(evidence)?evidence:[]})})}
async function markDone(studentId,lid,step){await api('/api/completion',{method:'POST',body:JSON.stringify({lessonId:lid,step})})}
function certificateForStudent(sid,bookId=bookIdForStudent(sid)){return (getDB().certificates||[]).find(c=>c.studentId===sid&&c.bookId===bookId)||null}
function certificateStatusHtml(sid){const cert=certificateForStudent(sid),pct=completionPct(sid),book=bookMeta(bookIdForStudent(sid));if(cert)return `<section class="section card"><span class="pill teal">Level completed</span><h3>${escapeHtml(cert.level)} Level Completion Certificate</h3><p>Issued ${new Date(cert.issuedAt).toLocaleDateString()} · ${escapeHtml(cert.certificateNumber)}</p><button class="primary-btn" type="button" data-view-student-certificate="${escapeAttr(cert.studentId)}">View certificate</button></section>`;return `<section class="section card"><span class="pill">${pct}% complete</span><h3>Level Completion Certificate</h3><p>${pct===100?'The level is complete. The certificate is ready to issue.':'Certificate unlocks when all required workbook activities in '+escapeHtml(book?.title||'this level')+' are completed.'}</p></section>`}
function renderLevelCertificate(cert){const date=new Date(cert.issuedAt).toLocaleDateString(undefined,{year:'numeric',month:'long',day:'numeric'}),verify=location.origin+'/api/certificates/verify/'+encodeURIComponent(cert.certificateNumber),w=window.open('','_blank');if(!w){alert('Allow pop-ups to view the certificate.');return}w.opener=null;w.document.write(`<!doctype html><html><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>${escapeHtml(cert.level)} Certificate · ${escapeHtml(cert.studentName)}</title><style>body{margin:0;background:#f3f6fb;font-family:Georgia,serif;color:#14213d}.toolbar{padding:14px;text-align:center;font-family:Arial,sans-serif}.toolbar button{padding:12px 20px;border:0;border-radius:10px;background:#1d4ed8;color:#fff;font-weight:700}.sheet{box-sizing:border-box;max-width:1000px;min-height:680px;margin:20px auto;background:#fff;border:14px solid #173b7a;outline:3px solid #d4af37;outline-offset:-28px;padding:78px 70px;text-align:center;box-shadow:0 18px 55px #0002}.brand{font:700 15px Arial,sans-serif;letter-spacing:.18em;text-transform:uppercase;color:#173b7a}.award{margin:18px 0 6px;font-size:42px;color:#173b7a}.lead{font-size:20px;margin:25px 0 8px}.name{font-size:44px;margin:8px 0;border-bottom:1px solid #c8a94a;padding-bottom:12px}.level{font-size:30px;margin:20px 0 8px}.course{font-size:19px}.meta{margin-top:55px;display:flex;justify-content:space-between;gap:20px;font:14px Arial,sans-serif;color:#475569}.verify{margin-top:28px;font:12px Arial,sans-serif;color:#64748b;word-break:break-all}@media print{body{background:#fff}.toolbar{display:none}.sheet{margin:0;box-shadow:none;max-width:none;min-height:95vh}}</style></head><body><div class="toolbar"><button onclick="window.print()">Print / Save as PDF</button></div><main class="sheet"><div class="brand">Islamic Online University-Borama · EnglishGate</div><h1 class="award">Level Completion Certificate</h1><p class="lead">This certifies that</p><div class="name">${escapeHtml(cert.studentName)}</div><p>has successfully completed all required EnglishGate digital workbook activities for</p><div class="level">${escapeHtml(cert.level)}</div><div class="course">${escapeHtml(cert.bookTitle)}</div><div class="meta"><span>Issued: ${escapeHtml(date)}</span><span>Teacher: ${escapeHtml(cert.teacherName||'EnglishGate')}</span><span>Certificate No: ${escapeHtml(cert.certificateNumber)}</span></div><div class="verify">Verification: ${escapeHtml(verify)}</div></main></body></html>`);w.document.close()}
async function claimLevelCertificate(){try{const r=await api('/api/certificates/claim',{method:'POST',body:'{}'}),cert=r.certificate;if(apiDB){apiDB.certificates=apiDB.certificates||[];const i=apiDB.certificates.findIndex(x=>x.id===cert.id);if(i>=0)apiDB.certificates[i]=cert;else apiDB.certificates.unshift(cert)}renderLevelCertificate(cert)}catch(e){showModal(`<div class="section-head"><div><span class="role-kicker">Level certificate</span><h3>Not ready yet</h3><p class="muted">${escapeHtml(e.message)}</p></div><button class="icon-btn" data-close>×</button></div>`);document.querySelector('[data-close]').onclick=closeModal}}
function studentProgress(){const sid=session.id,w=weakest(sid),ats=attempts(sid).slice(-5).reverse(),c=courseForStudent(sid),b=bookMeta(bookIdForStudent(sid))||{title:c.title||c.moduleTitle},scored=scoredActivityCount(sid),completed=completedActivityCount(sid),pct=completionPct(sid),cert=certificateForStudent(sid),total=readyLessons(c).reduce((n,l)=>n+workbookStepsForLesson(l).length,0),remaining=Math.max(0,total-completed);title('EnglishGate Workbook','Progress');$('content').innerHTML=`<section class="progress-shell"><div class="progress-hero-card"><div><span class="pill teal">${escapeHtml(b.title)}</span><h1>${pct}% complete</h1><p>${completed} of ${total} available workbook activities completed.</p></div><button class="ghost-btn" id="progressCourseBtn">My book</button></div><section class="section card"><span class="pill gold">Level Completion Certificate</span><h3>${pct===100?'Level complete — certificate unlocked':'Complete the level to unlock your certificate'}</h3><p>${pct===100?(cert?'Your permanent certificate has been issued.':'All required activities are complete. Generate your permanent certificate now.'):`${remaining} workbook activit${remaining===1?'y':'ies'} remaining before the certificate unlocks.`}</p>${pct===100?`<button class="primary-btn" id="levelCertificateBtn">${cert?'View certificate':'Get my certificate'}</button>`:progress(pct)}</section><div class="student-skill-grid">${['vocabulary','listening','grammar','writing'].map(k=>skillCard(skillLabel(k),mastery(sid,k),'Latest lesson scores')).join('')}</div><section class="section student-progress-grid"><div class="card"><span class="pill gold">Lowest recorded score average</span>${w?`<h3>${w.label} · ${w.score}%</h3><p>${w.tip}</p>${progress(w.score)}`:'<h3>No scored evidence yet</h3><p>Complete a scored activity to see a performance area.</p>'}</div><div class="card"><span class="pill">Evidence</span><h3>${scored} scored activities</h3><p>${completed} workbook activities completed.</p></div></section><section class="section card"><div class="section-head"><div><h3>Recent practice</h3><p>Exact scores from your latest five recorded workbook attempts.</p></div></div>${ats.length?ats.map(a=>`<div class="attempt-row"><div><strong>${escapeHtml(lessonById(a.lessonId)?.title||'Lesson')} · ${skillLabel(a.skill)}</strong><span>${new Date(a.at).toLocaleDateString()}</span></div><strong>${a.score}%</strong></div>`).join(''):'<p class="muted">Complete an activity to start your evidence history.</p>'}</section></section>`;$('progressCourseBtn').onclick=()=>{currentPage='course';renderNav();studentCourse()};if($('levelCertificateBtn'))$('levelCertificateBtn').onclick=()=>cert?renderLevelCertificate(cert):claimLevelCertificate()}
...
function teacherAttentionSummary(){
 ...
 assignments.forEach(a=>students.filter(s=>s.classIds?.includes(a.classId)).forEach(s=>{const l=lessonById(a.lessonId),required=Array.isArray(a.skills)&&a.skills.length?a.skills.length:(l?workbookStepsForLesson(l).length:WORKBOOK_STEPS.length);if(skillCompletionFor(s.id,a.lessonId).length<required)unfinished.add(s.id)}));
...
function teacherHome(){
 ...
 if(resume){
  ...
  hero=`<section class="teacher-assist-hero">
   ...
   <div class="teacher-assist-main"><div><span class="role-kicker">Continue where you left off</span><h1>Lesson ${resume.lesson.number} · ${escapeHtml(resume.lesson.title)}</h1><p><strong>${escapeHtml(resume.c.name)}</strong> · ${escapeHtml(resume.stageName)} · Stage ${resume.sectionIndex+1} of ${Math.max(1,resume.sections.length)}</p><small>${escapeHtml(teacherContextAge(resume.ctx.updatedAt))}</small></div><button class="primary-btn teacher-resume-btn" id="continueTeaching">Continue teaching →</button></div>
...
async function playTeacherPronunciation(target,word){
 ...
 try{
  ...
  if(!url){
   const res=await fetch('/api/pronunciation',{method:'POST',credentials:'include',headers:{'Content-Type':'application/json'},body:JSON.stringify({word:clean})});
...
function teacherLiveLesson(){
 ...
 $('content').innerHTML=`<section class="eg-lesson">
  ...
  <div class="teacher-annotation-stage" id="teacherAnnotationStage" data-annotation-mode="${teacherLiveTool}">
   <article class="live-book-content eg-stage-content ${/reading/i.test(stageName)?'is-reading-stage':''} ${/listening/i.test(stageName)?'is-listening-stage':''} ${/grammar/i.test(stageName)?'is-grammar-stage':''}" aria-labelledby="liveStageTitle">${activeTeacherSectionIndex===0?lessonVisualHtml(l):''}${section?renderLiveContent(liveSectionContent(section).split('\n').filter(line=>!/^LESSON\s+\d+|^WEEK\s+\d+.*LESSON\s+\d+/i.test(line.trim())).join('\n')):'<p>This lesson has no teaching content yet. Return to the book and choose another lesson.</p>'}</article>
...
function openAssignWorkbook(c,liveLesson,workbookLesson){const isA1Gold=workbookLesson?.standardVersion==='a1-gold-v1',assignmentSkills=isA1Gold?workbookStepsForLesson(workbookLesson):WORKBOOK_STEPS,assignmentLabels=assignmentSkills.map(k=>isA1Gold?workbookStageLabel(workbookLesson,k):skillLabel(k));showModal(`<div class="section-head"><div><span class="role-kicker">Post-class action</span><h3>Assign matching workbook</h3><p class="muted">${escapeHtml(c.name)}</p></div><button class="icon-btn" data-close>×</button></div><div class="assignment-match-card"><span>Live lesson</span><strong>Lesson ${liveLesson.number} · ${escapeHtml(liveLesson.title)}</strong><span>↓ automatically matched</span><strong>Workbook Lesson ${workbookLesson.number} · ${escapeHtml(workbookLesson.title)}</strong></div><div class="assignment-skill-row">${assignmentLabels.map(label=>`<span>${escapeHtml(label)}</span>`).join('')}</div><button class="primary-btn" id="confirmAssignment">Assign to class</button><div id="assignResult"></div>`);document.querySelector('[data-close]').onclick=closeModal;$('confirmAssignment').onclick=async()=>{const btn=$('confirmAssignment');btn.disabled=true;btn.textContent='Assigning…';try{const a=await api('/api/teacher/assignments',{method:'POST',body:JSON.stringify({classId:c.id,bookId:c.bookId||c.course_id,lessonId:workbookLesson.id,lessonNumber:workbookLesson.number,lessonTitle:workbookLesson.title,skills:assignmentSkills})});await refreshState();const link=window.location.origin+'/?assignment='+encodeURIComponent(a.id),message=`${c.name}: Lesson ${workbookLesson.number} · ${workbookLesson.title} workbook is ready. Complete ${assignmentLabels.join(', ')}: ${link}`;$('assignResult').innerHTML=`<div class="assignment-success"><strong>Assigned to ${escapeHtml(c.name)}</strong><p>Share this WhatsApp message with the class. The link opens the exact workbook lesson.</p><textarea id="whatsappMessage" readonly>${escapeHtml(message)}</textarea><div class="student-cta-row"><button class="secondary-btn" id="copyWhatsApp">Copy WhatsApp message</button><button class="primary-btn" id="openWhatsApp">Open WhatsApp</button></div></div>`;btn.classList.add('hidden');$('copyWhatsApp').onclick=async()=>{await navigator.clipboard.writeText(message);$('copyWhatsApp').textContent='Copied ✓'};$('openWhatsApp').onclick=()=>window.open('https://wa.me/?text='+encodeURIComponent(message),'_blank')}catch(e){btn.disabled=false;btn.textContent='Assign to class';$('assignResult').innerHTML=`<div class="feedback bad">${escapeHtml(e.message)}</div>`}}}
...
function studentRow(s){const w=weakest(s.id),className=getDB().classes.find(c=>s.classIds?.includes(c.id))?.name||'—';return `<tr><td><div class="student-cell">${studentAvatarMarkup(studentPhoto(s.id),s.name,'roster-avatar')}<div><button class="student-name-link" data-student-report="${escapeAttr(s.id)}" type="button">${escapeHtml(s.name)}</button><div class="muted">@${escapeHtml(s.username)}</div><small>WhatsApp: ${escapeHtml(s.whatsappNumber||'Not recorded')}</small></div></div></td><td>${escapeHtml(className)}</td><td>${completionPct(s.id)}%</td><td>${w?.label||'No scored activity yet'}</td><td><div class="management-row-meta"><button class="primary-btn compact-action" data-manage-student="${escapeAttr(s.id)}">Manage</button><button class="ghost-btn" data-review-student="${escapeAttr(s.id)}">View report</button></div></td></tr>`}
function bindStudentActions(){document.querySelectorAll('[data-student-report]').forEach(b=>b.onclick=()=>openStudentReportPage(b.dataset.studentReport));document.querySelectorAll('[data-review-student]').forEach(b=>b.onclick=()=>openStudentReportPage(b.dataset.reviewStudent));document.querySelectorAll('[data-manage-student]').forEach(b=>b.onclick=()=>openManageStudent(b.dataset.manageStudent));document.querySelectorAll('[data-password-student]').forEach(b=>b.onclick=()=>openStudentPasswordHelp(b.dataset.passwordStudent));document.querySelectorAll('[data-transfer-student]').forEach(b=>b.onclick=()=>openTransferStudent(b.dataset.transferStudent))}
function openManageStudent(id){
 const db=getDB(),st=db.users.find(u=>u.id===id);if(!st||st.role!=='student')return;
 const classes=db.classes||[],current=studentClass(id),photo=studentPhoto(id);
 const endpoint=session?.role==='admin'?'/api/admin/students/'+encodeURIComponent(id)+'/manage':'/api/teacher/students/'+encodeURIComponent(id)+'/manage';
 showModal(`<section class="student-manage-modal"><div class="section-head"><div class="student-manage-heading">${studentAvatarMarkup(photo,st.name,'manage-student-avatar')}<div><span class="role-kicker">Student management</span><h3>${escapeHtml(st.name)}</h3><p class="muted">Edit account details, move class, or help with access without changing learning history.</p></div></div><button class="icon-btn" data-close>×</button></div>
 <form id="manageStudentForm" class="form-grid">
  <label>Student name<input id="manageStudentName" value="${escapeAttr(st.name)}" required maxlength="100"></label>
  <label>Username<input id="manageStudentUsername" value="${escapeAttr(st.username)}" required maxlength="32"></label>
  <label>WhatsApp number<input id="manageStudentWhatsapp" type="tel" value="${escapeAttr(st.whatsappNumber||'')}" required maxlength="40"><small>Use the number the student receives EnglishGate messages on.</small></label>
  <label>Class<select id="manageStudentClass">${classes.map(c=>`<option value="${escapeAttr(c.id)}" ${c.id===current?.id?'selected':''}>${escapeHtml(c.name)} · ${escapeHtml(bookMeta(c.bookId||c.course_id)?.title||c.level)}</option>`).join('')}</select><small>Moving class keeps all existing scores, writing, attempts, and completion records.</small></label>
  <div class="student-manage-actions"><button class="primary-btn" type="submit">Save changes</button><button class="ghost-btn" id="manageStudentPassword" type="button">Change password</button></div>
 </form><div id="manageStudentResult"></div></section>`);
 document.querySelector('[data-close]').onclick=closeModal;
 $('manageStudentPassword').onclick=()=>{closeModal();openStudentPasswordHelp(id)};
 $('manageStudentForm').onsubmit=async e=>{
  e.preventDefault();const btn=e.submitter||$('manageStudentForm').querySelector('button[type="submit"]'),result=$('manageStudentResult');btn.disabled=true;btn.textContent='Saving…';
  try{
   await api(endpoint,{method:'PATCH',body:JSON.stringify({name:$('manageStudentName').value.trim(),username:$('manageStudentUsername').value.trim(),whatsappNumber:$('manageStudentWhatsapp').value.trim(),classId:$('manageStudentClass').value})});
   await refreshState();closeModal();renderPage()
  }catch(err){btn.disabled=false;btn.textContent='Save changes';result.innerHTML=`<div class="feedback bad">${escapeHtml(err.message)}</div>`}
 }
}
function openStudentPasswordHelp(id){
 ...
 showModal(`<div class="section-head"><div><span class="role-kicker">Student access</span><h3>Password help · ${escapeHtml(st.name)}</h3><p class="muted">@${escapeHtml(st.username)} · Existing passwords cannot be viewed. You can replace the password below.</p></div><button class="icon-btn" data-close>×</button></div>
  <section class="password-help-card">
   <label>Set a new password
    <div class="password-edit-row"><input id="studentNewPassword" type="password" inputmode="numeric" autocomplete="new-password" minlength="8" maxlength="20" pattern="[0-9]{8,20}" placeholder="8–20 digits"><button class="ghost-btn" id="toggleStudentPassword" type="button">Show</button></div>
    <small>Use 8–20 digits. Give the new password directly to the student.</small>
   </label>
 ...
 const save=async password=>{
  result.innerHTML='<div class="muted">Updating password…</div>';
  try{
   ...
   $('copyStudentPassword').onclick=async()=>{try{await navigator.clipboard.writeText(pw);$('copyStudentPassword').textContent='Copied ✓'}catch{$('copyStudentPassword').textContent='Copy failed'}};
  }catch(e){result.innerHTML=`<div class="feedback bad">${escapeHtml(e.message)}</div>`}
 };
 $('setStudentPassword').onclick=()=>{const pw=input.value.trim();if(!/^\d{8,20}$/.test(pw)){result.innerHTML='<div class="feedback bad">Enter a password containing 8–20 digits.</div>';return}save(pw)};
 $('generateStudentPassword').onclick=()=>save('');
}
function openTransferStudent(id){const db=getDB(),st=db.users.find(u=>u.id===id),classes=db.classes||[],current=studentClass(id);if(!st)return;if(!classes.length){showModal(`<div class="section-head"><div><span class="role-kicker">Transfer student</span><h3>No available classes</h3><p class="muted">Create or assign a class before moving students.</p></div><button class="icon-btn" data-close>×</button></div>`);document.querySelector('[data-close]').onclick=closeModal;return}showModal(`<div class="section-head"><div><span class="role-kicker">Transfer student</span><h3>${escapeHtml(st.name)}</h3><p class="muted">Current class: ${escapeHtml(current?.name||'—')}</p></div><button class="icon-btn" data-close>×</button></div><form id="transferStudentForm" class="form-grid"><label>Move to class<select id="transferClass">${classes.map(c=>`<option value="${escapeAttr(c.id)}" ${c.id===current?.id?'selected':''}>${escapeHtml(c.name)} · ${escapeHtml(bookMeta(c.bookId||c.course_id)?.title||c.level)}</option>`).join('')}</select></label><button class="primary-btn" type="submit">Transfer student</button></form><div id="transferResult"></div>`);document.querySelector('[data-close]').onclick=closeModal;$('transferStudentForm').onsubmit=async e=>{e.preventDefault();const btn=e.submitter||$('transferStudentForm').querySelector('button[type="submit"]'),result=$('transferResult'),classId=$('transferClass').value;btn.disabled=true;btn.textContent='Transferring...';try{const endpoint=session?.role==='admin'?`/api/admin/students/${id}/class`:`/api/teacher/students/${id}/class`;await api(endpoint,{method:'PATCH',body:JSON.stringify({classId})});await refreshState();closeModal();renderPage()}catch(err){btn.disabled=false;btn.textContent='Transfer student';result.innerHTML=`<div class="feedback bad">${escapeHtml(err.message)}</div>`}}}
function openAddStudent(prefillClassId=''){const cls=getDB().classes;if(!cls.length){showModal(`<div class="section-head"><div><span class="role-kicker">Students</span><h3>No class assigned</h3><p class="muted">A System Admin must create a class and assign it to you before you can add students.</p></div><button class="icon-btn" data-close>×</button></div>`);document.querySelector('[data-close]').onclick=closeModal;return}const selected=cls.some(c=>c.id===prefillClassId)?prefillClassId:cls[0].id;showModal(`<div class="section-head"><div><span class="role-kicker">New student</span><h3>Add student to your class</h3><p class="muted">The student automatically receives the book assigned to the class you choose.</p></div><button class="icon-btn" data-close>×</button></div><form id="studentForm" class="form-grid"><label>Student name<input id="newName" required></label><label>WhatsApp number<input id="newWhatsapp" type="tel" autocomplete="tel" required maxlength="40" placeholder="+252 63 1234567"><small>Include the country code, starting with +.</small></label><label>Username<input id="newUsername" required placeholder="e.g. ahmed.ali"></label><label>Class<select id="newClass">${cls.map(c=>`<option value="${c.id}" ${c.id===selected?'selected':''}>${escapeHtml(c.name)} · ${escapeHtml(bookMeta(c.bookId||c.course_id)?.title||c.level)}</option>`).join('')}</select><small>One class has one book. Moving a student later changes their active class/book without deleting previous learning evidence.</small></label><button class="primary-btn" type="submit">Create student & add to class</button></form><div id="createResult"></div>`);document.querySelector('[data-close]').onclick=closeModal;$('studentForm').onsubmit=async e=>{e.preventDefault();const name=$('newName').value.trim(),username=$('newUsername').value.trim().toLowerCase(),classId=$('newClass').value;try{const r=await api('/api/teacher/students',{method:'POST',body:JSON.stringify({name,username,classId,whatsappNumber:$('newWhatsapp').value.trim()})});await refreshState();$('createResult').innerHTML=credentialResult('Student',r);bindCredentialActions();openWhatsAppLink(r.whatsappLink)}catch(err){$('createResult').innerHTML=`<div class="feedback bad">${escapeHtml(err.message)}</div>`}}}
function finalWritingAssessmentHtml(id){const raw=writingFor(id,'su-b2-l22');if(!raw)return'';const submission=writingSubmission(id,'su-b2-l22'),score=getDB().writingScores?.[id]?.['su-b2-l22'],canGrade=session?.role==='teacher';if(!submission.final)return'';return `<section class="section teacher-writing-assessment"><div class="section-head"><div><span class="pill gold">Lesson 22 · Human graded</span><h3>Final B2 writing assessment</h3></div>${score===null||score===undefined?'':`<strong>${score}%</strong>`}</div><div class="submitted-writing">${escapeHtml(submission.final)}</div>${canGrade?`<label>Grade (0–100)<input id="finalWritingGrade" type="number" min="0" max="100" step="1" value="${score??''}"></label><button class="primary-btn" id="saveFinalWritingGrade">Save grade</button>`:`<p class="muted">${score===null||score===undefined?'Waiting for teacher grade.':'Teacher grade saved.'}</p>`}<div id="writingGradeResult"></div></section>`}
function reportDecisionLabel(value){
 return ({on_track:'On track',watch:'Watch',intervention:'Intervention needed',insufficient_data:'Insufficient evidence',improving:'Improving',stable:'Stable',declining:'Declining',strong:'Strong',moderate:'Moderate',weak:'Weak',recurring:'Recurring',one_off:'One-off',unclear:'Unclear',low:'Low',medium:'Medium',high:'High',continue:'Continue',review_activity:'Review activity',retry_lesson:'Retry lesson',teacher_review:'Teacher review',vocabulary:'Vocabulary',listening:'Listening & Reading',grammar:'Grammar',writing:'Writing',none:'No clear weakness'})[value]||cap(String(value||'').replace(/_/g,' '))}
function reportDecisionConfidence(item){return Number.isFinite(Number(item?.confidence))?Math.round(Number(item.confidence)*100)+'% confidence':''}
function reportNextActionText(action,priority){
 const skill=reportDecisionLabel(priority);
 return action==='review_activity'?'Open the weakest recurring '+skill+' activity and inspect the question evidence.':action==='retry_lesson'?'Give targeted review, then retry the affected lesson.':action==='teacher_review'?'Teacher should inspect the repeated errors and learner responses before the next lesson.':'Continue the planned workbook sequence and keep monitoring evidence.'
}
function performanceNarrative(payload){
 const d=payload?.decision||{},state=payload?.state||{},status=d.status?.choice||'insufficient_data',trend=d.trend?.choice||'insufficient_data',evidence=d.evidence?.choice||'weak',priority=d.priority?.choice||state.prioritySkill||'none',persistence=d.persistence?.choice||'unclear',urgency=d.urgency?.choice||'low',next=d.nextAction?.choice||'continue',scored=Number(state.scoredActivities||0),attemptCount=Number(state.totalAttempts||0),patterns=Array.isArray(state.recurrentErrors)?state.recurrentErrors.length:0;
 const opening=status==='on_track'
  ?'The student is progressing as expected based on the work recorded so far.'
  :status==='watch'
   ?'The student is making progress, but the current evidence shows an area that needs closer teacher attention.'
   :status==='intervention'
    ?'The current evidence shows repeated difficulty that needs targeted teacher support.'
    :'There is not enough scored work yet to make a reliable overall judgment about the student’s performance.';
 const trendText=trend==='improving'
  ?'Recent attempts show improvement.'
  :trend==='declining'
   ?'Recent attempts show a decline, so the teacher should check whether the learner is misunderstanding the skill or rushing through retries.'
   :trend==='stable'
    ?'Performance is currently stable across recent attempts.'
    :'A clear performance trend has not yet formed.';
 const priorityText=priority&&priority!=='none'
  ?`The clearest area for attention is ${reportDecisionLabel(priority)}.${patterns? ` The system has detected ${patterns} recurring error pattern${patterns===1?'':'s'}, suggesting that this is more than a one-off mistake.`:''}`
  :'No single skill is currently standing out as a clear weakness.';
 const evidenceText=evidence==='strong'
  ?`This interpretation is supported by ${scored} scored activit${scored===1?'y':'ies'} across ${attemptCount} attempt${attemptCount===1?'':'s'}.`
  :evidence==='moderate'
   ?`This interpretation is based on ${scored} scored activit${scored===1?'y':'ies'} and should be confirmed as more work is completed.`
   :`The evidence is still limited, so this summary should be treated as an early indication rather than a final judgment.`;
 const urgencyText=urgency==='high'?'This should be addressed before the learner moves much further ahead.':urgency==='medium'?'This should be reviewed in the next teaching cycle.':'There is no urgent intervention signal at the moment.';
 return {opening,trendText,priorityText,evidenceText,urgencyText,nextText:reportNextActionText(next,priority)}
}
function performanceIntelligenceHtml(payload){
 const d=payload?.decision||{},state=payload?.state||{},status=d.status?.choice||'insufficient_data',priority=d.priority?.choice||state.prioritySkill||'none',source=d.source==='jev'?'JEV decision':'Evidence rules',n=performanceNarrative(payload);
 return `<div class="performance-written-report"><div class="performance-written-heading"><div><span class="role-kicker">Teacher performance summary</span><h2>What the evidence says</h2></div><span class="performance-written-source">${escapeHtml(source)}</span></div><div class="performance-written-body"><p><strong>${escapeHtml(n.opening)}</strong></p><p>${escapeHtml(n.trendText)} ${escapeHtml(n.priorityText)}</p><p>${escapeHtml(n.evidenceText)} ${escapeHtml(n.urgencyText)}</p><div class="performance-written-action"><small>Recommended teacher action</small><p>${escapeHtml(n.nextText)}</p></div></div></div><details class="performance-evidence-details"><summary>View supporting evidence</summary><div class="performance-intelligence-head"><div><span class="role-kicker">${escapeHtml(source)}</span><div class="performance-intelligence-status"><strong class="status-${escapeAttr(status)}">${escapeHtml(reportDecisionLabel(status))}</strong><span>${escapeHtml(reportDecisionConfidence(d.status))}</span></div></div><div class="performance-intelligence-score"><small>Recorded evidence</small><strong>${state.scoredActivities||0}</strong><span>scored activities · ${state.totalAttempts||0} attempts</span></div></div><div class="performance-intelligence-grid"><div><small>Trend</small><strong>${escapeHtml(reportDecisionLabel(d.trend?.choice||'insufficient_data'))}</strong><span>${escapeHtml(reportDecisionConfidence(d.trend))}</span></div><div><small>Evidence strength</small><strong>${escapeHtml(reportDecisionLabel(d.evidence?.choice||'weak'))}</strong><span>${escapeHtml(reportDecisionConfidence(d.evidence))}</span></div><div><small>Priority</small><strong>${escapeHtml(reportDecisionLabel(priority))}</strong><span>${state.recurrentErrors?.length?state.recurrentErrors.length+' recurring error pattern'+(state.recurrentErrors.length===1?'':'s'):'No repeated error pattern recorded'}</span></div><div><small>Urgency</small><strong>${escapeHtml(reportDecisionLabel(d.urgency?.choice||'low'))}</strong><span>${escapeHtml(reportDecisionConfidence(d.urgency))}</span></div></div></details>`
}
async function loadStudentPerformanceDecision(id){
 const box=$('performanceIntelligence');if(!box)return;
 try{const payload=await api('/api/performance-decision/'+encodeURIComponent(id));if($('performanceIntelligence'))$('performanceIntelligence').innerHTML=performanceIntelligenceHtml(payload)}
 catch(e){if($('performanceIntelligence'))$('performanceIntelligence').innerHTML='<div class="report-evidence-missing"><strong>Performance decision support unavailable</strong><span>The recorded lesson and activity evidence below is still available for review.</span></div>'}
}
function lessonPerformanceSignal(id,lid){
 const rows=attempts(id,null,lid).filter(a=>Number.isFinite(Number(a.score))).slice().sort((a,b)=>new Date(a.at)-new Date(b.at));if(rows.length<2)return {label:rows.length?'Early evidence':'No evidence',tone:'neutral'};
 const first=averageNumbersClient(rows.slice(0,Math.min(2,rows.length)).map(a=>a.score)),last=averageNumbersClient(rows.slice(-Math.min(2,rows.length)).map(a=>a.score)),delta=last-first;
 return delta>=6?{label:'Improving',tone:'good'}:delta<=-6?{label:'Declining',tone:'bad'}:{label:'Stable',tone:'neutral'}
}
function averageNumbersClient(values){const nums=values.map(Number).filter(Number.isFinite);return nums.length?Math.round(nums.reduce((a,b)=>a+b,0)/nums.length):null}
function lessonPriorityActivity(id,lid,cells){
 const ranked=cells.map(x=>{const rows=attempts(id,x.skill,lid).filter(a=>Number.isFinite(Number(a.score))).slice().sort((a,b)=>new Date(a.at)-new Date(b.at));if(!rows.length)return null;const latest=Number(rows.at(-1).score),first=Number(rows[0].score),low=rows.filter(a=>Number(a.score)<70).length,delta=latest-first,priority=(100-latest)+Math.max(0,low-1)*12+(delta<=-6?10:0)-(delta>=15?12:0);return {...x,latest,low,delta,priority}}).filter(Boolean).sort((a,b)=>b.priority-a.priority);
 const top=ranked[0];if(!top)return null;
 return {...top,reason:top.low>=2?'Recurring low scores':top.delta<=-6?'Performance declined on retry':top.delta>=15?'Recovering after retry':'Lowest current evidence'}
}
let activeStudentReportId=null;
let activeStudentReportLessonId=null;
function openStudentReportPage(id){activeStudentReportId=id;activeStudentReportLessonId=null;currentPage='student-performance-report';rememberCurrentPage();renderNav();renderStudentReportPage()}
function closeStudentReportPage(){activeStudentReportId=null;currentPage=session?.role==='admin'?'admin-reports':'reports';rememberCurrentPage();renderNav();renderPage()}
function renderStudentReportPage(){
 const id=activeStudentReportId,db=getDB(),st=db.users.find(u=>u.id===id);if(!st||st.role!=='student'){closeStudentReportPage();return}
 const snap=studentReportSnapshot(id),c=studentClass(id),b=bookMeta(bookIdForStudent(id));
 title(session?.role==='admin'?'System Admin':'Teacher','Student performance');
 $('content').innerHTML=`<main class="student-performance-page"><button class="ghost-btn report-page-back" id="studentReportBack">← Back to reports</button><header class="student-performance-hero"><div><p class="eyebrow">Student performance report</p><h1>${escapeHtml(st.name)}</h1><p>${escapeHtml(b?.title||'Workbook')} · ${escapeHtml(c?.name||'No class')}</p></div></header><section class="student-report-summary"><div><small>Workbook progress</small><strong>${snap.pct}%</strong></div><div><small>Overall points</small><strong>${Number.isFinite(snap.o)?snap.o+'%':'—'}</strong></div><div><small>Lessons started</small><strong>${snap.activeLessons.length}</strong></div></section><section class="performance-intelligence-card" id="performanceIntelligence"><div class="performance-intelligence-loading"><strong>Reading performance evidence…</strong><span>JEV is classifying status, trend, evidence strength, priority, persistence, urgency, and next action.</span></div></section><section class="report-360-section"><div class="section-head"><div><h2>Lessons</h2><p>Select a lesson to see how the student performed in each activity.</p></div></div>${studentReportLessonCards(id)}</section></main>`;
 $('studentReportBack').onclick=closeStudentReportPage;wireStudentReportLessons();loadStudentPerformanceDecision(id)
}
function studentReportSnapshot(id){
 const c=courseForStudent(id),lessons=readyLessons(c),done=completedActivityCount(id),total=lessons.reduce((n,l)=>n+workbookStepsForLesson(l).length,0),pct=completionPct(id),o=overall(id),all=attempts(id).slice().sort((a,b)=>new Date(b.at)-new Date(a.at)),latest=all[0]||null,w=weakest(id);
 const activeLessons=lessons.filter(l=>skillCompletionFor(id,l.id).length||workbookStepsForLesson(l).some(k=>attempts(id,k,l.id).length));
 return {done,total,pct,o,latest,w,activeLessons,all}
}
function studentReportLessonCards(id){
 const lessons=readyLessons(courseForStudent(id));
 return `<div class="student-lesson-report-list">${lessons.map(l=>{const steps=workbookStepsForLesson(l),done=skillCompletionFor(id,l.id).length,cells=steps.map(k=>workbookEvidenceFor(id,l.id,k)),scores=cells.filter(e=>Number.isFinite(e.score)).map(e=>e.score),avg=scores.length?Math.round(scores.reduce((a,b)=>a+b,0)/scores.length):null,attemptCount=cells.reduce((n,e)=>n+e.attemptCount,0),started=done>0||attemptCount>0,signal=lessonPerformanceSignal(id,l.id);return `<button type="button" class="student-lesson-report-row ${started?'is-started':'is-empty'}" data-report-lesson-card="${escapeAttr(l.id)}" data-report-student="${escapeAttr(id)}"><span class="student-lesson-number">Lesson ${l.number}</span><span class="student-lesson-name">${escapeHtml(l.title)}</span><span class="student-lesson-points"><strong>${Number.isFinite(avg)?avg+'%':'—'}</strong><small>${started?`${done}/${steps.length} activities · ${attemptCount} attempt${attemptCount===1?'':'s'} · ${signal.label}`:'Not started'}</small></span><span class="student-lesson-open">View lesson →</span></button>`}).join('')}</div>`
}
function openStudentLesson360(id,lid){activeStudentReportId=id;activeStudentReportLessonId=lid;currentPage='student-lesson-performance';rememberCurrentPage();renderNav();renderStudentLessonPage()}
function renderStudentLessonPage(){
 const id=activeStudentReportId,lid=activeStudentReportLessonId,st=getDB().users.find(u=>u.id===id),l=lessonById(lid);if(!st||!l){openStudentReportPage(id);return}
 const steps=workbookStepsForLesson(l),cells=steps.map(k=>({skill:k,e:workbookEvidenceFor(id,lid,k)})),scores=cells.filter(x=>x.e.latest).map(x=>x.e.score),avg=scores.length?Math.round(scores.reduce((a,b)=>a+b,0)/scores.length):null,done=skillCompletionFor(id,lid).length,all=attempts(id,null,lid).slice().sort((a,b)=>new Date(b.at)-new Date(a.at)),priority=lessonPriorityActivity(id,lid,cells);
 const lessonNarrative=!all.length
 ?'The student has not yet submitted enough work in this lesson to describe performance.'
 :`The student has completed ${done} of ${steps.length} workbook activities in this lesson. ${Number.isFinite(avg)?`The current lesson average is ${avg}%.`:'There is not yet enough scored evidence for a lesson average.'} ${priority?`${skillLabel(priority.skill)} is the clearest area needing attention because ${priority.reason.toLowerCase()}.`:'No clear weak activity has emerged yet.'} ${lessonPerformanceSignal(id,lid).label==='Improving'?'Recent attempts are improving.':lessonPerformanceSignal(id,lid).label==='Declining'?'Recent attempts are declining and should be reviewed with the learner.':lessonPerformanceSignal(id,lid).label==='Stable'?'Recent performance is stable.':'A clear trend has not yet formed.'}`;
 title(session?.role==='admin'?'System Admin':'Teacher','Lesson performance');
 $('content').innerHTML=`<main class="student-performance-page lesson-performance-page"><button class="ghost-btn report-page-back" id="reportBackToStudent">← Back to ${escapeHtml(st.name)}</button><header class="student-performance-hero"><div><p class="eyebrow">${escapeHtml(st.name)} · Lesson ${l.number}</p><h1>${escapeHtml(l.title)}</h1><p>Performance in each workbook activity for this lesson.</p></div></header><section class="lesson-written-summary"><span class="role-kicker">Teacher lesson summary</span><p>${escapeHtml(lessonNarrative)}</p></section><div class="report-kpi-strip"><div><small>Completion</small><strong>${Math.round(done/steps.length*100)}%</strong><span>${done}/${steps.length} activities</span></div><div><small>Lesson score</small><strong>${Number.isFinite(avg)?avg+'%':'—'}</strong><span>${scores.length} scored activities</span></div><div><small>Total attempts</small><strong>${all.length}</strong><span>Across this lesson</span></div><div><small>Needs attention</small><strong>${priority?escapeHtml(workbookStageLabel(l,priority.skill)):'—'}</strong><span>${priority?escapeHtml(priority.reason)+' · '+priority.latest+'% latest':'No scored weakness yet'}</span></div></div><section class="report-360-section"><div class="section-head"><div><h2>Activity performance</h2><p>Select an activity to inspect the student's saved evidence and attempts.</p></div></div><div class="report-activity-grid">${cells.map(x=>{const e=x.e,state=e.completed?'Completed':e.latest?'Attempted':'Not started';return `<button type="button" class="report-activity-card ${e.completed?'is-complete':e.latest?'is-attempted':'is-empty'}" data-report-activity="${escapeAttr(x.skill)}" data-report-lesson="${escapeAttr(lid)}" data-report-student="${escapeAttr(id)}"><span>${escapeHtml(workbookStageLabel(l,x.skill))}</span><strong>${Number.isFinite(e.score)?e.score+'%':state}</strong><small>${e.attemptCount?e.attemptCount+' attempt'+(e.attemptCount===1?'':'s'):state} · View evidence</small></button>`}).join('')}</div></section><section class="report-360-section"><div class="section-head"><div><h2>Learning journey</h2><p>Retries and scores for this lesson in chronological order.</p></div></div>${all.length?`<div class="report-timeline">${all.slice().reverse().map(a=>`<div class="report-timeline-item"><span></span><div><strong>${escapeHtml(skillLabel(a.skill))} · ${Math.round(Number(a.score))}%</strong><small>${new Date(a.at).toLocaleString()}</small></div></div>`).join('')}</div>`:'<div class="empty-state"><h4>No lesson evidence yet</h4><p>The learner has not submitted work in this lesson.</p></div>'}</section></main>`;
 $('reportBackToStudent').onclick=()=>openStudentReportPage(id);wireWorkbookReportInteractions()
}
function wireStudentReportLessons(){document.querySelectorAll('[data-report-lesson-card]').forEach(b=>b.onclick=()=>openStudentLesson360(b.dataset.reportStudent,b.dataset.reportLessonCard))}
function openStudentDetail(id){
 const db=getDB(),st=db.users.find(u=>u.id===id);if(!st||st.role!=='student')return;const snap=studentReportSnapshot(id),c=studentClass(id),b=bookMeta(bookIdForStudent(id)),skills=['vocabulary','listening','grammar','writing'];
 showModal(`<div class="student-performance-shell student-performance-simple"><header class="student-performance-hero"><div><p class="eyebrow">Student performance report</p><h2>${escapeHtml(st.name)}</h2><p>${escapeHtml(b?.title||'Workbook')} · ${escapeHtml(c?.name||'No class')}</p></div><button class="icon-btn" data-close>×</button></header><section class="student-report-summary"><div><small>Workbook progress</small><strong>${snap.pct}%</strong></div><div><small>Overall points</small><strong>${Number.isFinite(snap.o)?snap.o+'%':'—'}</strong></div><div><small>Lessons started</small><strong>${snap.activeLessons.length}</strong></div></section><section class="report-360-section"><div class="section-head"><div><h3>Lessons</h3><p>Each row summarizes the points earned in that lesson. Select a lesson to see performance in each activity.</p></div></div>${studentReportLessonCards(id)}</section></div>`)
 document.querySelector('#modalRoot [data-close]')?.addEventListener('click',closeModal);wireStudentReportLessons();wireWorkbookReportInteractions();$('resetPass')?.addEventListener('click',()=>openStudentPasswordHelp(id));document.querySelectorAll('[data-view-student-certificate]').forEach(btn=>btn.onclick=()=>{const cert=(getDB().certificates||[]).find(x=>x.studentId===btn.dataset.viewStudentCertificate&&x.bookId===bookIdForStudent(btn.dataset.viewStudentCertificate));if(cert)renderLevelCertificate(cert)});if(session?.role==='teacher'&&$('saveFinalWritingGrade'))$('saveFinalWritingGrade').onclick=async()=>{const score=Number($('finalWritingGrade').value),result=$('writingGradeResult');try{await api(`/api/teacher/writing/${id}/su-b2-l22/grade`,{method:'POST',body:JSON.stringify({score})});await refreshState();result.innerHTML=`<div class="feedback good">Final writing grade saved: ${score}%.</div>`}catch(e){result.innerHTML=`<div class="feedback bad">${escapeHtml(e.message)}</div>`}}
}

function classes(){const cls=getDB().classes;title('Teacher','Classes');$('content').innerHTML=`<div class="role-page-head"><div><span class="role-kicker">Assigned to you</span><h1>Your classes</h1><p>Each class has one book. You can have multiple classes, including different books, and you add students into the appropriate class.</p></div></div><div class="class-card-grid">${cls.map(c=>{const sts=classStudents().filter(st=>st.classIds?.includes(c.id)),avg=sts.length?Math.round(sts.reduce((a,st)=>a+completionPct(st.id),0)/sts.length):0,b=bookMeta(c.bookId||c.course_id);return `<article class="management-card"><div class="management-card-head"><span class="pill teal">${escapeHtml(c.level)}</span><span>${sts.length} students</span></div><h3>${escapeHtml(c.name)}</h3><p class="book-line">Book: <strong>${escapeHtml(b?.title||c.course_id)}</strong></p><p>${avg}% average recorded completion</p>${progress(avg)}<div class="management-card-actions"><button class="primary-btn" data-add-student-class="${escapeAttr(c.id)}">+ Add student</button></div></article>`}).join('')||'<div class="empty-state"><h3>No classes yet</h3><p>Your System Admin must create a class, choose its book, and assign it to you.</p></div>'}</div>`;document.querySelectorAll('[data-add-student-class]').forEach(b=>b.onclick=()=>openAddStudent(b.dataset.addStudentClass))}
...
function adminTestApp(){
 ...
 document.querySelectorAll('[data-admin-test-live]').forEach(btn=>btn.onclick=async()=>{await ensureLiveBooks();if(!setActiveBook(btn.dataset.adminTestLive))return;activeTeacherLessonNumber=1;activeTeacherSectionIndex=0;currentPage='admin-live-book-browse';renderNav();adminLiveBookBrowse()});
 document.querySelectorAll('[data-admin-test-workbook]').forEach(btn=>btn.onclick=()=>{if(!setActiveBook(btn.dataset.adminTestWorkbook))return;currentPage='admin-book-browse';renderNav();adminBookBrowse()});
}
function adminBooks(){
...
async function adminLiveLesson(){
 ...
 wireLiveVocabulary();wireLiveChecks();wireLiveAudioPlayers();
}
...
function adminHome(){const admins=adminUsers('admin'),teachers=adminUsers('teacher'),students=adminUsers('student'),cls=getDB().classes,books=getDB().books||[],liveBooks=books.filter(b=>['ready','pilot'].includes(b.status)).length;title('System Admin','Overview');$('content').innerHTML=`<section class="role-hero admin-hero"><div><span class="role-kicker">EnglishGate setup</span><h1>Books power the classes.</h1><p>Create one class for one book, assign one teacher, then the teacher enrolls students into that class. A teacher can have several classes for different books.</p></div><button class="primary-btn" id="adminOpenBooks">Open books</button></section><section class="admin-metrics">${metric('Books',books.length,liveBooks+' usable now')}${metric('Classes',cls.length,'Book-based cohorts')}${metric('Admins',admins.length,'System access')}${metric('Teachers',teachers.length,'Accounts')}${metric('Students',students.length,'Accounts')}</section><section class="section setup-flow"><div class="setup-step"><b>1</b><div><strong>Books</strong><span>Choose the curriculum.</span></div><button id="goBooks" class="text-link">Open</button></div><div class="setup-step"><b>2</b><div><strong>Classes</strong><span>One class = one book + one teacher.</span></div><button id="goClasses" class="text-link">Open</button></div><div class="setup-step"><b>3</b><div><strong>Teachers & students</strong><span>Teachers add students to their assigned classes.</span></div><button id="goAdmins" class="text-link">Open</button></div></section>`;const go=(page)=>{currentPage=page;renderNav();renderPage()};$('adminOpenBooks').onclick=()=>go('admin-books');$('goBooks').onclick=()=>go('admin-books');$('goClasses').onclick=()=>go('admin-classes');$('goAdmins').onclick=()=>go('admin-admins')}
function adminClasses(){const cls=getDB().classes,teachers=adminUsers('teacher');title('System Admin','Classes');$('content').innerHTML=`<div class="role-page-head"><div><span class="role-kicker">Book-based classes</span><h1>Classes</h1><p>Admin creates the class, chooses one book, and assigns one teacher. The teacher then adds students to that class.</p></div><button class="primary-btn" id="newAdminClass">+ Create class</button></div><div class="feedback good"><strong>Class rule:</strong> One class = one teacher + one book. If the same teacher starts another book, create another class so the old level history stays intact.</div><div class="class-card-grid">${cls.map(c=>{const t=teachers.find(x=>x.id===c.teacher_id),count=adminUsers('student').filter(st=>st.classIds?.includes(c.id)).length,b=bookMeta(c.bookId||c.course_id);return `<article class="management-card"><div class="management-card-head"><span class="pill teal">${escapeHtml(c.level)}</span><span>${count} students</span></div><h3>${escapeHtml(c.name)}</h3><p class="book-line">Book: <strong>${escapeHtml(b?.title||c.course_id)}</strong></p><p>Teacher: <strong>${escapeHtml(t?.name||'Unassigned')}</strong></p><p class="muted">This book stays attached to this class for its learning history.</p><div class="management-card-actions"><button class="ghost-btn danger-action" data-admin-delete-class="${escapeAttr(c.id)}" data-name="${escapeAttr(c.name)}" data-students="${count}">Delete class</button></div></article>`}).join('')||'<div class="empty-state"><h3>No classes</h3><p>Create a class by choosing its book and teacher. The teacher can then add students.</p></div>'}</div>`;$('newAdminClass').onclick=()=>openAdminClass();bindAdminClassActions()}
...
function adminTeachers(){const teachers=adminUsers('teacher'),cls=getDB().classes;title('System Admin','Teachers');$('content').innerHTML=`<div class="role-page-head"><div><span class="role-kicker">Staff</span><h1>Teachers</h1><p>Create teacher accounts and see assigned classes.</p></div><button class="primary-btn" id="newAdminTeacher">+ Create teacher</button></div><div class="card clean-list">${teachers.map(t=>{const names=cls.filter(c=>c.teacher_id===t.id).map(c=>c.name);return `<div class="management-row"><div class="student-cell"><div class="avatar">${escapeHtml(t.name[0])}</div><div><strong>${escapeHtml(t.name)}</strong><small>@${escapeHtml(t.username)}</small><small>WhatsApp: ${escapeHtml(t.whatsappNumber||'Not recorded')}</small></div></div><div class="management-row-meta"><span>${escapeHtml(names.join(', ')||'No class assigned')}</span><button class="ghost-btn" data-admin-reset="${t.id}">Reset password</button><button class="ghost-btn danger-action" data-admin-delete="${t.id}" data-name="${escapeAttr(t.name)}" data-role="teacher">Delete</button></div></div>`}).join('')||'<div class="empty-state"><h3>No teachers</h3><p>Create the first teacher account.</p></div>'}</div>`;$('newAdminTeacher').onclick=openAdminTeacher;bindAdminUserActions()}
function adminStudents(){const students=adminUsers('student'),cls=getDB().classes;title('System Admin','Students');$('content').innerHTML=`<div class="role-page-head"><div><span class="role-kicker">Student oversight</span><h1>Students</h1><p>Teachers create students inside their assigned classes. Admin can review, move, reset access, or remove existing students.</p></div></div><div class="card table-wrap clean-table"><table class="data-table"><thead><tr><th>Student</th><th>Class</th><th></th></tr></thead><tbody>${students.map(s=>`<tr><td><div class="student-cell">${studentAvatarMarkup(studentPhoto(s.id),s.name,'roster-avatar')}<div><button class="student-name-link" data-student-report="${escapeAttr(s.id)}" type="button">${escapeHtml(s.name)}</button><small>@${escapeHtml(s.username)}</small><small>WhatsApp: ${escapeHtml(s.whatsappNumber||'Not recorded')}</small></div></div></td><td>${escapeHtml(cls.find(c=>s.classIds?.includes(c.id))?.name||'—')}</td><td><div class="management-row-meta"><button class="primary-btn compact-action" data-manage-student="${escapeAttr(s.id)}">Manage</button><button class="ghost-btn" data-review-student="${escapeAttr(s.id)}">View report</button><button class="ghost-btn danger-action" data-admin-delete="${escapeAttr(s.id)}" data-name="${escapeAttr(s.name)}" data-role="student">Delete</button></div></td></tr>`).join('')||'<tr><td colspan="3">No students yet. A teacher can add the first student from their Students or Classes page.</td></tr>'}</tbody></table></div>`;bindAdminUserActions();bindStudentActions()}
function openAdminTeacher(){showModal(`<div class="section-head"><div><span class="role-kicker">Teacher account</span><h3>Create teacher</h3></div><button class="icon-btn" data-close>×</button></div><form id="adminTeacherForm" class="form-grid"><label>Teacher name<input id="adminTeacherName" required></label><label>WhatsApp number<input id="adminTeacherWhatsapp" type="tel" autocomplete="tel" required maxlength="40" placeholder="+252 63 1234567"><small>Include the country code, starting with +.</small></label><label>Username<input id="adminTeacherUsername" required placeholder="e.g. teacher.ahmed"></label>${manualPasswordField('adminTeacherPassword')}<button class="primary-btn" type="submit">Create teacher</button></form><div id="adminCreateResult"></div>`);document.querySelector('[data-close]').onclick=closeModal;$('adminTeacherForm').onsubmit=async e=>{e.preventDefault();try{const r=await api('/api/admin/teachers',{method:'POST',body:JSON.stringify({name:$('adminTeacherName').value.trim(),username:$('adminTeacherUsername').value.trim(),whatsappNumber:$('adminTeacherWhatsapp').value.trim(),password:$('adminTeacherPassword').value})});await refreshState();$('adminCreateResult').innerHTML=credentialResult('Teacher',r);bindCredentialActions();openWhatsAppLink(r.whatsappLink)}catch(err){$('adminCreateResult').innerHTML=`<div class="feedback bad">${escapeHtml(err.message)}</div>`}}}
function openAdminClass(prefillBookId=''){const teachers=adminUsers('teacher'),books=(getDB().books||[]).filter(b=>['ready','pilot'].includes(b.status)&&BOOK_PACKS[b.id]);if(!teachers.length){showModal(`<div class="section-head"><div><span class="role-kicker">Create class</span><h3>Create a teacher first</h3><p class="muted">A class needs an assigned teacher.</p></div><button class="icon-btn" data-close>×</button></div><button class="primary-btn" id="goCreateTeacher">Create teacher</button>`);document.querySelector('[data-close]').onclick=closeModal;$('goCreateTeacher').onclick=openAdminTeacher;return}if(!books.length){showModal(`<div class="section-head"><div><span class="role-kicker">Create class</span><h3>No ready book</h3><p class="muted">A class must be connected to a ready curriculum book.</p></div><button class="icon-btn" data-close>×</button></div>`);document.querySelector('[data-close]').onclick=closeModal;return}const selected=books.some(b=>b.id===prefillBookId)?prefillBookId:books[0].id;showModal(`<div class="section-head"><div><span class="role-kicker">Class setup</span><h3>Create class</h3><p class="muted">One class has one book and one teacher. If this teacher later teaches another book, create another class.</p></div><button class="icon-btn" data-close>×</button></div><form id="adminClassForm" class="form-grid"><label>Class name<input id="adminClassName" required placeholder="e.g. A2-B1 Evening"></label><label>Book<select id="adminClassBook">${books.map(b=>`<option value="${b.id}" ${b.id===selected?'selected':''}>${escapeHtml(b.title)} · ${escapeHtml(b.level)}</option>`).join('')}</select></label><label>Teacher<select id="adminClassTeacher">${teachers.map(t=>`<option value="${t.id}">${escapeHtml(t.name)}</option>`).join('')}</select></label><button class="primary-btn" type="submit">Create class & assign teacher</button></form><div id="adminCreateResult"></div>`);document.querySelector('[data-close]').onclick=closeModal;$('adminClassForm').onsubmit=async e=>{e.preventDefault();try{await api('/api/admin/classes',{method:'POST',body:JSON.stringify({name:$('adminClassName').value.trim(),bookId:$('adminClassBook').value,teacherId:$('adminClassTeacher').value})});await refreshState();closeModal();currentPage='admin-classes';renderNav();adminClasses()}catch(err){$('adminCreateResult').innerHTML=`<div class="feedback bad">${escapeHtml(err.message)}</div>`}}}
function openAssignClassBook(classId){const c=getDB().classes.find(x=>x.id===classId),books=(getDB().books||[]).filter(b=>['ready','pilot'].includes(b.status)&&BOOK_PACKS[b.id]);if(!c)return;if(!books.length){showModal(`<div class="section-head"><div><span class="role-kicker">Assign book</span><h3>No ready books</h3><p class="muted">Add or restore a ready book before assigning it to a class.</p></div><button class="icon-btn" data-close>×</button></div>`);document.querySelector('[data-close]').onclick=closeModal;return}showModal(`<div class="section-head"><div><span class="role-kicker">Assign book</span><h3>${escapeHtml(c.name)}</h3><p class="muted">All students in this class will see this book.</p></div><button class="icon-btn" data-close>×</button></div><form id="assignClassBookForm" class="form-grid"><label>Class book<select id="assignClassBook">${books.map(b=>`<option value="${escapeAttr(b.id)}" ${b.id===(c.bookId||c.course_id)?'selected':''}>${escapeHtml(b.title)} · ${escapeHtml(b.level)}</option>`).join('')}</select></label><button class="primary-btn" type="submit">Save class book</button></form><p class="muted">Changing the class book clears old lesson assignments for this class, but student accounts and saved performance records remain.</p><div id="assignClassBookResult"></div>`);document.querySelector('[data-close]').onclick=closeModal;$('assignClassBookForm').onsubmit=async e=>{e.preventDefault();const btn=e.submitter||$('assignClassBookForm').querySelector('button[type="submit"]');btn.disabled=true;btn.textContent='Saving...';try{await api(`/api/admin/classes/${classId}/book`,{method:'PATCH',body:JSON.stringify({bookId:$('assignClassBook').value})});await refreshState();closeModal();adminClasses()}catch(err){btn.disabled=false;btn.textContent='Save class book';$('assignClassBookResult').innerHTML=`<div class="feedback bad">${escapeHtml(err.message)}</div>`}}}
function manualPasswordField(id){return `<label>Password (optional)<input id="${id}" type="password" inputmode="numeric" pattern="[0-9]{8,20}" autocomplete="new-password" minlength="8" maxlength="20" placeholder="Enter numbers or leave blank"><small>Use 8–20 numbers only. Blank creates a secure numeric password automatically.</small></label>`}
function openAdminStudent(){const cls=getDB().classes;if(!cls.length){showModal(`<div class="section-head"><div><span class="role-kicker">Add student</span><h3>Create a class first</h3><p class="muted">Students must belong to a class.</p></div><button class="icon-btn" data-close>×</button></div>`);document.querySelector('[data-close]').onclick=closeModal;return}showModal(`<div class="section-head"><div><span class="role-kicker">Student account</span><h3>Add initial student</h3></div><button class="icon-btn" data-close>×</button></div><form id="adminStudentForm" class="form-grid"><label>Student name<input id="adminStudentName" required></label><label>WhatsApp number<input id="adminStudentWhatsapp" type="tel" autocomplete="tel" required maxlength="40" placeholder="+252 63 1234567"><small>Include the country code, starting with +.</small></label><label>Username<input id="adminStudentUsername" required placeholder="e.g. amina.ali"></label>${manualPasswordField('adminStudentPassword')}<label>Class<select id="adminStudentClass">${cls.map(c=>`<option value="${c.id}">${escapeHtml(c.name)}</option>`).join('')}</select></label><button class="primary-btn" type="submit">Create student</button></form><div id="adminCreateResult"></div>`);document.querySelector('[data-close]').onclick=closeModal;$('adminStudentForm').onsubmit=async e=>{e.preventDefault();try{const r=await api('/api/admin/students',{method:'POST',body:JSON.stringify({name:$('adminStudentName').value.trim(),username:$('adminStudentUsername').value.trim(),classId:$('adminStudentClass').value,whatsappNumber:$('adminStudentWhatsapp').value.trim(),password:$('adminStudentPassword').value})});await refreshState();$('adminCreateResult').innerHTML=credentialResult('Student',r);bindCredentialActions();openWhatsAppLink(r.whatsappLink)}catch(err){$('adminCreateResult').innerHTML=`<div class="feedback bad">${escapeHtml(err.message)}</div>`}}}
function credentialResult(label,r){const title=label==='Password reset'?'New password ready':`${label} created`,extra=r.whatsappMessage?`<div class="whatsapp-delivery"><strong>WhatsApp message ready</strong><small>${r.className?`Class: ${escapeHtml(r.className)} · `:''}${r.courseName?`Course: ${escapeHtml(r.courseName)}`:''}</small><textarea class="whatsapp-message" readonly>${escapeHtml(r.whatsappMessage)}</textarea><div class="credential-actions">${r.whatsappLink?`<button class="primary-btn" type="button" data-open-whatsapp-link="${escapeAttr(r.whatsappLink)}">Open WhatsApp</button>`:''}<button class="ghost-btn" type="button" data-copy-whatsapp>Copy message</button></div></div>`:'';return `<div class="credential-card"><span>${title}</span><strong>@${escapeHtml(r.username)}</strong><code>${escapeHtml(r.temporaryPassword)}</code><small>${r.passwordWasGenerated?'A secure password was generated. ':''}${label==='Password reset'?'Use this password to sign in, then change it later if needed.':'Copy this password now. It will not be shown again.'}</small>${extra}</div>`}
function openWhatsAppLink(link){if(link)window.open(link,'_blank','noopener')}
function bindCredentialActions(){document.querySelectorAll('[data-open-whatsapp-link]').forEach(b=>b.onclick=()=>openWhatsAppLink(b.dataset.openWhatsappLink));document.querySelectorAll('[data-copy-whatsapp]').forEach(b=>b.onclick=async()=>{const box=b.closest('.whatsapp-delivery')?.querySelector('.whatsapp-message');if(!box)return;try{await navigator.clipboard.writeText(box.value);b.textContent='Copied'}catch{box.select();document.execCommand('copy');b.textContent='Copied'}})}
async function adminResetPassword(id){try{const r=await api(`/api/admin/users/${id}/reset-password`,{method:'POST'});showModal(`<div class="section-head"><div><span class="role-kicker">Password reset</span><h3>New temporary password</h3></div><button class="icon-btn" data-close>×</button></div>${credentialResult('Password reset',r)}`);document.querySelector('[data-close]').onclick=closeModal;bindCredentialActions();openWhatsAppLink(r.whatsappLink)}catch(e){alert(e.message)}}
function openAdminDelete(id,name,role){const labels={admin:'admin',teacher:'teacher',student:'student'},label=labels[role]||'user',effect=role==='admin'?'This removes their system admin access and sign-in account. At least one other admin must remain.':role==='teacher'?'Their classes and assignments will remain, but their classes will become unassigned.':'Their enrollment, progress, scores, and workbook responses will also be deleted.';showModal(`<div class="section-head"><div><span class="role-kicker">Permanent action</span><h3>Delete ${escapeHtml(name)}?</h3></div><button class="icon-btn" data-close>×</button></div><p>${effect}</p><p class="muted">This cannot be undone.</p><div class="delete-user-actions"><button class="ghost-btn" id="cancelAdminDelete">Cancel</button><button class="primary-btn danger-button" id="confirmAdminDelete">Delete ${label}</button></div><div id="adminDeleteResult"></div>`);document.querySelector('[data-close]').onclick=closeModal;$('cancelAdminDelete').onclick=closeModal;$('confirmAdminDelete').onclick=async()=>{const button=$('confirmAdminDelete');button.disabled=true;button.textContent='Deleting...';try{await api(`/api/admin/users/${id}`,{method:'DELETE'});await refreshState();closeModal();renderPage()}catch(e){button.disabled=false;button.textContent=`Delete ${label}`;$('adminDeleteResult').innerHTML=`<div class="feedback bad">${escapeHtml(e.message)}</div>`}}}
function openAdminClassDelete(id,name,studentCount){showModal(`<div class="section-head"><div><span class="role-kicker">Permanent action</span><h3>Delete class ${escapeHtml(name)}?</h3></div><button class="icon-btn" data-close>×</button></div><p>This deletes the class, its roster links, and its workbook assignments. User accounts stay in the system.</p><p class="muted">${studentCount} student${studentCount===1?' is':'s are'} currently enrolled. This cannot be undone.</p><div class="delete-user-actions"><button class="ghost-btn" id="cancelAdminDelete">Cancel</button><button class="primary-btn danger-button" id="confirmAdminDelete">Delete class</button></div><div id="adminDeleteResult"></div>`);document.querySelector('[data-close]').onclick=closeModal;$('cancelAdminDelete').onclick=closeModal;$('confirmAdminDelete').onclick=async()=>{const button=$('confirmAdminDelete');button.disabled=true;button.textContent='Deleting...';try{await api(`/api/admin/classes/${id}`,{method:'DELETE'});await refreshState();closeModal();renderPage()}catch(e){button.disabled=false;button.textContent='Delete class';$('adminDeleteResult').innerHTML=`<div class="feedback bad">${escapeHtml(e.message)}</div>`}}}
function openAdminBookDelete(id,name){const used=getDB().classes.filter(c=>(c.bookId||c.course_id)===id).length;showModal(`<div class="section-head"><div><span class="role-kicker">Permanent action</span><h3>Delete book ${escapeHtml(name)}?</h3></div><button class="icon-btn" data-close>×</button></div><p>This removes the book from the admin library and prevents it from being used for new classes.</p><p class="muted">${used?`${used} class${used===1?' is':'es are'} still using this book. Delete those classes first.`:'No class is currently using this book.'} This cannot be undone.</p><div class="delete-user-actions"><button class="ghost-btn" id="cancelAdminDelete">Cancel</button><button class="primary-btn danger-button" id="confirmAdminDelete" ${used?'disabled':''}>Delete book</button></div><div id="adminDeleteResult"></div>`);document.querySelector('[data-close]').onclick=closeModal;$('cancelAdminDelete').onclick=closeModal;if(!$('confirmAdminDelete').disabled)$('confirmAdminDelete').onclick=async()=>{const button=$('confirmAdminDelete');button.disabled=true;button.textContent='Deleting...';try{await api(`/api/admin/books/${id}`,{method:'DELETE'});await refreshState();closeModal();renderPage()}catch(e){button.disabled=false;button.textContent='Delete book';$('adminDeleteResult').innerHTML=`<div class="feedback bad">${escapeHtml(e.message)}</div>`}}}
