...
function liveSectionHeading(line){
 ...
 const patterns=[
  /^(?:PAGE\s+\d+\s*[—-]\s*)?(WARM UP|VOCABULARY|READING|LANGUAGE FOCUS|FLUENCY MISSION|REFLECTION(?:\s*&\s*HOMEWORK)?|HOMEWORK)$/i,
...
function teacherLiveLesson(){
 ...
 $('content').innerHTML=`<section class="eg-lesson">
  ...
  <div class="teacher-annotation-stage" id="teacherAnnotationStage" data-annotation-mode="${teacherLiveTool}">
   <article class="live-book-content eg-stage-content ${/reading/i.test(stageName)?'is-reading-stage':''} ${/language focus/i.test(stageName)?'is-language-focus-stage':''}" aria-labelledby="liveStageTitle">${activeTeacherSectionIndex===0?lessonVisualHtml(l):''}${section?renderLiveContent(liveSectionContent(section).split('\n').filter(line=>!/^LESSON\s+\d+|^WEEK\s+\d+.*LESSON\s+\d+/i.test(line.trim())).join('\n')):'<p>This lesson has no teaching content yet. Return to the book and choose another lesson.</p>'}</article>
