...
function openStudentDetail(id){const db=getDB(),st=db.users.find(u=>u.id===id);if(!st||st.role!=='student')return;const w=weakest(id),ats=attempts(id).slice().sort((a,b)=>new Date(b.at)-new Date(a.at)).slice(0,10),c=studentClass(id),b=bookMeta(bookIdForStudent(id)),completed=completedActivityCount(id),scored=scoredActivityCount(id);showModal(`<div class="section-head student-report-head"><div><p class="eyebrow">Student report · ${escapeHtml(b?.title||'Workbook')}</p><h3>${escapeHtml(st.name)}</h3><p>WhatsApp: ${escapeHtml(st.whatsappNumber||'Not recorded')}</p><p class="muted">${escapeHtml(c?.name||'')}</p></div><button class="icon-btn" data-close>×</button></div><div class="section card report-source-note"><strong>Workbook evidence</strong><span class="muted"> This page reports the work saved from the student's workbook: completion, latest score and attempts for each activity.</span></div><div class="grid grid-4 compact-grid">${['vocabulary','listening','grammar','writing'].map(k=>skillCard(skillLabel(k),mastery(id,k),'Latest lesson scores')).join('')}</div><div class="section card"><strong>${completed} workbook activities completed</strong><span class="muted"> · ${scored} activities with a saved score</span></div>${w?`<div class="weakness section"><strong>Lowest current workbook area: ${w.label} · ${w.score}%</strong><span>${w.tip}</span></div>`:'<div class="weakness section"><strong>No scored workbook evidence yet.</strong></div>'}${studentPerformanceTable(id)}${certificateStatusHtml(id)}${finalWritingAssessmentHtml(id)}<div class="section-head section"><div><h3>Recent workbook attempts</h3><p>Newest saved attempts from the activities above.</p></div><button class="ghost-btn" id="resetPass">Password help</button></div>${ats.length?ats.map(a=>`<div class="attempt-row"><div><strong>Lesson ${lessonById(a.lessonId)?.number||'—'} · ${skillLabel(a.skill)} ${(a.tags||[]).some(t=>String(t).startsWith('boost:'))?'<span class="pill teal">Boost</span>':''}</strong><span>${new Date(a.at).toLocaleString()}</span></div><strong>${a.score}%</strong></div>`).join(''):'<p class="muted">No workbook attempts saved yet.</p>'}<div id="resetResult"></div>`);document.querySelector('[data-close]').onclick=closeModal;wireWorkbookReportInteractions();$('resetPass').onclick=()=>openStudentPasswordHelp(id);document.querySelectorAll('[data-view-student-certificate]').forEach(b=>b.onclick=()=>{const cert=(getDB().certificates||[]).find(c=>c.studentId===b.dataset.viewStudentCertificate&&c.bookId===bookIdForStudent(b.dataset.viewStudentCertificate));if(cert)renderLevelCertificate(cert)});if(session?.role==='teacher'&&$('saveFinalWritingGrade'))$('saveFinalWritingGrade').onclick=async()=>{const score=Number($('finalWritingGrade').value),result=$('writingGradeResult');try{await api(`/api/teacher/writing/${id}/su-b2-l22/grade`,{method:'POST',body:JSON.stringify({score})});await refreshState();result.innerHTML=`<div class="feedback good">Final writing grade saved: ${score}%.</div>`}catch(e){result.innerHTML=`<div class="feedback bad">${escapeHtml(e.message)}</div>`}}}
