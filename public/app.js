...
function wireLiveAudioPlayers(root=document){
 root.querySelectorAll('[data-live-audio-player]').forEach(player=>{
  ...
  const sync=audio=>{
   if(current)current.textContent=fmt(audio.currentTime);if(duration)duration.textContent=fmt(audio.duration);
  ...
  const getAudio=async()=>{
   ...
   const audio=await ensureLiveLessonAudio(player);audio.playbackRate=Number(speed?.value)||1;
   audio.onloadedmetadata=()=>sync(audio);audio.ontimeupdate=()=>sync(audio);
   audio.onplay=()=>{if(status)status.textContent='Playing';sync(audio)};
...
function liveSectionHeading(line){
 ...
 const patterns=[
  /^(?:PAGE\s+\d+\s*[—-]\s*)?(WARM UP|VOCABULARY|READING|LANGUAGE FOCUS|LISTENING|FLUENCY MISSION|CLASSROOM CHALLENGE|REFLECTION(?:\s*&\s*HOMEWORK)?|HOMEWORK)$/i,
...
function teacherLiveLesson(){
 ...
 $('content').innerHTML=`<section class="eg-lesson">
  ...
  <div class="teacher-annotation-stage" id="teacherAnnotationStage" data-annotation-mode="${teacherLiveTool}">
   <article class="live-book-content eg-stage-content ${/reading/i.test(stageName)?'is-reading-stage':''} ${/listening/i.test(stageName)?'is-listening-stage':''} ${/language focus/i.test(stageName)?'is-language-focus-stage':''}" aria-labelledby="liveStageTitle">${activeTeacherSectionIndex===0?lessonVisualHtml(l):''}${section?renderLiveContent(liveSectionContent(section).split('\n').filter(line=>!/^LESSON\s+\d+|^WEEK\s+\d+.*LESSON\s+\d+/i.test(line.trim())).join('\n')):'<p>This lesson has no teaching content yet. Return to the book and choose another lesson.</p>'}</article>
