...
function naturalListeningQuestion(source,responder,previousSource=''){
 ...
 if(!raw)return'';
 raw=raw.replace(/^(?:[A-Z][A-Za-z'’-]*,\s*)/,'').replace(/^(?:Sorry|Excuse me),\s*/i,'').trim();
 if(/^how do you spell that$/i.test(raw))return 'How does '+responder+' spell the name?';
 if(/^was that b or d$/i.test(raw))return 'Was the letter B or D?';
 if(/^and your last name$/i.test(original))return 'What is '+possessiveName(responder)+' last name?';
 if(/^which page$/i.test(raw))return 'Which page should the class use?';
 if(/^and those by the wall$/i.test(original))return 'What are those by the wall?';
 if(/^and the break$/i.test(original))return 'What time is the break?';
 if(/^what about speaking class$/i.test(original))return 'What time does speaking class start?';
 if(/^(?:and|what about) sunday$/i.test(original))return 'What does '+responder+' do on Sunday?';
 if(/^and water$/i.test(original))return 'Is there water?';
 if(/^and today$/i.test(original))return 'Where is '+responder+' today?';
 if(/^what about café city$/i.test(original))return 'What does '+responder+' say about Café City?';
 if(/^what about speaking$/i.test(original))return 'What can '+responder+' do in speaking now?';
 if(/^is it late$/i.test(raw))return 'Is the bus late?';
 raw=raw.replace(/^And\s+(?=who\b|what\b|where\b|when\b|why\b|how\b)/i,'').trim();
 ...
 raw=raw.replace(/\bwhere were you\b/i,'where was '+responder);
 raw=raw.replace(/\bwere you\b/i,'was '+responder);
 ...
 raw=raw.replace(/\bwhich days do you\b/i,'which days does '+responder);
