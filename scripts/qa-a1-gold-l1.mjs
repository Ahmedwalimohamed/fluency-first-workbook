...
const checks=[
  ...
  ['production B2-only guard with isolated A1 preview exception',server.includes('function operationalLearningLesson')&&server.includes("process.env.A1_PREVIEW_MODE==='1'")&&server.includes("String(lessonId||'')==='a1-gold-l1'")&&server.includes('requireB2LearningLesson')],
  ['separated activity guard with isolated A1 preview exception',read('reading-listening-separation-bootstrap.js').includes('b2LessonOnly')&&read('reading-listening-separation-bootstrap.js').includes("A1_PREVIEW_MODE==='1'")],
