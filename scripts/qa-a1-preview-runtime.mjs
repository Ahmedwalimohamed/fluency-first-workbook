...
const CASES=[
 ...
 {id:'a1-gold-l18',title:'Global Connections',task:'Write an international introduction with place, languages, visit goal, and reason.',min:40,max:60,writing:'My name is Ali and I live in Borama. I speak Somali and some English. I want to visit Nairobi because my friend lives there. I like meeting visitors from different countries.',turns:['I can speaking English. What country you from? I want visit Kenya.','My name is Ali. I live in Borama. I speak Somali and some English. I want to visit Kenya because my friend lives there. What country are you from? What languages do you speak? The meeting place changed. Where is the community centre?']},
 {id:'a1-gold-l19',title:'Shopping & Services',task:'Write a product enquiry with item, price, size or colour, and payment or availability question.',min:30,max:50,writing:'Hello. I want to buy a large shirt. How much is it? Do you have it in blue or black? Can I pay by card, and can I have a receipt, please?',turns:['You have blue this? Can I trying this? I pay card.','How much is this shirt? Do you have this in blue, large? Blue is unavailable, so do you have black? I will take it. Can I pay by card? Can I have a receipt, please?']},
 {id:'a1-gold-l20',title:'Home & Daily Life',task:'Write a home-and-routine message with rooms, routine time, and visitor detail.',min:40,max:60,writing:'There are two bedrooms in my home. The kitchen is next to the living room. I wake up at six and usually get home at five. Today I am late, so I will be home at six.',turns:['There is two bedrooms. I wake up in six. What time you get home?','There are two bedrooms and the kitchen is next to the living room. I wake up at six. What time do you get home? Today I am late, so I will get home at six.']},
 {id:'a1-gold-l21',title:'Plans & Events',task:'Write an invitation or arrangement with activity, day/time, place, and availability question.',min:35,max:55,writing:'Are you free on Saturday? I want to go to the community event. Let us meet at four at the town hall. If the event time changes, we can meet later or choose another activity.',turns:['Do you free Saturday? I cannot to meet at three. Let us meeting at five.','Are you free on Saturday? I can meet at four. Let us meet at the town hall. What time can you meet? Where should we meet? The event starts later and I cannot stay, so let us go to the afternoon market instead.']},
 {id:'a1-gold-l22',title:'My English in the Real World',task:'Write a real-world welcome message with introduction, place/time detail, help, and next action.',min:45,max:70,writing:'Hi. My name is Ali and I live in Borama. Welcome to our English event. The community centre is next to the city market, and the event now starts at six. Can I help you? Let us meet near the entrance at five thirty.',turns:['Where the community centre is? I can to get black. Let us meeting at six.','Hi. My name is Ali. I live in Borama and I am a teacher. Where is the community centre? What time does the event start? Blue is unavailable, so black is okay. How much is it? Can you help me, please? Let us meet at six near the market. The time changed, so this is our new plan.']}
];
...
try{
 ...
 const audio=await req('/api/audio',{method:'POST',body:{lessonId:'a1-gold-l22',text:'Host: Welcome. The event now starts at six. Learner: Where is the community centre? Host: Next to the city market. Seller: Blue is unavailable, but black is available. Learner: Black is okay. How much is it?',speakers:[{name:'Host',gender:'female',voice:'nova'},{name:'Learner',gender:'male',voice:'onyx'},{name:'Seller',gender:'female',voice:'nova'}]}});
 if(audio.status===200)record('Lessons 1-22 natural dialogue audio','PASS','HTTP 200');
 else{
  ...
  record('Lessons 1-22 natural dialogue audio','FAIL','HTTP '+audio.status+' '+(ttsLines||'no upstream detail captured'));
 }
 ...
 console.log(`A1 LESSONS 1-22 RUNTIME QA: ${results.filter(x=>x.status==='PASS').length} PASS, ${failed.length} FAIL`);
 process.exitCode=failed.length?1:0;
}catch(e){console.error('A1 Lessons 1-22 runtime QA error:',e.message);const tail=childLogs.split(/\n/).slice(-100).join('\n').replace(/Bearer\s+[A-Za-z0-9._-]+/gi,'Bearer [redacted]');if(tail)console.error('A1 preview child startup tail:\n'+tail);process.exitCode=1}
