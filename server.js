...
app.get('/api/state',auth,async(req,res)=>{
 ...
 const bookRows=(await pool.query('select id,title,level,audience,status,total_lessons,activity_model from books order by case status when \'pilot\' then 1 when \'ready\' then 2 else 3 end,title')).rows; const assignmentRows=classIds.length?(await pool.query('select id,class_id,book_id,lesson_id,lesson_number,lesson_title,skills,created_at from assignments where class_id=any($1::text[]) order by created_at desc',[classIds])).rows:[]; res.json({version:7,assignments:assignmentRows.map(a=>({id:a.id,classId:a.class_id,bookId:a.book_id,lessonId:a.lesson_id,lessonNumber:a.lesson_number,lessonTitle:a.lesson_title,skills:a.skills,createdAt:a.created_at})),books:bookRows.map(b=>({id:b.id,title:b.title,level:b.level,audience:b.audience,status:b.status,totalLessons:b.total_lessons,activityModel:b.activity_model})),users,classes:classes.map(c=>({...c,bookId:c.course_id})),profiles,attempts:atRows.map(a=>({id:String(a.id),studentId:a.student_id,lessonId:a.lesson_id,skill:a.skill,score:a.score,tags:a.tags,at:a.at})),completion,writing,listeningLocks});
});
