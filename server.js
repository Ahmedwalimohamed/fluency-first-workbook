...
app.get('/api/health',async(req,res)=>{try{await pool.query('select 1');res.json({ok:true,service:'fluency-first-api',openaiTtsConfigured:Boolean(process.env.OPENAI_API_KEY),ttsModel:OPENAI_TTS_MODEL})}catch(e){res.status(503).json({ok:false})}});
