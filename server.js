...
async function initDb(){
 ...
 let t=await pool.query('select id,role from users where lower(username)=lower($1)',[tUser]);
 let tid;
 ...
 else{if(t.rows[0].role!=='teacher')throw new Error('TEACHER_USERNAME is already used by a non-teacher account');tid=t.rows[0].id;await pool.query('update users set password_hash=$1,name=$2 where id=$3',[await bcrypt.hash(tPass,12),process.env.TEACHER_NAME||'Teacher Ahmed',tid]);}
